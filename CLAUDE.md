...
```
User → Chat (GCP Cloud Run) → Azure OpenAI (GPT-5.1) → RemoteMCPHost → WSS Tunnel → Laptop → Playwright MCP → Chrome CDP
                                                                                                      ↓
User ← SSE progress ← API ← Agent ←────────────────────────────────────── Results ←─────────────────┘
                  ↕
         L2 Payment Panel (Razorpay) — collects booking cost + tip before finalizing
```

**Key principles**:
- **Azure OpenAI (GPT-5.1)** handles all chat, reasoning, and tool calling
...
```
apps/
  web/                 → Chat Interface (Cloud Run): Next.js, auth, payments, relay client
    lib/
      credential-vault/  → AES-256-GCM encrypted credential storage
      workflow-engine/    → Task state machine + pause/resume
      relay-client.ts     → WebSocket client to laptop relay
...
```
Cloud Run (slim Alpine, no browser)
  ├─ Chat UI (Next.js)
  ├─ Azure OpenAI LLM (GPT-5.1 via Azure endpoint)
  └─ RemoteMCPHost → WSS → Cloudflare Tunnel → Laptop
                                                  ├─ Relay Server (port 8765)
                                                  ├─ MCPHost → Playwright MCP
                                                  └─ Chrome Debug (CDP port 9222, Profile 3)
```
...
**Laptop's role**: Execute ALL browser actions. Chrome Debug with Profile 3 (signed-in sessions). The relay server receives tool calls from Cloud Run and executes them via Playwright MCP.
...
- **Azure OpenAI**: Single LLM provider — GPT-5.1 via Azure OpenAI
- **Relay pattern**: `MCPHost` (local stdio) vs `RemoteMCPHost` (WebSocket) — same interface, zero agent-core changes
