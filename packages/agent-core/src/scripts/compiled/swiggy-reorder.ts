/**
 * Reorder last food order from Swiggy — view order history, repeat a previous order, checkout, pay.
 *
 * Auto-generated Playwright script for swiggy-reorder.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Swiggy Reorder ──────────────────────────
    log({ step: 'Opening Swiggy Reorder...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Swiggy Reorder. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Swiggy Reorder',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Swiggy Reorder order',
      details: 'Completing your order on Swiggy Reorder',
      amountInr: 0, // Extracted from page at runtime
      description: 'swiggy-reorder order',
    });
    ...
    log({ step: 'Swiggy Reorder workflow completed', status: 'completed' });
    log({ message: 'Task completed on Swiggy Reorder. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'swiggy-reorder-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'swiggy-reorder';
