/**
 * Create and manage Google Calendar events — set title, date, time, location, invitees, reminders.
 *
 * Auto-generated Playwright script for google-calendar.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Google Calendar...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Google Calendar. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Google Calendar',
    ...
    log({ step: 'Google Calendar workflow completed', status: 'completed' });
    log({ message: 'Task completed on Google Calendar. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'google-calendar-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'google-calendar';
