/**
 * Search property to buy or rent on 99acres — filter by type, budget, location, compare listings, contact owner/builder.
 *
 * Auto-generated Playwright script for 99acres-property.
 * Site: https://www.99acres.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to 99acres Property ──────────────────────────
    log({ step: 'Opening 99acres Property...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on 99acres Property. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with 99acres Property',
    ...
    log({ step: '99acres Property workflow completed', status: 'completed' });
    log({ message: 'Task completed on 99acres Property. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, '99acres-property-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = '99acres-property';
