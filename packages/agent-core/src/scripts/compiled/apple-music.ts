/**
 * Subscribe to Apple Music India. Individual, Student, and Family plans with lossless audio and spatial audio.
 *
 * Auto-generated Playwright script for apple-music.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Apple Music...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Apple Music. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Apple Music',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Apple Music order',
      details: 'Completing your order on Apple Music',
      amountInr: 0, // Extracted from page at runtime
      description: 'apple-music order',
    });
    ...
    log({ step: 'Apple Music workflow completed', status: 'completed' });
    log({ message: 'Task completed on Apple Music. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'apple-music-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'apple-music';
