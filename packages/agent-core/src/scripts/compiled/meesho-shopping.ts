/**
 * Shop on Meesho — browse affordable products, compare sellers, buy clothing, home decor, accessories at lowest prices.
 *
 * Auto-generated Playwright script for meesho-shopping.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Meesho Shopping ──────────────────────────
    log({ step: 'Opening Meesho Shopping...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Meesho Shopping. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Meesho Shopping',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Meesho Shopping order',
      details: 'Completing your order on Meesho Shopping',
      amountInr: 0, // Extracted from page at runtime
      description: 'meesho-shopping order',
    });
    ...
    log({ step: 'Meesho Shopping workflow completed', status: 'completed' });
    log({ message: 'Task completed on Meesho Shopping. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'meesho-shopping-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'meesho-shopping';
