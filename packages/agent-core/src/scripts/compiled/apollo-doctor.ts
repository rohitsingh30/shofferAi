/**
 * Book doctor consultation on Apollo 24|7 — search specialists, choose video/in-person, book appointment.
 *
 * Auto-generated Playwright script for apollo-doctor.
 * Site: https://www.apollo247.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Apollo Doctor ──────────────────────────
    log({ step: 'Opening Apollo Doctor...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Apollo Doctor. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Apollo Doctor',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Apollo Doctor order',
      details: 'Completing your order on Apollo Doctor',
      amountInr: 0, // Extracted from page at runtime
      description: 'apollo-doctor order',
    });
    ...
    log({ step: 'Apollo Doctor workflow completed', status: 'completed' });
    log({ message: 'Task completed on Apollo Doctor. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'apollo-doctor-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'apollo-doctor';
