/**
 * Book hotels on Yatra.com — search by city/dates, compare properties, select room, fill details, pay.
 *
 * Auto-generated Playwright script for yatra-hotel.
 * Site: https://www.yatra.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Yatra Hotel ──────────────────────────
    log({ step: 'Opening Yatra Hotel...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Yatra Hotel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Yatra Hotel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Yatra Hotel order',
      details: 'Completing your order on Yatra Hotel',
      amountInr: 0, // Extracted from page at runtime
      description: 'yatra-hotel order',
    });
    ...
    log({ step: 'Yatra Hotel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Yatra Hotel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'yatra-hotel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'yatra-hotel';
