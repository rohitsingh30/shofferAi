/**
 * Research bikes on BikeWale — compare specifications, prices, mileage, reviews, find dealers, and check on-road price.
 *
 * Auto-generated Playwright script for bikewale-info.
 * Site: https://www.bikewale.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bikewale Info ──────────────────────────
    log({ step: 'Opening Bikewale Info...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bikewale Info. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bikewale Info',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bikewale Info order',
      details: 'Completing your order on Bikewale Info',
      amountInr: 0, // Extracted from page at runtime
      description: 'bikewale-info order',
    });
    ...
    log({ step: 'Bikewale Info workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bikewale Info. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bikewale-info-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bikewale-info';
