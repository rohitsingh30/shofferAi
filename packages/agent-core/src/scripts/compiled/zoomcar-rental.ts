/**
 * Rent a self-drive car on Zoomcar — search by city/dates, filter by car type, compare options, book and pay.
 *
 * Auto-generated Playwright script for zoomcar-rental.
 * Site: https://www.zoomcar.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Zoomcar Rental ──────────────────────────
    log({ step: 'Opening Zoomcar Rental...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zoomcar Rental. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zoomcar Rental',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Zoomcar Rental order',
      details: 'Completing your order on Zoomcar Rental',
      amountInr: 0, // Extracted from page at runtime
      description: 'zoomcar-rental order',
    });
    ...
    log({ step: 'Zoomcar Rental workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zoomcar Rental. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'zoomcar-rental-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'zoomcar-rental';
