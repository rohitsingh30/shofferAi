/**
 * Order cold-pressed juices, smoothies, and cleanses from RAW Pressery — one-time purchase or subscription delivery.
 *
 * Auto-generated Playwright script for raw-pressery.
 * Site: https://www.rawpressery.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Raw Pressery ──────────────────────────
    log({ step: 'Opening Raw Pressery...', status: 'running' });
    await page.goto('https://www.rawpressery.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Raw Pressery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Raw Pressery',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Raw Pressery order',
      details: 'Completing your order on Raw Pressery',
      amountInr: 0, // Extracted from page at runtime
      description: 'raw-pressery order',
    });
    ...
    log({ step: 'Raw Pressery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Raw Pressery. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'raw-pressery-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'raw-pressery';
