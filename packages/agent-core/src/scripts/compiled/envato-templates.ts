/**
 * Buy themes, templates, and digital assets on Envato/ThemeForest — WordPress themes, HTML templates, plugins, graphics, and code.
 *
 * Auto-generated Playwright script for envato-templates.
 * Site: https://themeforest.net
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Envato Templates ──────────────────────────
    log({ step: 'Opening Envato Templates...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Envato Templates. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Envato Templates',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Envato Templates order',
      details: 'Completing your order on Envato Templates',
      amountInr: 0, // Extracted from page at runtime
      description: 'envato-templates order',
    });
    ...
    log({ step: 'Envato Templates workflow completed', status: 'completed' });
    log({ message: 'Task completed on Envato Templates. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'envato-templates-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'envato-templates';
