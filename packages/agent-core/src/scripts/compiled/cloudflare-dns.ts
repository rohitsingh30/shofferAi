/**
 * Manage domains and DNS on Cloudflare — add sites, configure DNS records, enable SSL/TLS, set up page rules and security.
 *
 * Auto-generated Playwright script for cloudflare-dns.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Cloudflare Dns...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cloudflare Dns. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cloudflare Dns',
    ...
    log({ step: 'Cloudflare Dns workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cloudflare Dns. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cloudflare-dns-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cloudflare-dns';
