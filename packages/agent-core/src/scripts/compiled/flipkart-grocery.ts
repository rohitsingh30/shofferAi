/**
 * Buy groceries on Flipkart Minutes/Grocery — search items, add to cart, checkout with quick delivery.
 *
 * Auto-generated Playwright script for flipkart-grocery.
 * Site: https://www.flipkart.com/grocery-supermart-store
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Flipkart Grocery...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Flipkart Grocery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Flipkart Grocery',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Flipkart Grocery order',
      details: 'Completing your order on Flipkart Grocery',
    ...
    log({ step: 'Flipkart Grocery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Flipkart Grocery. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'flipkart-grocery-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'flipkart-grocery';
