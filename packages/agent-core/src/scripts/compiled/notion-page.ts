/**
 * Create a Notion page or database — use templates, add content blocks, customize layout, and share with collaborators.
 *
 * Auto-generated Playwright script for notion-page.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Notion Page...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Notion Page. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Notion Page',
    ...
    log({ step: 'Notion Page workflow completed', status: 'completed' });
    log({ message: 'Task completed on Notion Page. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'notion-page-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'notion-page';
