/**
 * Create an email campaign on Mailchimp — design template, add content, select audience, schedule or send immediately.
 *
 * Auto-generated Playwright script for mailchimp-newsletter.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Mailchimp Newsletter ──────────────────────────
    log({ step: 'Opening Mailchimp Newsletter...', status: 'running' });
    await page.goto('https://mailchimp.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Mailchimp Newsletter. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Mailchimp Newsletter',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Mailchimp Newsletter order',
      details: 'Completing your order on Mailchimp Newsletter',
      amountInr: 0, // Extracted from page at runtime
      description: 'mailchimp-newsletter order',
    });
    ...
    log({ step: 'Mailchimp Newsletter workflow completed', status: 'completed' });
    log({ message: 'Task completed on Mailchimp Newsletter. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'mailchimp-newsletter-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'mailchimp-newsletter';
