/**
 * Book a Rapido bike taxi — set pickup and drop, estimate fare, confirm ride, track.
 *
 * Auto-generated Playwright script for rapido-bike.
 * Site: https://www.rapido.bike
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Rapido Bike ──────────────────────────
    log({ step: 'Opening Rapido Bike...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Rapido Bike. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Rapido Bike',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Rapido Bike order',
      details: 'Completing your order on Rapido Bike',
      amountInr: 0, // Extracted from page at runtime
      description: 'rapido-bike order',
    });
    ...
    log({ step: 'Rapido Bike workflow completed', status: 'completed' });
    log({ message: 'Task completed on Rapido Bike. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'rapido-bike-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'rapido-bike';
