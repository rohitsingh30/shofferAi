/**
 * Search property on MagicBricks — filter by type, budget, location, view details, contact owner/agent.
 *
 * Auto-generated Playwright script for magicbricks-property.
 * Site: https://www.magicbricks.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Magicbricks Property ──────────────────────────
    log({ step: 'Opening Magicbricks Property...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Magicbricks Property. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Magicbricks Property',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Magicbricks Property order',
      details: 'Completing your order on Magicbricks Property',
      amountInr: 0, // Extracted from page at runtime
      description: 'magicbricks-property order',
    });
    ...
    log({ step: 'Magicbricks Property workflow completed', status: 'completed' });
    log({ message: 'Task completed on Magicbricks Property. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'magicbricks-property-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'magicbricks-property';
