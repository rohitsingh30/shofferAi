/**
 * Deploy web services on Render — static sites, APIs, background workers, PostgreSQL databases, cron jobs.
 *
 * Auto-generated Playwright script for render-deploy.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Render Deploy ──────────────────────────
    log({ step: 'Opening Render Deploy...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Render Deploy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Render Deploy',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Render Deploy order',
      details: 'Completing your order on Render Deploy',
      amountInr: 0, // Extracted from page at runtime
      description: 'render-deploy order',
    });
    ...
    log({ step: 'Render Deploy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Render Deploy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'render-deploy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'render-deploy';
