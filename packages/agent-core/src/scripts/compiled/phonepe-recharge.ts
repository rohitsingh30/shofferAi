/**
 * Mobile recharge and bill payments (electricity, gas, broadband, DTH) on PhonePe web.
 *
 * Auto-generated Playwright script for phonepe-recharge.
 * Site: https://www.phonepe.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Phonepe Recharge ──────────────────────────
    log({ step: 'Opening Phonepe Recharge...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Phonepe Recharge. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Phonepe Recharge',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Phonepe Recharge order',
      details: 'Completing your order on Phonepe Recharge',
      amountInr: 0, // Extracted from page at runtime
      description: 'phonepe-recharge order',
    });
    ...
    log({ step: 'Phonepe Recharge workflow completed', status: 'completed' });
    log({ message: 'Task completed on Phonepe Recharge. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'phonepe-recharge-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'phonepe-recharge';
