/**
 * Order from Wendy's India — burgers, chicken sandwiches, sides, combos, checkout, pay.
 *
 * Auto-generated Playwright script for wendy-order.
 * Site: https://www.wendys.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wendy Order ──────────────────────────
    log({ step: 'Opening Wendy Order...', status: 'running' });
    await page.goto('https://www.wendys.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wendy Order. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Wendy Order',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wendy Order order',
      details: 'Completing your order on Wendy Order',
      amountInr: 0, // Extracted from page at runtime
      description: 'wendy-order order',
    });
    ...
    log({ step: 'Wendy Order workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wendy Order. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wendy-order-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wendy-order';
