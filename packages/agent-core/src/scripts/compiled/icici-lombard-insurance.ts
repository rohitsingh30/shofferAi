/**
 * Buy ICICI Lombard insurance — motor, health, corporate, travel, and home insurance plans.
 *
 * Auto-generated Playwright script for icici-lombard-insurance.
 * Site: https://www.icicilombard.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Icici Lombard Insurance ──────────────────────────
    log({ step: 'Opening Icici Lombard Insurance...', status: 'running' });
    await page.goto('https://www.icicilombard.com');
    ...
    const profileEl = page.locator('[class*="profile"]').first();
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Icici Lombard Insurance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const resultCards = page.locator('[class*="listing"]');
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Icici Lombard Insurance',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Icici Lombard Insurance order',
      details: 'Completing your order on Icici Lombard Insurance',
      amountInr: 0, // Extracted from page at runtime
      description: 'icici-lombard-insurance order',
    });
    ...
    log({ step: 'Icici Lombard Insurance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Icici Lombard Insurance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'icici-lombard-insurance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'icici-lombard-insurance';
