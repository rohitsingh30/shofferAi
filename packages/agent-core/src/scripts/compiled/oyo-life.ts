/**
 * Book long-stay OYO Life apartments — monthly furnished rentals with zero deposit options.
 *
 * Auto-generated Playwright script for oyo-life.
 * Site: https://www.oyolife.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Oyo Life ──────────────────────────
    log({ step: 'Opening Oyo Life...', status: 'running' });
    await page.goto('https://www.oyolife.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Oyo Life. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Oyo Life order',
      details: 'Completing your order on Oyo Life',
      amountInr: 0, // Extracted from page at runtime
      description: 'oyo-life order',
    });
    ...
    log({ step: 'Oyo Life workflow completed', status: 'completed' });
    log({ message: 'Task completed on Oyo Life. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'oyo-life-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'oyo-life';
