/**
 * Enroll in edX university courses — MIT, Harvard, Berkeley; free audit or verified certificate.
 *
 * Auto-generated Playwright script for edx-course.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Edx Course ──────────────────────────
    log({ step: 'Opening Edx Course...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Edx Course. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Edx Course',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Edx Course order',
      details: 'Completing your order on Edx Course',
      amountInr: 0, // Extracted from page at runtime
      description: 'edx-course order',
    });
    ...
    log({ step: 'Edx Course workflow completed', status: 'completed' });
    log({ message: 'Task completed on Edx Course. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'edx-course-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'edx-course';
