/**
 * Book flights on ixigo — search across airlines, compare fares/times, select cheapest option, fill details, pay.
 *
 * Auto-generated Playwright script for ixigo-flight.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ixigo Flight ──────────────────────────
    log({ step: 'Opening Ixigo Flight...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ixigo Flight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Ixigo Flight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ixigo Flight order',
      details: 'Completing your order on Ixigo Flight',
      amountInr: 0, // Extracted from page at runtime
      description: 'ixigo-flight order',
    });
    ...
    log({ step: 'Ixigo Flight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ixigo Flight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ixigo-flight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'ixigo-flight';
