/**
 * Order from Taco Bell India — tacos, burritos, quesadillas, Mexican combos, checkout, pay.
 *
 * Auto-generated Playwright script for taco-bell.
 * Site: https://www.tacobell.co.in
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Taco Bell...', status: 'running' });
    await page.goto('https://www.tacobell.co.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Taco Bell. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Taco Bell order',
      details: 'Completing your order on Taco Bell',
    ...
    log({ step: 'Taco Bell workflow completed', status: 'completed' });
    log({ message: 'Task completed on Taco Bell. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'taco-bell-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'taco-bell';
