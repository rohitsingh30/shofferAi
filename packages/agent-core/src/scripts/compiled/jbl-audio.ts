/**
 * Buy JBL speakers, headphones, and earbuds on JBL India — browse, compare, buy.
 *
 * Auto-generated Playwright script for jbl-audio.
 * Site: https://www.jbl.com/in/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Jbl Audio ──────────────────────────
    log({ step: 'Opening Jbl Audio...', status: 'running' });
    await page.goto('https://www.jbl.com/in/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Jbl Audio. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Jbl Audio order',
      details: 'Completing your order on Jbl Audio',
      amountInr: 0, // Extracted from page at runtime
      description: 'jbl-audio order',
    });
    ...
    log({ step: 'Jbl Audio workflow completed', status: 'completed' });
    log({ message: 'Task completed on Jbl Audio. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'jbl-audio-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'jbl-audio';
