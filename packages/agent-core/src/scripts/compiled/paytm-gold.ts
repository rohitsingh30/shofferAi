/**
 * Buy digital gold on Paytm — select amount or grams, buy 24K gold, track holdings.
 *
 * Auto-generated Playwright script for paytm-gold.
 * Site: https://paytm.com/gold
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Gold ──────────────────────────
    log({ step: 'Opening Paytm Gold...', status: 'running' });
    await page.goto('https://paytm.com/gold');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Gold. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Paytm Gold',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Gold order',
      details: 'Completing your order on Paytm Gold',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-gold order',
    });
    ...
    log({ step: 'Paytm Gold workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Gold. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-gold-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-gold';
