/**
 * Add money to Paytm wallet — load wallet balance for payments, transfers, and recharges.
 *
 * Auto-generated Playwright script for paytm-wallet.
 * Site: https://paytm.com/wallet
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Wallet ──────────────────────────
    log({ step: 'Opening Paytm Wallet...', status: 'running' });
    await page.goto('https://paytm.com/wallet');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Wallet. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Paytm Wallet',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Wallet order',
      details: 'Completing your order on Paytm Wallet',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-wallet order',
    });
    ...
    log({ step: 'Paytm Wallet workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Wallet. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-wallet-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-wallet';
