/**
 * Create and manage DigitalOcean Droplets — select region, size, OS image, configure networking, deploy cloud servers.
 *
 * Auto-generated Playwright script for digitalocean-droplet.
 * Site: https://cloud.digitalocean.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Digitalocean Droplet ──────────────────────────
    log({ step: 'Opening Digitalocean Droplet...', status: 'running' });
    await page.goto('https://cloud.digitalocean.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Digitalocean Droplet. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Digitalocean Droplet',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Digitalocean Droplet order',
      details: 'Completing your order on Digitalocean Droplet',
      amountInr: 0, // Extracted from page at runtime
      description: 'digitalocean-droplet order',
    });
    ...
    log({ step: 'Digitalocean Droplet workflow completed', status: 'completed' });
    log({ message: 'Task completed on Digitalocean Droplet. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'digitalocean-droplet-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'digitalocean-droplet';
