/**
 * Shop on AJIO — browse fashion, clothing, shoes, accessories from top brands, apply coupons, checkout, pay.
 *
 * Auto-generated Playwright script for ajio-fashion.
 * Site: https://www.ajio.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ajio Fashion ──────────────────────────
    log({ step: 'Opening Ajio Fashion...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ajio Fashion. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Ajio Fashion',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ajio Fashion order',
      details: 'Completing your order on Ajio Fashion',
    ...
    log({ step: 'Ajio Fashion workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ajio Fashion. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ajio-fashion-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'ajio-fashion';
