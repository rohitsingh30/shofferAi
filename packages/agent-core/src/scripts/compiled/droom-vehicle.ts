/**
 * Buy or sell vehicles on Droom — cars, bikes, scooters. Browse marketplace, compare prices, get ECO inspection, transact.
 *
 * Auto-generated Playwright script for droom-vehicle.
 * Site: https://www.droom.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Droom Vehicle ──────────────────────────
    log({ step: 'Opening Droom Vehicle...', status: 'running' });
    await page.goto('https://www.droom.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Droom Vehicle. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Droom Vehicle',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Droom Vehicle order',
      details: 'Completing your order on Droom Vehicle',
      amountInr: 0, // Extracted from page at runtime
      description: 'droom-vehicle order',
    });
    ...
    log({ step: 'Droom Vehicle workflow completed', status: 'completed' });
    log({ message: 'Task completed on Droom Vehicle. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'droom-vehicle-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'droom-vehicle';
