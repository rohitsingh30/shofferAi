/**
 * Book AC service, repair, or installation on Urban Company. Regular service, deep clean, gas refill, by brand and type.
 *
 * Auto-generated Playwright script for urbancompany-repair.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbancompany Repair ──────────────────────────
    log({ step: 'Opening Urbancompany Repair...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbancompany Repair. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbancompany Repair',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbancompany Repair order',
      details: 'Completing your order on Urbancompany Repair',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbancompany-repair order',
    });
    ...
    log({ step: 'Urbancompany Repair workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbancompany Repair. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbancompany-repair-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbancompany-repair';
