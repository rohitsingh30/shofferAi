/**
 * Book a restaurant table on Dineout (now part of Swiggy) — search restaurants, select time slot, reserve.
 *
 * Auto-generated Playwright script for dineout-table.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Dineout Table ──────────────────────────
    log({ step: 'Opening Dineout Table...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Dineout Table. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Dineout Table',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Dineout Table order',
      details: 'Completing your order on Dineout Table',
      amountInr: 0, // Extracted from page at runtime
      description: 'dineout-table order',
    });
    ...
    log({ step: 'Dineout Table workflow completed', status: 'completed' });
    log({ message: 'Task completed on Dineout Table. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'dineout-table-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'dineout-table';
