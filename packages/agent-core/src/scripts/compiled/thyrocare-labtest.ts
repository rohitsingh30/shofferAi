/**
 * Book lab tests at home via Thyrocare — select tests or packages, schedule home collection, pay online.
 *
 * Auto-generated Playwright script for thyrocare-labtest.
 * Site: https://www.thyrocare.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Thyrocare Labtest ──────────────────────────
    log({ step: 'Opening Thyrocare Labtest...', status: 'running' });
    await page.goto('https://www.thyrocare.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Thyrocare Labtest. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Thyrocare Labtest',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Thyrocare Labtest order',
      details: 'Completing your order on Thyrocare Labtest',
      amountInr: 0, // Extracted from page at runtime
      description: 'thyrocare-labtest order',
    });
    ...
    log({ step: 'Thyrocare Labtest workflow completed', status: 'completed' });
    log({ message: 'Task completed on Thyrocare Labtest. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'thyrocare-labtest-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'thyrocare-labtest';
