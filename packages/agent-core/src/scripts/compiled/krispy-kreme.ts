/**
 * Order donuts from Krispy Kreme India — dozen boxes, individual donuts, beverages, delivery, pay.
 *
 * Auto-generated Playwright script for krispy-kreme.
 * Site: https://www.krispykreme.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Krispy Kreme ──────────────────────────
    log({ step: 'Opening Krispy Kreme...', status: 'running' });
    await page.goto('https://www.krispykreme.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Krispy Kreme. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Krispy Kreme',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Krispy Kreme order',
      details: 'Completing your order on Krispy Kreme',
      amountInr: 0, // Extracted from page at runtime
      description: 'krispy-kreme order',
    });
    ...
    log({ step: 'Krispy Kreme workflow completed', status: 'completed' });
    log({ message: 'Task completed on Krispy Kreme. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'krispy-kreme-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'krispy-kreme';
