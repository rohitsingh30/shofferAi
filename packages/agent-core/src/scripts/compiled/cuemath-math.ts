/**
 * Enroll kids in Cuemath online math classes — select grade, plan, schedule, subscribe, pay.
 *
 * Auto-generated Playwright script for cuemath-math.
 * Site: https://www.cuemath.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cuemath Math ──────────────────────────
    log({ step: 'Opening Cuemath Math...', status: 'running' });
    await page.goto('https://www.cuemath.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cuemath Math. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cuemath Math',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cuemath Math order',
      details: 'Completing your order on Cuemath Math',
      amountInr: 0, // Extracted from page at runtime
      description: 'cuemath-math order',
    });
    ...
    log({ step: 'Cuemath Math workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cuemath Math. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cuemath-math-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cuemath-math';
