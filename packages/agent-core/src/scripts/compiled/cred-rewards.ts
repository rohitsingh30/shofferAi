/**
 * Redeem CRED coins for rewards, offers, cashback, and exclusive deals on the CRED platform.
 *
 * Auto-generated Playwright script for cred-rewards.
 * Site: https://cred.club/rewards
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cred Rewards ──────────────────────────
    log({ step: 'Opening Cred Rewards...', status: 'running' });
    await page.goto('https://cred.club/rewards');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cred Rewards. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cred Rewards',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cred Rewards order',
      details: 'Completing your order on Cred Rewards',
      amountInr: 0, // Extracted from page at runtime
      description: 'cred-rewards order',
    });
    ...
    log({ step: 'Cred Rewards workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cred Rewards. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cred-rewards-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cred-rewards';
