/**
 * Add money to Amazon Pay wallet balance — load wallet for shopping, bill payments, and transfers.
 *
 * Auto-generated Playwright script for amazon-pay.
 * Site: https://www.amazon.in/pay
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Amazon Pay...', status: 'running' });
    await page.goto('https://www.amazon.in/pay');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Pay. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Pay',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Amazon Pay order',
      details: 'Completing your order on Amazon Pay',
      amountInr: 0, // Extracted from page at runtime
      description: 'amazon-pay order',
    });
    ...
    log({ step: 'Amazon Pay workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Pay. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-pay-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-pay';
