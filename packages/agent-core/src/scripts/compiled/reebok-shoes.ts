/**
 * Shop on Reebok India — browse, select size, buy shoes, apparel, and accessories, checkout, pay.
 *
 * Auto-generated Playwright script for reebok-shoes.
 * Site: https://www.reebok.co.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Reebok Shoes ──────────────────────────
    log({ step: 'Opening Reebok Shoes...', status: 'running' });
    await page.goto('https://www.reebok.co.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Reebok Shoes. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Reebok Shoes',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Reebok Shoes order',
      details: 'Completing your order on Reebok Shoes',
      amountInr: 0, // Extracted from page at runtime
      description: 'reebok-shoes order',
    });
    ...
    log({ step: 'Reebok Shoes workflow completed', status: 'completed' });
    log({ message: 'Task completed on Reebok Shoes. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'reebok-shoes-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'reebok-shoes';
