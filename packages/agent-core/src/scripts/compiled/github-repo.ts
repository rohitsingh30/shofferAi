/**
 * Create a GitHub repository — initialize with README, license, .gitignore, configure settings, and invite collaborators.
 *
 * Auto-generated Playwright script for github-repo.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Github Repo...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Github Repo. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Github Repo',
    ...
    log({ step: 'Github Repo workflow completed', status: 'completed' });
    log({ message: 'Task completed on Github Repo. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'github-repo-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'github-repo';
