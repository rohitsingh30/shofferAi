/**
 * Check PNR status on IRCTC — enter PNR number, view booking status, berth allocation, coach details.
 *
 * Auto-generated Playwright script for irctc-pnr.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Irctc Pnr ──────────────────────────
    log({ step: 'Opening Irctc Pnr...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Irctc Pnr. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    log({ step: 'Irctc Pnr workflow completed', status: 'completed' });
    log({ message: 'Task completed on Irctc Pnr. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'irctc-pnr-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'irctc-pnr';
