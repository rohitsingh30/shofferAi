/**
 * Recharge Jio prepaid or postpaid on jio.com. Browse plans, select, and pay.
 *
 * Auto-generated Playwright script for jio-recharge.
 * Site: https://www.jio.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Jio Recharge ──────────────────────────
    log({ step: 'Opening Jio Recharge...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Jio Recharge. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Jio Recharge',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Jio Recharge order',
      details: 'Completing your order on Jio Recharge',
      amountInr: 0, // Extracted from page at runtime
      description: 'jio-recharge order',
    });
    ...
    log({ step: 'Jio Recharge workflow completed', status: 'completed' });
    log({ message: 'Task completed on Jio Recharge. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'jio-recharge-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'jio-recharge';
