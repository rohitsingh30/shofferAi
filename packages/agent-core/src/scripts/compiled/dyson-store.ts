/**
 * Buy Dyson products on Dyson India — vacuum cleaners, air purifiers, hair dryers, straighteners, fans, lights.
 *
 * Auto-generated Playwright script for dyson-store.
 * Site: https://www.dyson.in/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Dyson Store ──────────────────────────
    log({ step: 'Opening Dyson Store...', status: 'running' });
    await page.goto('https://www.dyson.in/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Dyson Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Dyson Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Dyson Store order',
      details: 'Completing your order on Dyson Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'dyson-store order',
    });
    ...
    log({ step: 'Dyson Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Dyson Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'dyson-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'dyson-store';
