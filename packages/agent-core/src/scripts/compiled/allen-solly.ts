/**
 * Shop smart casual fashion on Allen Solly — browse shirts, chinos, dresses, work wear, casual wear for men and women, checkout, pay.
 *
 * Auto-generated Playwright script for allen-solly.
 * Site: https://www.allensolly.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Allen Solly ──────────────────────────
    log({ step: 'Opening Allen Solly...', status: 'running' });
    await page.goto('https://www.allensolly.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Allen Solly. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Allen Solly',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Allen Solly order',
      details: 'Completing your order on Allen Solly',
      amountInr: 0, // Extracted from page at runtime
      description: 'allen-solly order',
    });
    ...
    log({ step: 'Allen Solly workflow completed', status: 'completed' });
    log({ message: 'Task completed on Allen Solly. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'allen-solly-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'allen-solly';
