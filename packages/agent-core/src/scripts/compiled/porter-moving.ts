/**
 * Book mini-truck, tempo, or packers & movers on Porter — local shifting, goods transport, intercity moving.
 *
 * Auto-generated Playwright script for porter-moving.
 * Site: https://porter.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Porter Moving ──────────────────────────
    log({ step: 'Opening Porter Moving...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Porter Moving. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Porter Moving',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Porter Moving order',
      details: 'Completing your order on Porter Moving',
      amountInr: 0, // Extracted from page at runtime
      description: 'porter-moving order',
    });
    ...
    log({ step: 'Porter Moving workflow completed', status: 'completed' });
    log({ message: 'Task completed on Porter Moving. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'porter-moving-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'porter-moving';
