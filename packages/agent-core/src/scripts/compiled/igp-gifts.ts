/**
 * Send gifts online via IGP — cakes, flowers, hampers, personalized gifts delivered to any address in India.
 *
 * Auto-generated Playwright script for igp-gifts.
 * Site: https://www.igp.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Igp Gifts ──────────────────────────
    log({ step: 'Opening Igp Gifts...', status: 'running' });
    await page.goto('https://www.igp.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Igp Gifts. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Igp Gifts order',
      details: 'Completing your order on Igp Gifts',
      amountInr: 0, // Extracted from page at runtime
      description: 'igp-gifts order',
    });
    ...
    log({ step: 'Igp Gifts workflow completed', status: 'completed' });
    log({ message: 'Task completed on Igp Gifts. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'igp-gifts-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'igp-gifts';
