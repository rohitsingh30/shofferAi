/**
 * Buy or sell crypto on WazirX — search coins, place market/limit orders, check portfolio and P&L.
 *
 * Auto-generated Playwright script for wazirx-crypto.
 * Site: https://wazirx.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wazirx Crypto ──────────────────────────
    log({ step: 'Opening Wazirx Crypto...', status: 'running' });
    await page.goto('https://wazirx.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wazirx Crypto. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Wazirx Crypto',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wazirx Crypto order',
      details: 'Completing your order on Wazirx Crypto',
      amountInr: 0, // Extracted from page at runtime
      description: 'wazirx-crypto order',
    });
    ...
    log({ step: 'Wazirx Crypto workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wazirx Crypto. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wazirx-crypto-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wazirx-crypto';
