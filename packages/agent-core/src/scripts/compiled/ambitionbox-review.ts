/**
 * Research company reviews, salaries, and interview questions on AmbitionBox (by Naukri) — India-focused employer insights.
 *
 * Auto-generated Playwright script for ambitionbox-review.
 * Site: https://www.ambitionbox.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ambitionbox Review ──────────────────────────
    log({ step: 'Opening Ambitionbox Review...', status: 'running' });
    ...
    const profileEl = page.locator('[data-testid*="profile"]').first();
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ambitionbox Review. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const resultCards = page.locator('[data-testid*="card"]');
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Ambitionbox Review',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ambitionbox Review order',
      details: 'Completing your order on Ambitionbox Review',
      amountInr: 0, // Extracted from page at runtime
      description: 'ambitionbox-review order',
    });
    ...
    log({ step: 'Ambitionbox Review workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ambitionbox Review. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ambitionbox-review-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'ambitionbox-review';
