/**
 * Order from Subway India — customize subs with bread, protein, veggies, sauce, add sides, checkout, pay.
 *
 * Auto-generated Playwright script for subway-order.
 * Site: https://www.subway.com/en-IN
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Subway Order ──────────────────────────
    log({ step: 'Opening Subway Order...', status: 'running' });
    await page.goto('https://www.subway.com/en-IN');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Subway Order. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Subway Order',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Subway Order order',
      details: 'Completing your order on Subway Order',
      amountInr: 0, // Extracted from page at runtime
      description: 'subway-order order',
    });
    ...
    log({ step: 'Subway Order workflow completed', status: 'completed' });
    log({ message: 'Task completed on Subway Order. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'subway-order-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'subway-order';
