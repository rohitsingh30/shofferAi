/**
 * Donate to verified NGOs on GiveIndia — tax-deductible donations to trusted nonprofits across education, health, disaster relief, and more.
 *
 * Auto-generated Playwright script for giveindia-donate.
 * Site: https://www.giveindia.org
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Giveindia Donate ──────────────────────────
    log({ step: 'Opening Giveindia Donate...', status: 'running' });
    await page.goto('https://www.giveindia.org');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Giveindia Donate. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Giveindia Donate',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Giveindia Donate order',
      details: 'Completing your order on Giveindia Donate',
      amountInr: 0, // Extracted from page at runtime
      description: 'giveindia-donate order',
    });
    ...
    log({ step: 'Giveindia Donate workflow completed', status: 'completed' });
    log({ message: 'Task completed on Giveindia Donate. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'giveindia-donate-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'giveindia-donate';
