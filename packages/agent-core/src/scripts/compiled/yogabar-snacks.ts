/**
 * Buy protein bars, muesli, oats, and healthy snacks from Yogabar — browse, add to cart, order online.
 *
 * Auto-generated Playwright script for yogabar-snacks.
 * Site: https://www.yogabars.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Yogabar Snacks ──────────────────────────
    log({ step: 'Opening Yogabar Snacks...', status: 'running' });
    await page.goto('https://www.yogabars.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Yogabar Snacks. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Yogabar Snacks',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Yogabar Snacks order',
      details: 'Completing your order on Yogabar Snacks',
      amountInr: 0, // Extracted from page at runtime
      description: 'yogabar-snacks order',
    });
    ...
    log({ step: 'Yogabar Snacks workflow completed', status: 'completed' });
    log({ message: 'Task completed on Yogabar Snacks. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'yogabar-snacks-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'yogabar-snacks';
