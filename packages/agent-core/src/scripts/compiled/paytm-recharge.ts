/**
 * Recharge mobile prepaid, pay bills (electricity, broadband, DTH) on Paytm.
 *
 * Auto-generated Playwright script for paytm-recharge.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Recharge ──────────────────────────
    log({ step: 'Opening Paytm Recharge...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Recharge. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Paytm Recharge',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Recharge order',
      details: 'Completing your order on Paytm Recharge',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-recharge order',
    });
    ...
    log({ step: 'Paytm Recharge workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Recharge. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-recharge-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-recharge';
