/**
 * Shop on ShopClues — search budget products, compare prices, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for shopclues-shopping.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Shopclues Shopping ──────────────────────────
    log({ step: 'Opening Shopclues Shopping...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Shopclues Shopping. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Shopclues Shopping',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Shopclues Shopping order',
      details: 'Completing your order on Shopclues Shopping',
      amountInr: 0, // Extracted from page at runtime
      description: 'shopclues-shopping order',
    });
    ...
    log({ step: 'Shopclues Shopping workflow completed', status: 'completed' });
    log({ message: 'Task completed on Shopclues Shopping. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'shopclues-shopping-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'shopclues-shopping';
