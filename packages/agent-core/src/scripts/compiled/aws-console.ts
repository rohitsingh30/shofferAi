/**
 * Manage AWS services via the console — launch EC2 instances, manage S3 buckets, configure Lambda functions, monitor billing.
 *
 * Auto-generated Playwright script for aws-console.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Aws Console...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Aws Console. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Aws Console',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Aws Console order',
      details: 'Completing your order on Aws Console',
      amountInr: 0, // Extracted from page at runtime
      description: 'aws-console order',
    });
    ...
    log({ step: 'Aws Console workflow completed', status: 'completed' });
    log({ message: 'Task completed on Aws Console. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'aws-console-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'aws-console';
