/**
 * Buy Uniqlo basics and essentials on uniqlo.com/in — browse LifeWear, select size and color, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for uniqlo-fashion.
 * Site: https://www.uniqlo.com/in/en/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Uniqlo Fashion ──────────────────────────
    log({ step: 'Opening Uniqlo Fashion...', status: 'running' });
    await page.goto('https://www.uniqlo.com/in/en/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Uniqlo Fashion. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Uniqlo Fashion',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Uniqlo Fashion order',
      details: 'Completing your order on Uniqlo Fashion',
    ...
    log({ step: 'Uniqlo Fashion workflow completed', status: 'completed' });
    log({ message: 'Task completed on Uniqlo Fashion. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'uniqlo-fashion-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'uniqlo-fashion';
