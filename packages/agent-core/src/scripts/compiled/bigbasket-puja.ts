/**
 * Order puja items and samagri from BigBasket — incense, diyas, flowers, camphor, turmeric, and complete puja kits for festivals and rituals.
 *
 * Auto-generated Playwright script for bigbasket-puja.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bigbasket Puja ──────────────────────────
    log({ step: 'Opening Bigbasket Puja...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bigbasket Puja. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bigbasket Puja',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bigbasket Puja order',
      details: 'Completing your order on Bigbasket Puja',
      amountInr: 0, // Extracted from page at runtime
      description: 'bigbasket-puja order',
    });
    ...
    log({ step: 'Bigbasket Puja workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bigbasket Puja. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bigbasket-puja-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bigbasket-puja';
