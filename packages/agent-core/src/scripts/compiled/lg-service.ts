/**
 * Book LG appliance service or repair on lg.com/in — AC, washing machine, refrigerator, TV, microwave repair.
 *
 * Auto-generated Playwright script for lg-service.
 * Site: https://www.lg.com/in/support/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Lg Service ──────────────────────────
    log({ step: 'Opening Lg Service...', status: 'running' });
    await page.goto('https://www.lg.com/in/support/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Lg Service. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Lg Service',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Lg Service order',
      details: 'Completing your order on Lg Service',
      amountInr: 0, // Extracted from page at runtime
      description: 'lg-service order',
    });
    ...
    log({ step: 'Lg Service workflow completed', status: 'completed' });
    log({ message: 'Task completed on Lg Service. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'lg-service-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'lg-service';
