/**
 * Find jobs and chat directly with founders and HR on Hirect — instant chat-based hiring for tech, product, and business roles.
 *
 * Auto-generated Playwright script for hirect-chat.
 * Site: https://www.hirect.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hirect Chat ──────────────────────────
    log({ step: 'Opening Hirect Chat...', status: 'running' });
    await page.goto('https://www.hirect.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hirect Chat. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hirect Chat',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hirect Chat order',
      details: 'Completing your order on Hirect Chat',
      amountInr: 0, // Extracted from page at runtime
      description: 'hirect-chat order',
    });
    ...
    log({ step: 'Hirect Chat workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hirect Chat. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hirect-chat-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hirect-chat';
