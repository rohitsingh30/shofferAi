/**
 * Start a language course on Duolingo — select language, begin learning, upgrade to Duolingo Plus/Super.
 *
 * Auto-generated Playwright script for duolingo-language.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Duolingo Language ──────────────────────────
    log({ step: 'Opening Duolingo Language...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Duolingo Language. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Duolingo Language',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Duolingo Language order',
      details: 'Completing your order on Duolingo Language',
      amountInr: 0, // Extracted from page at runtime
      description: 'duolingo-language order',
    });
    ...
    log({ step: 'Duolingo Language workflow completed', status: 'completed' });
    log({ message: 'Task completed on Duolingo Language. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'duolingo-language-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'duolingo-language';
