/**
 * Order groceries from Amazon Fresh — search products, add to cart, schedule delivery, checkout, pay.
 *
 * Auto-generated Playwright script for amazon-fresh.
 * Site: https://www.amazon.in/fresh
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Amazon Fresh...', status: 'running' });
    await page.goto('https://www.amazon.in/fresh');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Fresh. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Fresh',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Amazon Fresh order',
      details: 'Completing your order on Amazon Fresh',
      amountInr: 0, // Extracted from page at runtime
      description: 'amazon-fresh order',
    });
    ...
    log({ step: 'Amazon Fresh workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Fresh. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-fresh-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-fresh';
