/**
 * Buy beauty and skincare products on Purplle — search, compare, apply offers, add to bag, checkout and pay.
 *
 * Auto-generated Playwright script for purplle-beauty.
 * Site: https://www.purplle.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Purplle Beauty ──────────────────────────
    log({ step: 'Opening Purplle Beauty...', status: 'running' });
    await page.goto('https://www.purplle.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Purplle Beauty. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Purplle Beauty',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Purplle Beauty order',
      details: 'Completing your order on Purplle Beauty',
    ...
    log({ step: 'Purplle Beauty workflow completed', status: 'completed' });
    log({ message: 'Task completed on Purplle Beauty. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'purplle-beauty-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'purplle-beauty';
