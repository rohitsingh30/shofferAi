/**
 * Shop on Noise (gonoise.com) — buy smartwatches, earbuds, audio accessories, browse collections, checkout, pay.
 *
 * Auto-generated Playwright script for noise-smartwatch.
 * Site: https://www.gonoise.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Noise Smartwatch ──────────────────────────
    log({ step: 'Opening Noise Smartwatch...', status: 'running' });
    await page.goto('https://www.gonoise.com');
    ...
    const profileEl = page.locator('[class*="account"]').first();
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Noise Smartwatch. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const resultCards = page.locator('[class*="product-card"]');
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Noise Smartwatch',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Noise Smartwatch order',
      details: 'Completing your order on Noise Smartwatch',
      amountInr: 0, // Extracted from page at runtime
      description: 'noise-smartwatch order',
    });
    ...
    log({ step: 'Noise Smartwatch workflow completed', status: 'completed' });
    log({ message: 'Task completed on Noise Smartwatch. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'noise-smartwatch-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'noise-smartwatch';
