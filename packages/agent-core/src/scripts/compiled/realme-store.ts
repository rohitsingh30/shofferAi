/**
 * Buy Realme products on Realme India — phones, AIoT devices, earbuds, accessories.
 *
 * Auto-generated Playwright script for realme-store.
 * Site: https://www.realme.com/in/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Realme Store ──────────────────────────
    log({ step: 'Opening Realme Store...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Realme Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Realme Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Realme Store order',
      details: 'Completing your order on Realme Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'realme-store order',
    });
    ...
    log({ step: 'Realme Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Realme Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'realme-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'realme-store';
