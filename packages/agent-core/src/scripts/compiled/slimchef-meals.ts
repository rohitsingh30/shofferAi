/**
 * Order calorie-counted, macro-balanced meals from SlimChef or fitness meal delivery services — diet plans, single meals, or subscriptions.
 *
 * Auto-generated Playwright script for slimchef-meals.
 * Site: https://www.slimchef.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Slimchef Meals ──────────────────────────
    log({ step: 'Opening Slimchef Meals...', status: 'running' });
    await page.goto('https://www.slimchef.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Slimchef Meals. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Slimchef Meals',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Slimchef Meals order',
      details: 'Completing your order on Slimchef Meals',
      amountInr: 0, // Extracted from page at runtime
      description: 'slimchef-meals order',
    });
    ...
    log({ step: 'Slimchef Meals workflow completed', status: 'completed' });
    log({ message: 'Task completed on Slimchef Meals. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'slimchef-meals-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'slimchef-meals';
