/**
 * Subscribe to Spotify Premium India. Choose plan (Individual, Duo, Family, Student), pay and activate.
 *
 * Auto-generated Playwright script for spotify-subscribe.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Spotify Subscribe ──────────────────────────
    log({ step: 'Opening Spotify Subscribe...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Spotify Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Spotify Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Spotify Subscribe order',
      details: 'Completing your order on Spotify Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'spotify-subscribe order',
    });
    ...
    log({ step: 'Spotify Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Spotify Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'spotify-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'spotify-subscribe';
