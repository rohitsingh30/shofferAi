/**
 * Deploy backend application on Railway.app — connect GitHub repo, configure environment variables, provision database, deploy services.
 *
 * Auto-generated Playwright script for railway-deploy.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Railway Deploy ──────────────────────────
    log({ step: 'Opening Railway Deploy...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Railway Deploy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Railway Deploy',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Railway Deploy order',
      details: 'Completing your order on Railway Deploy',
      amountInr: 0, // Extracted from page at runtime
      description: 'railway-deploy order',
    });
    ...
    log({ step: 'Railway Deploy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Railway Deploy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'railway-deploy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'railway-deploy';
