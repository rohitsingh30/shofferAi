/**
 * Create Google Ads campaign — set up search, display, or video ads with keywords, budget, ad copy, and targeting.
 *
 * Auto-generated Playwright script for google-ads.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Google Ads...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Google Ads. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Google Ads',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Google Ads order',
      details: 'Completing your order on Google Ads',
      amountInr: 0, // Extracted from page at runtime
      description: 'google-ads order',
    });
    ...
    log({ step: 'Google Ads workflow completed', status: 'completed' });
    log({ message: 'Task completed on Google Ads. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'google-ads-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'google-ads';
