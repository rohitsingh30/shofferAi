/**
 * Top up gaming credits on Codashop India. BGMI UC, Free Fire Diamonds, Genshin Genesis Crystals, Mobile Legends, and more.
 *
 * Auto-generated Playwright script for codashop-topup.
 * Site: https://www.codashop.com/in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Codashop Topup ──────────────────────────
    log({ step: 'Opening Codashop Topup...', status: 'running' });
    await page.goto('https://www.codashop.com/in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Codashop Topup. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Codashop Topup',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Codashop Topup order',
      details: 'Completing your order on Codashop Topup',
      amountInr: 0, // Extracted from page at runtime
      description: 'codashop-topup order',
    });
    ...
    log({ step: 'Codashop Topup workflow completed', status: 'completed' });
    log({ message: 'Task completed on Codashop Topup. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'codashop-topup-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'codashop-topup';
