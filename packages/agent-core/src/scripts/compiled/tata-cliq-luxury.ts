/**
 * Shop luxury fashion and beauty on Tata CLiQ Luxury — browse Gucci, Armani, Burberry, designer brands, add to bag, checkout, pay.
 *
 * Auto-generated Playwright script for tata-cliq-luxury.
 * Site: https://luxury.tatacliq.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Tata Cliq Luxury ──────────────────────────
    log({ step: 'Opening Tata Cliq Luxury...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Tata Cliq Luxury. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Tata Cliq Luxury',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Tata Cliq Luxury order',
      details: 'Completing your order on Tata Cliq Luxury',
      amountInr: 0, // Extracted from page at runtime
      description: 'tata-cliq-luxury order',
    });
    ...
    log({ step: 'Tata Cliq Luxury workflow completed', status: 'completed' });
    log({ message: 'Task completed on Tata Cliq Luxury. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'tata-cliq-luxury-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'tata-cliq-luxury';
