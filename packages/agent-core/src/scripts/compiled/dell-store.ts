/**
 * Buy Dell laptops, desktops, and monitors on Dell India — configure specs, customize, buy.
 *
 * Auto-generated Playwright script for dell-store.
 * Site: https://www.dell.com/in-en
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Dell Store ──────────────────────────
    log({ step: 'Opening Dell Store...', status: 'running' });
    await page.goto('https://www.dell.com/in-en');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Dell Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Dell Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Dell Store order',
      details: 'Completing your order on Dell Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'dell-store order',
    });
    ...
    log({ step: 'Dell Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Dell Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'dell-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'dell-store';
