/**
 * Subscribe to Microsoft 365 India. Personal or Family plan with Word, Excel, PowerPoint, Outlook, 1TB OneDrive, and more.
 *
 * Auto-generated Playwright script for microsoft365-subscribe.
 * Site: https://www.microsoft.com/en-in/microsoft-365
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Microsoft365 Subscribe ──────────────────────────
    log({ step: 'Opening Microsoft365 Subscribe...', status: 'running' });
    await page.goto('https://www.microsoft.com/en-in/microsoft-365');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Microsoft365 Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Microsoft365 Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Microsoft365 Subscribe order',
      details: 'Completing your order on Microsoft365 Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'microsoft365-subscribe order',
    });
    ...
    log({ step: 'Microsoft365 Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Microsoft365 Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'microsoft365-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'microsoft365-subscribe';
