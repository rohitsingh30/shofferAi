/**
 * Book amusement park tickets at Wonderla — select park location, date, ticket type, and pay online.
 *
 * Auto-generated Playwright script for wonderla-tickets.
 * Site: https://www.wonderla.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wonderla Tickets ──────────────────────────
    log({ step: 'Opening Wonderla Tickets...', status: 'running' });
    await page.goto('https://www.wonderla.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wonderla Tickets. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Wonderla Tickets',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wonderla Tickets order',
      details: 'Completing your order on Wonderla Tickets',
      amountInr: 0, // Extracted from page at runtime
      description: 'wonderla-tickets order',
    });
    ...
    log({ step: 'Wonderla Tickets workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wonderla Tickets. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wonderla-tickets-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wonderla-tickets';
