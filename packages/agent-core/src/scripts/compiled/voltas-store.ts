/**
 * Buy Voltas ACs, coolers, air purifiers, and commercial cooling on Voltas India — Tata brand.
 *
 * Auto-generated Playwright script for voltas-store.
 * Site: https://www.voltas.com/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Voltas Store ──────────────────────────
    log({ step: 'Opening Voltas Store...', status: 'running' });
    await page.goto('https://www.voltas.com/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Voltas Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Voltas Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Voltas Store order',
      details: 'Completing your order on Voltas Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'voltas-store order',
    });
    ...
    log({ step: 'Voltas Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Voltas Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'voltas-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'voltas-store';
