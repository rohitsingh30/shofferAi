/**
 * Buy Xiaomi/Redmi products on mi.com — phones, TVs, smart home devices, accessories.
 *
 * Auto-generated Playwright script for mi-store.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Mi Store ──────────────────────────
    log({ step: 'Opening Mi Store...', status: 'running' });
    await page.goto('https://www.mi.com/in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Mi Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Mi Store order',
      details: 'Completing your order on Mi Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'mi-store order',
    });
    ...
    log({ step: 'Mi Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Mi Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'mi-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'mi-store';
