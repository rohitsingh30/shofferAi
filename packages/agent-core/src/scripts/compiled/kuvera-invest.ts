/**
 * Invest in mutual funds on Kuvera — goal-based investing, direct plans, SIP, lump sum, tax saving ELSS.
 *
 * Auto-generated Playwright script for kuvera-invest.
 * Site: https://kuvera.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Kuvera Invest ──────────────────────────
    log({ step: 'Opening Kuvera Invest...', status: 'running' });
    await page.goto('https://kuvera.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Kuvera Invest. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Kuvera Invest',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Kuvera Invest order',
      details: 'Completing your order on Kuvera Invest',
      amountInr: 0, // Extracted from page at runtime
      description: 'kuvera-invest order',
    });
    ...
    log({ step: 'Kuvera Invest workflow completed', status: 'completed' });
    log({ message: 'Task completed on Kuvera Invest. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'kuvera-invest-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'kuvera-invest';
