/**
 * Book train tickets or check PNR on ixigo — search trains, predict confirmation chances, compare options, book and pay.
 *
 * Auto-generated Playwright script for ixigo-train.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ixigo Train ──────────────────────────
    log({ step: 'Opening Ixigo Train...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ixigo Train. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Ixigo Train',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ixigo Train order',
      details: 'Completing your order on Ixigo Train',
      amountInr: 0, // Extracted from page at runtime
      description: 'ixigo-train order',
    });
    ...
    log({ step: 'Ixigo Train workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ixigo Train. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ixigo-train-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'ixigo-train';
