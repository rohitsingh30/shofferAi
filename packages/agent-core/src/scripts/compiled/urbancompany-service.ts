/**
 * Book home services on Urban Company — salon, cleaning, repairs, AC service, plumbing.
 *
 * Auto-generated Playwright script for urbancompany-service.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbancompany Service ──────────────────────────
    log({ step: 'Opening Urbancompany Service...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbancompany Service. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbancompany Service',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbancompany Service order',
      details: 'Completing your order on Urbancompany Service',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbancompany-service order',
    });
    ...
    log({ step: 'Urbancompany Service workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbancompany Service. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbancompany-service-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbancompany-service';
