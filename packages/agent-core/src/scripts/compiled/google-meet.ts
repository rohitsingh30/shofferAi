/**
 * Create a Google Meet video meeting link — schedule via Google Calendar, set date/time, share link with participants.
 *
 * Auto-generated Playwright script for google-meet.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Google Meet...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Google Meet. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Google Meet',
    ...
    log({ step: 'Google Meet workflow completed', status: 'completed' });
    log({ message: 'Task completed on Google Meet. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'google-meet-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'google-meet';
