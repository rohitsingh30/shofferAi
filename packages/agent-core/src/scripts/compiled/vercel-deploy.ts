/**
 * Deploy a website on Vercel — import from GitHub, configure build settings, set environment variables, deploy to production.
 *
 * Auto-generated Playwright script for vercel-deploy.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Vercel Deploy ──────────────────────────
    log({ step: 'Opening Vercel Deploy...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Vercel Deploy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Vercel Deploy',
    ...
    log({ step: 'Vercel Deploy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Vercel Deploy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'vercel-deploy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'vercel-deploy';
