/**
 * Enroll kids in coding classes on WhiteHat Jr — browse courses, select plan, book trial or subscribe, pay.
 *
 * Auto-generated Playwright script for whitehatjr-coding.
 * Site: https://www.whitehatjr.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Whitehatjr Coding ──────────────────────────
    log({ step: 'Opening Whitehatjr Coding...', status: 'running' });
    await page.goto('https://www.whitehatjr.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Whitehatjr Coding. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Whitehatjr Coding',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Whitehatjr Coding order',
      details: 'Completing your order on Whitehatjr Coding',
      amountInr: 0, // Extracted from page at runtime
      description: 'whitehatjr-coding order',
    });
    ...
    log({ step: 'Whitehatjr Coding workflow completed', status: 'completed' });
    log({ message: 'Task completed on Whitehatjr Coding. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'whitehatjr-coding-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'whitehatjr-coding';
