/**
 * Book doctor consultations or lab tests on Tata Health (1mg) — find doctors, book appointments, order lab tests.
 *
 * Auto-generated Playwright script for tata-health.
 * Site: https://www.tatahealth.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Tata Health ──────────────────────────
    log({ step: 'Opening Tata Health...', status: 'running' });
    await page.goto('https://www.tatahealth.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Tata Health. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Tata Health',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Tata Health order',
      details: 'Completing your order on Tata Health',
      amountInr: 0, // Extracted from page at runtime
      description: 'tata-health order',
    });
    ...
    log({ step: 'Tata Health workflow completed', status: 'completed' });
    log({ message: 'Task completed on Tata Health. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'tata-health-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'tata-health';
