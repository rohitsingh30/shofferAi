/**
 * Pay Hathway broadband bill or upgrade plan. Login, view bill, pay, or change broadband plan.
 *
 * Auto-generated Playwright script for hathway-broadband.
 * Site: https://www.hathway.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hathway Broadband ──────────────────────────
    log({ step: 'Opening Hathway Broadband...', status: 'running' });
    await page.goto('https://www.hathway.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hathway Broadband. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hathway Broadband',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hathway Broadband order',
      details: 'Completing your order on Hathway Broadband',
      amountInr: 0, // Extracted from page at runtime
      description: 'hathway-broadband order',
    });
    ...
    log({ step: 'Hathway Broadband workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hathway Broadband. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hathway-broadband-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hathway-broadband';
