/**
 * Order healthy salads and bowls from SaladDays — salads, wraps, smoothie bowls, checkout, pay.
 *
 * Auto-generated Playwright script for salad-days.
 * Site: https://www.saladdays.co
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Salad Days ──────────────────────────
    log({ step: 'Opening Salad Days...', status: 'running' });
    await page.goto('https://www.saladdays.co');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Salad Days. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Salad Days',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Salad Days order',
      details: 'Completing your order on Salad Days',
      amountInr: 0, // Extracted from page at runtime
      description: 'salad-days order',
    });
    ...
    log({ step: 'Salad Days workflow completed', status: 'completed' });
    log({ message: 'Task completed on Salad Days. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'salad-days-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'salad-days';
