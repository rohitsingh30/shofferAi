/**
 * Book coworking on Smartworks — managed offices, flex desks, enterprise solutions across India.
 *
 * Auto-generated Playwright script for smartworks-cowork.
 * Site: https://www.smartworksoffice.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Smartworks Cowork ──────────────────────────
    log({ step: 'Opening Smartworks Cowork...', status: 'running' });
    await page.goto('https://www.smartworksoffice.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Smartworks Cowork. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Smartworks Cowork',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Smartworks Cowork order',
      details: 'Completing your order on Smartworks Cowork',
      amountInr: 0, // Extracted from page at runtime
      description: 'smartworks-cowork order',
    });
    ...
    log({ step: 'Smartworks Cowork workflow completed', status: 'completed' });
    log({ message: 'Task completed on Smartworks Cowork. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'smartworks-cowork-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'smartworks-cowork';
