/**
 * Buy fans, coolers, or home appliances on Orient Electric — ceiling fans, BLDC fans, tower fans, air coolers, geysers.
 *
 * Auto-generated Playwright script for orient-fan.
 * Site: https://www.orientelectric.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Orient Fan ──────────────────────────
    log({ step: 'Opening Orient Fan...', status: 'running' });
    await page.goto('https://www.orientelectric.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Orient Fan. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Orient Fan',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Orient Fan order',
      details: 'Completing your order on Orient Fan',
      amountInr: 0, // Extracted from page at runtime
      description: 'orient-fan order',
    });
    ...
    log({ step: 'Orient Fan workflow completed', status: 'completed' });
    log({ message: 'Task completed on Orient Fan. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'orient-fan-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'orient-fan';
