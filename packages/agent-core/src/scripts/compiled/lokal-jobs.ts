/**
 * Find local and blue-collar jobs on Lokal app/website — delivery, driver, field sales, warehouse, BPO, and other hyperlocal roles.
 *
 * Auto-generated Playwright script for lokal-jobs.
 * Site: https://www.getlokal.app
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Lokal Jobs ──────────────────────────
    log({ step: 'Opening Lokal Jobs...', status: 'running' });
    await page.goto('https://www.getlokal.app');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Lokal Jobs. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Lokal Jobs',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Lokal Jobs order',
      details: 'Completing your order on Lokal Jobs',
      amountInr: 0, // Extracted from page at runtime
      description: 'lokal-jobs order',
    });
    ...
    log({ step: 'Lokal Jobs workflow completed', status: 'completed' });
    log({ message: 'Task completed on Lokal Jobs. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'lokal-jobs-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'lokal-jobs';
