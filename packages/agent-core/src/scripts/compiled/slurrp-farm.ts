/**
 * Buy healthy kids snacks, cereals, pancake mixes, and nutrition from Slurrp Farm — browse, add to cart, subscribe or buy.
 *
 * Auto-generated Playwright script for slurrp-farm.
 * Site: https://www.slurrpfarm.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Slurrp Farm ──────────────────────────
    log({ step: 'Opening Slurrp Farm...', status: 'running' });
    await page.goto('https://www.slurrpfarm.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Slurrp Farm. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Slurrp Farm',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Slurrp Farm order',
      details: 'Completing your order on Slurrp Farm',
    ...
    log({ step: 'Slurrp Farm workflow completed', status: 'completed' });
    log({ message: 'Task completed on Slurrp Farm. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'slurrp-farm-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'slurrp-farm';
