/**
 * Book hostels on Hostelworld — international backpacker hostels, dorms and privates, worldwide coverage, traveller reviews.
 *
 * Auto-generated Playwright script for hostelworld-hostel.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hostelworld Hostel ──────────────────────────
    log({ step: 'Opening Hostelworld Hostel...', status: 'running' });
    await page.goto('https://www.hostelworld.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hostelworld Hostel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hostelworld Hostel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hostelworld Hostel order',
      details: 'Completing your order on Hostelworld Hostel',
      amountInr: 0, // Extracted from page at runtime
      description: 'hostelworld-hostel order',
    });
    ...
    log({ step: 'Hostelworld Hostel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hostelworld Hostel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hostelworld-hostel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hostelworld-hostel';
