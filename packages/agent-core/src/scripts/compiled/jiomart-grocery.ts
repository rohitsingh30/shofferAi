/**
 * Order groceries from JioMart — search products, add to cart, schedule delivery, checkout, pay.
 *
 * Auto-generated Playwright script for jiomart-grocery.
 * Site: https://www.jiomart.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Jiomart Grocery ──────────────────────────
    log({ step: 'Opening Jiomart Grocery...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Jiomart Grocery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Jiomart Grocery',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Jiomart Grocery order',
      details: 'Completing your order on Jiomart Grocery',
    ...
    log({ step: 'Jiomart Grocery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Jiomart Grocery. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'jiomart-grocery-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'jiomart-grocery';
