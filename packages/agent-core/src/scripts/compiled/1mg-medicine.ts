/**
 * Order medicines on 1mg (Tata Health) — search by name, upload prescription, compare generics, checkout.
 *
 * Auto-generated Playwright script for 1mg-medicine.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to 1mg Medicine ──────────────────────────
    log({ step: 'Opening 1mg Medicine...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on 1mg Medicine. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with 1mg Medicine',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete 1mg Medicine order',
      details: 'Completing your order on 1mg Medicine',
    ...
    log({ step: '1mg Medicine workflow completed', status: 'completed' });
    log({ message: 'Task completed on 1mg Medicine. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, '1mg-medicine-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = '1mg-medicine';
