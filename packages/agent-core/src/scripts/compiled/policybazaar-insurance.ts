/**
 * Buy car, bike, health, or term life insurance on PolicyBazaar — compare plans, get quotes, purchase.
 *
 * Auto-generated Playwright script for policybazaar-insurance.
 * Site: https://www.policybazaar.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Policybazaar Insurance ──────────────────────────
    log({ step: 'Opening Policybazaar Insurance...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Policybazaar Insurance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Policybazaar Insurance',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Policybazaar Insurance order',
      details: 'Completing your order on Policybazaar Insurance',
      amountInr: 0, // Extracted from page at runtime
      description: 'policybazaar-insurance order',
    });
    ...
    log({ step: 'Policybazaar Insurance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Policybazaar Insurance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'policybazaar-insurance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'policybazaar-insurance';
