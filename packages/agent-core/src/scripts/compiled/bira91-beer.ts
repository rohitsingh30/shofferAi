/**
 * Order craft beer from Bira 91 or HipBar — wheat beer, IPA, lager, packs, delivery where legally permitted, checkout, pay.
 *
 * Auto-generated Playwright script for bira91-beer.
 * Site: https://www.bira91.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bira91 Beer ──────────────────────────
    log({ step: 'Opening Bira91 Beer...', status: 'running' });
    await page.goto('https://www.bira91.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bira91 Beer. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bira91 Beer',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bira91 Beer order',
      details: 'Completing your order on Bira91 Beer',
      amountInr: 0, // Extracted from page at runtime
      description: 'bira91-beer order',
    });
    ...
    log({ step: 'Bira91 Beer workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bira91 Beer. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bira91-beer-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bira91-beer';
