/**
 * Get device repair or protection plan on Servify — phone screen repair, extended warranty, damage protection.
 *
 * Auto-generated Playwright script for servify-repair.
 * Site: https://www.servify.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Servify Repair ──────────────────────────
    log({ step: 'Opening Servify Repair...', status: 'running' });
    await page.goto('https://www.servify.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Servify Repair. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Servify Repair',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Servify Repair order',
      details: 'Completing your order on Servify Repair',
      amountInr: 0, // Extracted from page at runtime
      description: 'servify-repair order',
    });
    ...
    log({ step: 'Servify Repair workflow completed', status: 'completed' });
    log({ message: 'Task completed on Servify Repair. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'servify-repair-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'servify-repair';
