/**
 * Subscribe to Voot Select (JioCinema). Colors, MTV, Nickelodeon content, originals, and Paramount+ shows.
 *
 * Auto-generated Playwright script for voot-subscribe.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Voot Subscribe ──────────────────────────
    log({ step: 'Opening Voot Subscribe...', status: 'running' });
    await page.goto('https://www.voot.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Voot Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Voot Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Voot Subscribe order',
      details: 'Completing your order on Voot Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'voot-subscribe order',
    });
    ...
    log({ step: 'Voot Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Voot Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'voot-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'voot-subscribe';
