/**
 * Subscribe to Wynk Music (by Airtel). Music streaming, podcasts, Wynk Studio, and offline downloads.
 *
 * Auto-generated Playwright script for wynk-music.
 * Site: https://wynk.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wynk Music ──────────────────────────
    log({ step: 'Opening Wynk Music...', status: 'running' });
    await page.goto('https://wynk.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wynk Music. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Wynk Music',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wynk Music order',
      details: 'Completing your order on Wynk Music',
      amountInr: 0, // Extracted from page at runtime
      description: 'wynk-music order',
    });
    ...
    log({ step: 'Wynk Music workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wynk Music. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wynk-music-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wynk-music';
