/**
 * Pay postpaid mobile bill on Paytm. Enter number, auto-detect operator, fetch bill, and pay.
 *
 * Auto-generated Playwright script for paytm-postpaid.
 * Site: https://paytm.com/postpaid-mobile-bill-payment
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Postpaid ──────────────────────────
    log({ step: 'Opening Paytm Postpaid...', status: 'running' });
    await page.goto('https://paytm.com/postpaid-mobile-bill-payment');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Postpaid. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Paytm Postpaid',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Postpaid order',
      details: 'Completing your order on Paytm Postpaid',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-postpaid order',
    });
    ...
    log({ step: 'Paytm Postpaid workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Postpaid. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-postpaid-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-postpaid';
