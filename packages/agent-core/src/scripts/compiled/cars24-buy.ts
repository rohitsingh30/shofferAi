/**
 * Buy a used car on Cars24 — search, filter, compare, book test drive, apply for financing, purchase.
 *
 * Auto-generated Playwright script for cars24-buy.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cars24 Buy ──────────────────────────
    log({ step: 'Opening Cars24 Buy...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cars24 Buy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cars24 Buy',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cars24 Buy order',
      details: 'Completing your order on Cars24 Buy',
      amountInr: 0, // Extracted from page at runtime
      description: 'cars24-buy order',
    });
    ...
    log({ step: 'Cars24 Buy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cars24 Buy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cars24-buy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cars24-buy';
