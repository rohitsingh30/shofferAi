/**
 * Book student and working professional stays on Stanza Living — managed PG, co-living residences.
 *
 * Auto-generated Playwright script for stanza-living.
 * Site: https://www.stanzaliving.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Stanza Living ──────────────────────────
    log({ step: 'Opening Stanza Living...', status: 'running' });
    await page.goto('https://www.stanzaliving.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Stanza Living. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Stanza Living',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Stanza Living order',
      details: 'Completing your order on Stanza Living',
      amountInr: 0, // Extracted from page at runtime
      description: 'stanza-living order',
    });
    ...
    log({ step: 'Stanza Living workflow completed', status: 'completed' });
    log({ message: 'Task completed on Stanza Living. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'stanza-living-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'stanza-living';
