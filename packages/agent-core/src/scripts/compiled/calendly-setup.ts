/**
 * Set up Calendly scheduling pages with event types, availability, and booking links — meeting scheduling automation.
 *
 * Auto-generated Playwright script for calendly-setup.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Calendly Setup ──────────────────────────
    log({ step: 'Opening Calendly Setup...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Calendly Setup. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Calendly Setup',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Calendly Setup order',
      details: 'Completing your order on Calendly Setup',
      amountInr: 0, // Extracted from page at runtime
      description: 'calendly-setup order',
    });
    ...
    log({ step: 'Calendly Setup workflow completed', status: 'completed' });
    log({ message: 'Task completed on Calendly Setup. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'calendly-setup-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'calendly-setup';
