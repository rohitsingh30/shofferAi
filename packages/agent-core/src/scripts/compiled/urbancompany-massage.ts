/**
 * Book a spa massage at home on Urban Company — select massage type, duration, therapist gender, schedule, pay.
 *
 * Auto-generated Playwright script for urbancompany-massage.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbancompany Massage ──────────────────────────
    log({ step: 'Opening Urbancompany Massage...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbancompany Massage. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbancompany Massage',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbancompany Massage order',
      details: 'Completing your order on Urbancompany Massage',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbancompany-massage order',
    });
    ...
    log({ step: 'Urbancompany Massage workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbancompany Massage. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbancompany-massage-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbancompany-massage';
