/**
 * Pay water bill on Paytm. Select provider, enter connection/consumer number, and pay.
 *
 * Auto-generated Playwright script for paytm-water.
 * Site: https://paytm.com/water-bill-payment
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Water ──────────────────────────
    log({ step: 'Opening Paytm Water...', status: 'running' });
    await page.goto('https://paytm.com/water-bill-payment');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Water. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Paytm Water',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Water order',
      details: 'Completing your order on Paytm Water',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-water order',
    });
    ...
    log({ step: 'Paytm Water workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Water. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-water-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-water';
