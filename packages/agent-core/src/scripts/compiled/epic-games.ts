/**
 * Buy or claim games on Epic Games Store. Browse store, buy games, claim free weekly games, and manage library.
 *
 * Auto-generated Playwright script for epic-games.
 * Site: https://store.epicgames.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Epic Games ──────────────────────────
    log({ step: 'Opening Epic Games...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Epic Games. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Epic Games',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Epic Games order',
      details: 'Completing your order on Epic Games',
      amountInr: 0, // Extracted from page at runtime
      description: 'epic-games order',
    });
    ...
    log({ step: 'Epic Games workflow completed', status: 'completed' });
    log({ message: 'Task completed on Epic Games. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'epic-games-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'epic-games';
