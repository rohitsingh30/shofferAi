/**
 * Buy extended warranty or damage protection on Onsitego — for phones, laptops, appliances, TVs, wearables.
 *
 * Auto-generated Playwright script for onsitego-warranty.
 * Site: https://www.onsitego.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Onsitego Warranty ──────────────────────────
    log({ step: 'Opening Onsitego Warranty...', status: 'running' });
    await page.goto('https://www.onsitego.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Onsitego Warranty. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Onsitego Warranty',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Onsitego Warranty order',
      details: 'Completing your order on Onsitego Warranty',
      amountInr: 0, // Extracted from page at runtime
      description: 'onsitego-warranty order',
    });
    ...
    log({ step: 'Onsitego Warranty workflow completed', status: 'completed' });
    log({ message: 'Task completed on Onsitego Warranty. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'onsitego-warranty-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'onsitego-warranty';
