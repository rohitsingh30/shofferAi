/**
 * Shop on Urban Ladder — browse sofas, beds, tables, storage, check delivery, checkout, pay.
 *
 * Auto-generated Playwright script for urbanladder-furniture.
 * Site: https://www.urbanladder.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbanladder Furniture ──────────────────────────
    log({ step: 'Opening Urbanladder Furniture...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbanladder Furniture. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbanladder Furniture',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbanladder Furniture order',
      details: 'Completing your order on Urbanladder Furniture',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbanladder-furniture order',
    });
    ...
    log({ step: 'Urbanladder Furniture workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbanladder Furniture. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbanladder-furniture-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbanladder-furniture';
