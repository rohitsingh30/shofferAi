/**
 * Reorder last food order from Zomato — view order history, repeat a previous order, checkout, pay.
 *
 * Auto-generated Playwright script for zomato-reorder.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Zomato Reorder ──────────────────────────
    log({ step: 'Opening Zomato Reorder...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zomato Reorder. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zomato Reorder',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Zomato Reorder order',
      details: 'Completing your order on Zomato Reorder',
      amountInr: 0, // Extracted from page at runtime
      description: 'zomato-reorder order',
    });
    ...
    log({ step: 'Zomato Reorder workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zomato Reorder. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'zomato-reorder-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'zomato-reorder';
