/**
 * Buy A2 ghee, honey, cold-pressed oils, and farm-to-table products from Anveshan — browse, add to cart, order online.
 *
 * Auto-generated Playwright script for anveshan-ghee.
 * Site: https://www.anveshan.farm
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Anveshan Ghee ──────────────────────────
    log({ step: 'Opening Anveshan Ghee...', status: 'running' });
    await page.goto('https://www.anveshan.farm');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Anveshan Ghee. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Anveshan Ghee',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Anveshan Ghee order',
      details: 'Completing your order on Anveshan Ghee',
      amountInr: 0, // Extracted from page at runtime
      description: 'anveshan-ghee order',
    });
    ...
    log({ step: 'Anveshan Ghee workflow completed', status: 'completed' });
    log({ message: 'Task completed on Anveshan Ghee. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'anveshan-ghee-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'anveshan-ghee';
