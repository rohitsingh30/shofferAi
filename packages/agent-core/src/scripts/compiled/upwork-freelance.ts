/**
 * Find and apply for freelance jobs on Upwork — search projects, review details, submit proposals with cover letter.
 *
 * Auto-generated Playwright script for upwork-freelance.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Upwork Freelance ──────────────────────────
    log({ step: 'Opening Upwork Freelance...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Upwork Freelance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Upwork Freelance',
    ...
    log({ step: 'Upwork Freelance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Upwork Freelance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'upwork-freelance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'upwork-freelance';
