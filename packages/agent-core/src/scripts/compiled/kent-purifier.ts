/**
 * Book Kent water purifier service — AMC, filter change, membrane replacement, repair, installation.
 *
 * Auto-generated Playwright script for kent-purifier.
 * Site: https://www.kent.co.in/customer-services
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Kent Purifier ──────────────────────────
    log({ step: 'Opening Kent Purifier...', status: 'running' });
    await page.goto('https://www.kent.co.in/customer-services');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Kent Purifier. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Kent Purifier',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Kent Purifier order',
      details: 'Completing your order on Kent Purifier',
      amountInr: 0, // Extracted from page at runtime
      description: 'kent-purifier order',
    });
    ...
    log({ step: 'Kent Purifier workflow completed', status: 'completed' });
    log({ message: 'Task completed on Kent Purifier. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'kent-purifier-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'kent-purifier';
