/**
 * Buy protein, vitamins, and supplements on HealthKart — search, compare brands, check authenticity, order.
 *
 * Auto-generated Playwright script for healthkart-supplements.
 * Site: https://www.healthkart.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Healthkart Supplements ──────────────────────────
    log({ step: 'Opening Healthkart Supplements...', status: 'running' });
    await page.goto('https://www.healthkart.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Healthkart Supplements. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Healthkart Supplements',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Healthkart Supplements order',
      details: 'Completing your order on Healthkart Supplements',
      amountInr: 0, // Extracted from page at runtime
      description: 'healthkart-supplements order',
    });
    ...
    log({ step: 'Healthkart Supplements workflow completed', status: 'completed' });
    log({ message: 'Task completed on Healthkart Supplements. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'healthkart-supplements-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'healthkart-supplements';
