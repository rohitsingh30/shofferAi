/**
 * Shop on Max Fashion India — buy affordable clothing, footwear, accessories for the whole family, checkout, pay.
 *
 * Auto-generated Playwright script for max-fashion.
 * Site: https://www.maxfashion.in
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Max Fashion...', status: 'running' });
    await page.goto('https://www.maxfashion.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Max Fashion. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Max Fashion',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Max Fashion order',
      details: 'Completing your order on Max Fashion',
    ...
    log({ step: 'Max Fashion workflow completed', status: 'completed' });
    log({ message: 'Task completed on Max Fashion. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'max-fashion-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'max-fashion';
