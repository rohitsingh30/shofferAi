/**
 * Create Figma design files with frames, templates, and wireframes — UI/UX design and prototyping.
 *
 * Auto-generated Playwright script for figma-design.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Figma Design...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Figma Design. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Figma Design',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Figma Design order',
      details: 'Completing your order on Figma Design',
      amountInr: 0, // Extracted from page at runtime
      description: 'figma-design order',
    });
    ...
    log({ step: 'Figma Design workflow completed', status: 'completed' });
    log({ message: 'Task completed on Figma Design. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'figma-design-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'figma-design';
