/**
 * Buy Havells products on Havells India — fans, lights, water heaters, switches, wires, kitchen appliances, water purifiers.
 *
 * Auto-generated Playwright script for havells-store.
 * Site: https://www.havells.com/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Havells Store ──────────────────────────
    log({ step: 'Opening Havells Store...', status: 'running' });
    await page.goto('https://www.havells.com/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Havells Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Havells Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Havells Store order',
      details: 'Completing your order on Havells Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'havells-store order',
    });
    ...
    log({ step: 'Havells Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Havells Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'havells-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'havells-store';
