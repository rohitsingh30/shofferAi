/**
 * Order food from EatSure multi-brand kitchen — Behrouz Biryani, Faasos, Oven Story Pizza, and more.
 *
 * Auto-generated Playwright script for eatsure-food.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Eatsure Food ──────────────────────────
    log({ step: 'Opening Eatsure Food...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Eatsure Food. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Eatsure Food',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Eatsure Food order',
      details: 'Completing your order on Eatsure Food',
    ...
    log({ step: 'Eatsure Food workflow completed', status: 'completed' });
    log({ message: 'Task completed on Eatsure Food. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'eatsure-food-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'eatsure-food';
