/**
 * Design and order custom t-shirts on Bewakoof — upload your design, select style/color/size, checkout.
 *
 * Auto-generated Playwright script for bewakoof-custom.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bewakoof Custom ──────────────────────────
    log({ step: 'Opening Bewakoof Custom...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bewakoof Custom. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bewakoof Custom',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bewakoof Custom order',
      details: 'Completing your order on Bewakoof Custom',
      amountInr: 0, // Extracted from page at runtime
      description: 'bewakoof-custom order',
    });
    ...
    log({ step: 'Bewakoof Custom workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bewakoof Custom. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bewakoof-custom-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bewakoof-custom';
