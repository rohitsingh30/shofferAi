/**
 * Create Typeform surveys and quizzes with questions, logic jumps, and shareable links — beautiful form design.
 *
 * Auto-generated Playwright script for typeform-survey.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Typeform Survey ──────────────────────────
    log({ step: 'Opening Typeform Survey...', status: 'running' });
    await page.goto('https://www.typeform.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Typeform Survey. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Typeform Survey',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Typeform Survey order',
      details: 'Completing your order on Typeform Survey',
      amountInr: 0, // Extracted from page at runtime
      description: 'typeform-survey order',
    });
    ...
    log({ step: 'Typeform Survey workflow completed', status: 'completed' });
    log({ message: 'Task completed on Typeform Survey. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'typeform-survey-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'typeform-survey';
