/**
 * Create YouTube ad campaign — video ads with targeting, budget, and bidding via Google Ads for YouTube placements.
 *
 * Auto-generated Playwright script for youtube-ads.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Youtube Ads ──────────────────────────
    log({ step: 'Opening Youtube Ads...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Youtube Ads. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Youtube Ads',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Youtube Ads order',
      details: 'Completing your order on Youtube Ads',
      amountInr: 0, // Extracted from page at runtime
      description: 'youtube-ads order',
    });
    ...
    log({ step: 'Youtube Ads workflow completed', status: 'completed' });
    log({ message: 'Task completed on Youtube Ads. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'youtube-ads-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'youtube-ads';
