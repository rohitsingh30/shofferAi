/**
 * Order food on Uber Eats — browse restaurants, select dishes, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for uber-eats.
 * Site: https://www.ubereats.com
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Uber Eats...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Uber Eats. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Uber Eats order',
      details: 'Completing your order on Uber Eats',
    ...
    log({ step: 'Uber Eats workflow completed', status: 'completed' });
    log({ message: 'Task completed on Uber Eats. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'uber-eats-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'uber-eats';
