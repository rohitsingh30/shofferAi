/**
 * Buy protein supplements on MuscleBlaze — browse whey protein, mass gainers, BCAAs, compare, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for muscleblaze-protein.
 * Site: https://www.muscleblaze.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Muscleblaze Protein ──────────────────────────
    log({ step: 'Opening Muscleblaze Protein...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Muscleblaze Protein. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Muscleblaze Protein',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Muscleblaze Protein order',
      details: 'Completing your order on Muscleblaze Protein',
      amountInr: 0, // Extracted from page at runtime
      description: 'muscleblaze-protein order',
    });
    ...
    log({ step: 'Muscleblaze Protein workflow completed', status: 'completed' });
    log({ message: 'Task completed on Muscleblaze Protein. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'muscleblaze-protein-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'muscleblaze-protein';
