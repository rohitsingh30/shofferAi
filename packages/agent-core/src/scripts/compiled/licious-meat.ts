/**
 * Order fresh meat and seafood from Licious — browse cuts, marinations, ready-to-cook, checkout, pay.
 *
 * Auto-generated Playwright script for licious-meat.
 * Site: https://www.licious.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Licious Meat ──────────────────────────
    log({ step: 'Opening Licious Meat...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Licious Meat. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Licious Meat',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Licious Meat order',
      details: 'Completing your order on Licious Meat',
      amountInr: 0, // Extracted from page at runtime
      description: 'licious-meat order',
    });
    ...
    log({ step: 'Licious Meat workflow completed', status: 'completed' });
    log({ message: 'Task completed on Licious Meat. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'licious-meat-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'licious-meat';
