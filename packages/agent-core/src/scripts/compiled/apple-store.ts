/**
 * Buy Apple products on Apple India — iPhone, Mac, iPad, Apple Watch, AirPods, accessories.
 *
 * Auto-generated Playwright script for apple-store.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Apple Store...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Apple Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Apple Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Apple Store order',
      details: 'Completing your order on Apple Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'apple-store order',
    });
    ...
    log({ step: 'Apple Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Apple Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'apple-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'apple-store';
