/**
 * Book group tours on SOTC — international packages, fixed departures, guided tours, flights+hotels+sightseeing combos.
 *
 * Auto-generated Playwright script for sotc-holiday.
 * Site: https://www.sotc.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Sotc Holiday ──────────────────────────
    log({ step: 'Opening Sotc Holiday...', status: 'running' });
    await page.goto('https://www.sotc.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Sotc Holiday. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Sotc Holiday',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Sotc Holiday order',
      details: 'Completing your order on Sotc Holiday',
      amountInr: 0, // Extracted from page at runtime
      description: 'sotc-holiday order',
    });
    ...
    log({ step: 'Sotc Holiday workflow completed', status: 'completed' });
    log({ message: 'Task completed on Sotc Holiday. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'sotc-holiday-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'sotc-holiday';
