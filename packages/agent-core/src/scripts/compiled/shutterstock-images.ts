/**
 * Search and purchase stock images on Shutterstock — find photos, vectors, illustrations, license and download high-resolution assets.
 *
 * Auto-generated Playwright script for shutterstock-images.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Shutterstock Images ──────────────────────────
    log({ step: 'Opening Shutterstock Images...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Shutterstock Images. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Shutterstock Images',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Shutterstock Images order',
      details: 'Completing your order on Shutterstock Images',
      amountInr: 0, // Extracted from page at runtime
      description: 'shutterstock-images order',
    });
    ...
    log({ step: 'Shutterstock Images workflow completed', status: 'completed' });
    log({ message: 'Task completed on Shutterstock Images. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'shutterstock-images-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'shutterstock-images';
