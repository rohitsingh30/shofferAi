/**
 * Shop men's casual and smart casual wear on Indian Terrain — browse shirts, trousers, jackets, polos, add to bag, checkout, pay.
 *
 * Auto-generated Playwright script for indian-terrain.
 * Site: https://www.indianterrain.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Indian Terrain ──────────────────────────
    log({ step: 'Opening Indian Terrain...', status: 'running' });
    await page.goto('https://www.indianterrain.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Indian Terrain. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Indian Terrain',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Indian Terrain order',
      details: 'Completing your order on Indian Terrain',
      amountInr: 0, // Extracted from page at runtime
      description: 'indian-terrain order',
    });
    ...
    log({ step: 'Indian Terrain workflow completed', status: 'completed' });
    log({ message: 'Task completed on Indian Terrain. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'indian-terrain-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'indian-terrain';
