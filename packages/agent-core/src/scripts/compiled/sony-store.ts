/**
 * Buy Sony products on Sony India — headphones, cameras, TVs, PS5, speakers, earbuds.
 *
 * Auto-generated Playwright script for sony-store.
 * Site: https://www.sony.co.in/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Sony Store ──────────────────────────
    log({ step: 'Opening Sony Store...', status: 'running' });
    await page.goto('https://www.sony.co.in/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Sony Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Sony Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Sony Store order',
      details: 'Completing your order on Sony Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'sony-store order',
    });
    ...
    log({ step: 'Sony Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Sony Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'sony-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'sony-store';
