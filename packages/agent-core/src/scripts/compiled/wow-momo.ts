/**
 * Order momos from Wow! Momo — steamed, fried, baked momos, momo bowls, combos, checkout, pay.
 *
 * Auto-generated Playwright script for wow-momo.
 * Site: https://www.wowmomo.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wow Momo ──────────────────────────
    log({ step: 'Opening Wow Momo...', status: 'running' });
    await page.goto('https://www.wowmomo.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wow Momo. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wow Momo order',
      details: 'Completing your order on Wow Momo',
    ...
    log({ step: 'Wow Momo workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wow Momo. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wow-momo-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wow-momo';
