/**
 * Book a meeting via someone's Calendly link — select available time slot, fill in details, confirm booking.
 *
 * Auto-generated Playwright script for calendly-booking.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Calendly Booking ──────────────────────────
    log({ step: 'Opening Calendly Booking...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Calendly Booking. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Calendly Booking',
    ...
    log({ step: 'Calendly Booking workflow completed', status: 'completed' });
    log({ message: 'Task completed on Calendly Booking. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'calendly-booking-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'calendly-booking';
