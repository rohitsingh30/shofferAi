/**
 * Shop on WoodenStreet — browse solid wood furniture, customize finish/fabric, checkout, pay.
 *
 * Auto-generated Playwright script for woodenstreet-furniture.
 * Site: https://www.woodenstreet.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Woodenstreet Furniture ──────────────────────────
    log({ step: 'Opening Woodenstreet Furniture...', status: 'running' });
    await page.goto('https://www.woodenstreet.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Woodenstreet Furniture. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Woodenstreet Furniture',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Woodenstreet Furniture order',
      details: 'Completing your order on Woodenstreet Furniture',
      amountInr: 0, // Extracted from page at runtime
      description: 'woodenstreet-furniture order',
    });
    ...
    log({ step: 'Woodenstreet Furniture workflow completed', status: 'completed' });
    log({ message: 'Task completed on Woodenstreet Furniture. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'woodenstreet-furniture-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'woodenstreet-furniture';
