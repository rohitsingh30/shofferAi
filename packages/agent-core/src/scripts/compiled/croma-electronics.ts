/**
 * Shop on Croma — buy electronics, appliances, gadgets, compare specs, buy with installation, checkout, pay.
 *
 * Auto-generated Playwright script for croma-electronics.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Croma Electronics ──────────────────────────
    log({ step: 'Opening Croma Electronics...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Croma Electronics. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Croma Electronics',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Croma Electronics order',
      details: 'Completing your order on Croma Electronics',
    ...
    log({ step: 'Croma Electronics workflow completed', status: 'completed' });
    log({ message: 'Task completed on Croma Electronics. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'croma-electronics-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'croma-electronics';
