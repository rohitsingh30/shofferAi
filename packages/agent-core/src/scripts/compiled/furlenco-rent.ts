/**
 * Rent furniture on Furlenco — browse packages and individual items, select monthly subscription, pay.
 *
 * Auto-generated Playwright script for furlenco-rent.
 * Site: https://www.furlenco.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Furlenco Rent ──────────────────────────
    log({ step: 'Opening Furlenco Rent...', status: 'running' });
    await page.goto('https://www.furlenco.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Furlenco Rent. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Furlenco Rent',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Furlenco Rent order',
      details: 'Completing your order on Furlenco Rent',
      amountInr: 0, // Extracted from page at runtime
      description: 'furlenco-rent order',
    });
    ...
    log({ step: 'Furlenco Rent workflow completed', status: 'completed' });
    log({ message: 'Task completed on Furlenco Rent. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'furlenco-rent-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'furlenco-rent';
