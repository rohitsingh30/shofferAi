/**
 * Order chai and snacks from Chai Point — masala chai, iced tea, snacks, combos, delivery to office/home, checkout, pay.
 *
 * Auto-generated Playwright script for chai-point.
 * Site: https://www.chaipoint.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Chai Point ──────────────────────────
    log({ step: 'Opening Chai Point...', status: 'running' });
    await page.goto('https://www.chaipoint.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Chai Point. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Chai Point',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Chai Point order',
      details: 'Completing your order on Chai Point',
    ...
    log({ step: 'Chai Point workflow completed', status: 'completed' });
    log({ message: 'Task completed on Chai Point. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'chai-point-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'chai-point';
