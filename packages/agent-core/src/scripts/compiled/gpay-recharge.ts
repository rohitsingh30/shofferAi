/**
 * Mobile recharge on Google Pay web — prepaid/postpaid recharge, select plan, pay via GPay.
 *
 * Auto-generated Playwright script for gpay-recharge.
 * Site: https://pay.google.com/gp/w/home/activity
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Gpay Recharge ──────────────────────────
    log({ step: 'Opening Gpay Recharge...', status: 'running' });
    await page.goto('https://pay.google.com/gp/w/home/activity');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Gpay Recharge. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Gpay Recharge',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Gpay Recharge order',
      details: 'Completing your order on Gpay Recharge',
      amountInr: 0, // Extracted from page at runtime
      description: 'gpay-recharge order',
    });
    ...
    log({ step: 'Gpay Recharge workflow completed', status: 'completed' });
    log({ message: 'Task completed on Gpay Recharge. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'gpay-recharge-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'gpay-recharge';
