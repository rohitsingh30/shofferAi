/**
 * Book Apple device repair at iService or Apple Authorized Service Provider — iPhone, iPad, Mac, Apple Watch repair.
 *
 * Auto-generated Playwright script for iservice-apple.
 * Site: https://www.iservice.co.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Iservice Apple ──────────────────────────
    log({ step: 'Opening Iservice Apple...', status: 'running' });
    await page.goto('https://www.iservice.co.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Iservice Apple. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Iservice Apple',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Iservice Apple order',
      details: 'Completing your order on Iservice Apple',
      amountInr: 0, // Extracted from page at runtime
      description: 'iservice-apple order',
    });
    ...
    log({ step: 'Iservice Apple workflow completed', status: 'completed' });
    log({ message: 'Task completed on Iservice Apple. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'iservice-apple-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'iservice-apple';
