/**
 * Shop on CustomFurnish — buy customized curtains, blinds, wallpapers, measure and order, checkout, pay.
 *
 * Auto-generated Playwright script for customfurnish-curtains.
 * Site: https://www.customfurnish.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Customfurnish Curtains ──────────────────────────
    log({ step: 'Opening Customfurnish Curtains...', status: 'running' });
    await page.goto('https://www.customfurnish.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Customfurnish Curtains. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Customfurnish Curtains',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Customfurnish Curtains order',
      details: 'Completing your order on Customfurnish Curtains',
      amountInr: 0, // Extracted from page at runtime
      description: 'customfurnish-curtains order',
    });
    ...
    log({ step: 'Customfurnish Curtains workflow completed', status: 'completed' });
    log({ message: 'Task completed on Customfurnish Curtains. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'customfurnish-curtains-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'customfurnish-curtains';
