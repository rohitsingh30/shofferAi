/**
 * Trade stocks, options, and IPOs on Angel One — search stocks, place buy/sell orders, check portfolio, trade F&O.
 *
 * Auto-generated Playwright script for angel-one-trading.
 * Site: https://www.angelone.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Angel One Trading ──────────────────────────
    log({ step: 'Opening Angel One Trading...', status: 'running' });
    await page.goto('https://www.angelone.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Angel One Trading. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Angel One Trading',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Angel One Trading order',
      details: 'Completing your order on Angel One Trading',
      amountInr: 0, // Extracted from page at runtime
      description: 'angel-one-trading order',
    });
    ...
    log({ step: 'Angel One Trading workflow completed', status: 'completed' });
    log({ message: 'Task completed on Angel One Trading. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'angel-one-trading-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'angel-one-trading';
