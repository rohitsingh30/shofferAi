/**
 * Set up daily milk and grocery subscription on Milkbasket — select items, schedule deliveries, manage subscription.
 *
 * Auto-generated Playwright script for milkbasket-daily.
 * Site: https://www.milkbasket.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Milkbasket Daily ──────────────────────────
    log({ step: 'Opening Milkbasket Daily...', status: 'running' });
    await page.goto('https://www.milkbasket.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Milkbasket Daily. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Milkbasket Daily',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Milkbasket Daily order',
      details: 'Completing your order on Milkbasket Daily',
      amountInr: 0, // Extracted from page at runtime
      description: 'milkbasket-daily order',
    });
    ...
    log({ step: 'Milkbasket Daily workflow completed', status: 'completed' });
    log({ message: 'Task completed on Milkbasket Daily. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'milkbasket-daily-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'milkbasket-daily';
