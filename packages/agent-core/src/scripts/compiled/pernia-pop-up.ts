/**
 * Shop designer Indian wear on Pernia's Pop-Up Shop — browse lehengas, sarees, kurtas, bridal wear from top Indian designers, checkout, pay.
 *
 * Auto-generated Playwright script for pernia-pop-up.
 * Site: https://www.perniaspopupshop.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Pernia Pop Up ──────────────────────────
    log({ step: 'Opening Pernia Pop Up...', status: 'running' });
    await page.goto('https://www.perniaspopupshop.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Pernia Pop Up. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Pernia Pop Up',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Pernia Pop Up order',
      details: 'Completing your order on Pernia Pop Up',
      amountInr: 0, // Extracted from page at runtime
      description: 'pernia-pop-up order',
    });
    ...
    log({ step: 'Pernia Pop Up workflow completed', status: 'completed' });
    log({ message: 'Task completed on Pernia Pop Up. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'pernia-pop-up-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'pernia-pop-up';
