/**
 * Open or contribute to NPS (National Pension System) account on npscra.nsdl.co.in — register, invest, check balance.
 *
 * Auto-generated Playwright script for nps-account.
 * Site: https://enps.nsdl.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Nps Account ──────────────────────────
    log({ step: 'Opening Nps Account...', status: 'running' });
    await page.goto('https://enps.nsdl.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Nps Account. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Nps Account',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Nps Account order',
      details: 'Completing your order on Nps Account',
      amountInr: 0, // Extracted from page at runtime
      description: 'nps-account order',
    });
    ...
    log({ step: 'Nps Account workflow completed', status: 'completed' });
    log({ message: 'Task completed on Nps Account. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'nps-account-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'nps-account';
