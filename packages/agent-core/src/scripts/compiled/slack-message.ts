/**
 * Send Slack messages, create channels, and manage team communication — DMs, channels, threads.
 *
 * Auto-generated Playwright script for slack-message.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Slack Message ──────────────────────────
    log({ step: 'Opening Slack Message...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Slack Message. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Slack Message',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Slack Message order',
      details: 'Completing your order on Slack Message',
      amountInr: 0, // Extracted from page at runtime
      description: 'slack-message order',
    });
    ...
    log({ step: 'Slack Message workflow completed', status: 'completed' });
    log({ message: 'Task completed on Slack Message. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'slack-message-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'slack-message';
