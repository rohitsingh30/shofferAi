/**
 * Buy HDFC ERGO insurance — car, health, travel, two-wheeler, home, and cyber insurance plans.
 *
 * Auto-generated Playwright script for hdfc-ergo-insurance.
 * Site: https://www.hdfcergo.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hdfc Ergo Insurance ──────────────────────────
    log({ step: 'Opening Hdfc Ergo Insurance...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hdfc Ergo Insurance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hdfc Ergo Insurance',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hdfc Ergo Insurance order',
      details: 'Completing your order on Hdfc Ergo Insurance',
      amountInr: 0, // Extracted from page at runtime
      description: 'hdfc-ergo-insurance order',
    });
    ...
    log({ step: 'Hdfc Ergo Insurance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hdfc Ergo Insurance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hdfc-ergo-insurance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hdfc-ergo-insurance';
