/**
 * Ship packages via Shadowfax — hyperlocal and intracity delivery, same-day and next-day shipping, track consignments.
 *
 * Auto-generated Playwright script for shadowfax-delivery.
 * Site: https://www.shadowfax.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Shadowfax Delivery ──────────────────────────
    log({ step: 'Opening Shadowfax Delivery...', status: 'running' });
    await page.goto('https://www.shadowfax.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Shadowfax Delivery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Shadowfax Delivery',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Shadowfax Delivery order',
      details: 'Completing your order on Shadowfax Delivery',
      amountInr: 0, // Extracted from page at runtime
      description: 'shadowfax-delivery order',
    });
    ...
    log({ step: 'Shadowfax Delivery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Shadowfax Delivery. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'shadowfax-delivery-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'shadowfax-delivery';
