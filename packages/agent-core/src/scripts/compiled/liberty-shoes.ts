/**
 * Shop on Liberty Shoes — browse, select size, buy formal, casual, kids, and sports shoes, checkout, pay.
 *
 * Auto-generated Playwright script for liberty-shoes.
 * Site: https://www.libertyshoes.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Liberty Shoes ──────────────────────────
    log({ step: 'Opening Liberty Shoes...', status: 'running' });
    await page.goto('https://www.libertyshoes.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Liberty Shoes. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Liberty Shoes',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Liberty Shoes order',
      details: 'Completing your order on Liberty Shoes',
    ...
    log({ step: 'Liberty Shoes workflow completed', status: 'completed' });
    log({ message: 'Task completed on Liberty Shoes. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'liberty-shoes-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'liberty-shoes';
