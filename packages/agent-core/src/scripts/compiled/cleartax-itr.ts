/**
 * File income tax return on ClearTax — import Form 16, auto-fill details, claim deductions, file ITR.
 *
 * Auto-generated Playwright script for cleartax-itr.
 * Site: https://cleartax.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cleartax Itr ──────────────────────────
    log({ step: 'Opening Cleartax Itr...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cleartax Itr. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cleartax Itr',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cleartax Itr order',
      details: 'Completing your order on Cleartax Itr',
      amountInr: 0, // Extracted from page at runtime
      description: 'cleartax-itr order',
    });
    ...
    log({ step: 'Cleartax Itr workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cleartax Itr. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cleartax-itr-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cleartax-itr';
