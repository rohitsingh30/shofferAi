/**
 * Subscribe to YouTube Premium India — choose individual or family plan, ad-free videos and music, pay.
 *
 * Auto-generated Playwright script for youtube-premium.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Youtube Premium ──────────────────────────
    log({ step: 'Opening Youtube Premium...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Youtube Premium. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Youtube Premium',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Youtube Premium order',
      details: 'Completing your order on Youtube Premium',
      amountInr: 0, // Extracted from page at runtime
      description: 'youtube-premium order',
    });
    ...
    log({ step: 'Youtube Premium workflow completed', status: 'completed' });
    log({ message: 'Task completed on Youtube Premium. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'youtube-premium-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'youtube-premium';
