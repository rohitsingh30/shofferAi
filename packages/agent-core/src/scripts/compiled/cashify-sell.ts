/**
 * Sell old phone, laptop, tablet, or smartwatch on Cashify — get instant quote, schedule pickup, get paid.
 *
 * Auto-generated Playwright script for cashify-sell.
 * Site: https://www.cashify.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cashify Sell ──────────────────────────
    log({ step: 'Opening Cashify Sell...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cashify Sell. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cashify Sell',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cashify Sell order',
      details: 'Completing your order on Cashify Sell',
      amountInr: 0, // Extracted from page at runtime
      description: 'cashify-sell order',
    });
    ...
    log({ step: 'Cashify Sell workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cashify Sell. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cashify-sell-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cashify-sell';
