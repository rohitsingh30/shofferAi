/**
 * Create designs on Canva — browse templates, customize text and images, download or share the final design.
 *
 * Auto-generated Playwright script for canva-design.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Canva Design ──────────────────────────
    log({ step: 'Opening Canva Design...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Canva Design. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Canva Design',
    ...
    log({ step: 'Canva Design workflow completed', status: 'completed' });
    log({ message: 'Task completed on Canva Design. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'canva-design-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'canva-design';
