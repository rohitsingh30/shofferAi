/**
 * Recharge Tata Play (formerly Tata Sky) DTH — select plan/pack, enter subscriber ID, pay.
 *
 * Auto-generated Playwright script for tata-play-dth.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Tata Play Dth ──────────────────────────
    log({ step: 'Opening Tata Play Dth...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Tata Play Dth. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Tata Play Dth',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Tata Play Dth order',
      details: 'Completing your order on Tata Play Dth',
      amountInr: 0, // Extracted from page at runtime
      description: 'tata-play-dth order',
    });
    ...
    log({ step: 'Tata Play Dth workflow completed', status: 'completed' });
    log({ message: 'Task completed on Tata Play Dth. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'tata-play-dth-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'tata-play-dth';
