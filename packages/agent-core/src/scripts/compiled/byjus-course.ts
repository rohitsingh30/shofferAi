/**
 * Enroll in BYJU'S courses — K-12 learning, competitive exam prep, subscribe to BYJU'S app or classes.
 *
 * Auto-generated Playwright script for byjus-course.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Byjus Course ──────────────────────────
    log({ step: 'Opening Byjus Course...', status: 'running' });
    await page.goto('https://byjus.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Byjus Course. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Byjus Course',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Byjus Course order',
      details: 'Completing your order on Byjus Course',
      amountInr: 0, // Extracted from page at runtime
      description: 'byjus-course order',
    });
    ...
    log({ step: 'Byjus Course workflow completed', status: 'completed' });
    log({ message: 'Task completed on Byjus Course. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'byjus-course-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'byjus-course';
