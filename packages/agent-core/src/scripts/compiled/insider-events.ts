/**
 * Book events, workshops, and experiences on Insider.in — browse events, select tickets, pay.
 *
 * Auto-generated Playwright script for insider-events.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Insider Events ──────────────────────────
    log({ step: 'Opening Insider Events...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Insider Events. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Insider Events',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Insider Events order',
      details: 'Completing your order on Insider Events',
      amountInr: 0, // Extracted from page at runtime
      description: 'insider-events order',
    });
    ...
    log({ step: 'Insider Events workflow completed', status: 'completed' });
    log({ message: 'Task completed on Insider Events. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'insider-events-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'insider-events';
