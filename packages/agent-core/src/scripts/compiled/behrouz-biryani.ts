/**
 * Order biryani from Behrouz Biryani (via EatSure/Swiggy) — royal biryanis, kebabs, sides, checkout, pay.
 *
 * Auto-generated Playwright script for behrouz-biryani.
 * Site: https://www.eatsure.com/behrouz-biryani
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Behrouz Biryani ──────────────────────────
    log({ step: 'Opening Behrouz Biryani...', status: 'running' });
    await page.goto('https://www.eatsure.com/behrouz-biryani');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Behrouz Biryani. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Behrouz Biryani',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Behrouz Biryani order',
      details: 'Completing your order on Behrouz Biryani',
    ...
    log({ step: 'Behrouz Biryani workflow completed', status: 'completed' });
    log({ message: 'Task completed on Behrouz Biryani. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'behrouz-biryani-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'behrouz-biryani';
