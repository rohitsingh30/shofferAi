/**
 * Buy graphic tees, oversized t-shirts, and casual wear on Bewakoof — browse designs, select size, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for bewakoof-tshirts.
 * Site: https://www.bewakoof.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bewakoof Tshirts ──────────────────────────
    log({ step: 'Opening Bewakoof Tshirts...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bewakoof Tshirts. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bewakoof Tshirts',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bewakoof Tshirts order',
      details: 'Completing your order on Bewakoof Tshirts',
      amountInr: 0, // Extracted from page at runtime
      description: 'bewakoof-tshirts order',
    });
    ...
    log({ step: 'Bewakoof Tshirts workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bewakoof Tshirts. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bewakoof-tshirts-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bewakoof-tshirts';
