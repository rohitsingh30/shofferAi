/**
 * Create and publish a LinkedIn post — write content, add images or documents, set audience, and publish.
 *
 * Auto-generated Playwright script for linkedin-post.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Linkedin Post...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Linkedin Post. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Linkedin Post',
    ...
    log({ step: 'Linkedin Post workflow completed', status: 'completed' });
    log({ message: 'Task completed on Linkedin Post. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'linkedin-post-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'linkedin-post';
