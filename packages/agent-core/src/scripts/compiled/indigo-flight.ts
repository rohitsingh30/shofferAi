/**
 * Book IndiGo flights on goindigo.in — search routes, select flights, choose fares, fill details, pay.
 *
 * Auto-generated Playwright script for indigo-flight.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Indigo Flight ──────────────────────────
    log({ step: 'Opening Indigo Flight...', status: 'running' });
    ...
    const profileEl = page.locator('[aria-label*="profile" i]').first();
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Indigo Flight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Indigo Flight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Indigo Flight order',
      details: 'Completing your order on Indigo Flight',
      amountInr: 0, // Extracted from page at runtime
      description: 'indigo-flight order',
    });
    ...
    log({ step: 'Indigo Flight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Indigo Flight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'indigo-flight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'indigo-flight';
