/**
 * Recharge FASTag on PhonePe — enter vehicle number, select amount, pay highway toll charges.
 *
 * Auto-generated Playwright script for phonepe-fastag.
 * Site: https://www.phonepe.com/fastag
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Phonepe Fastag ──────────────────────────
    log({ step: 'Opening Phonepe Fastag...', status: 'running' });
    await page.goto('https://www.phonepe.com/fastag');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Phonepe Fastag. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Phonepe Fastag',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Phonepe Fastag order',
      details: 'Completing your order on Phonepe Fastag',
      amountInr: 0, // Extracted from page at runtime
      description: 'phonepe-fastag order',
    });
    ...
    log({ step: 'Phonepe Fastag workflow completed', status: 'completed' });
    log({ message: 'Task completed on Phonepe Fastag. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'phonepe-fastag-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'phonepe-fastag';
