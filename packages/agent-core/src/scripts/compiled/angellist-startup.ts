/**
 * Find and apply for startup jobs on AngelList (Wellfound) — equity-based roles, remote-first startups, engineering, product, and design positions.
 *
 * Auto-generated Playwright script for angellist-startup.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Angellist Startup ──────────────────────────
    log({ step: 'Opening Angellist Startup...', status: 'running' });
    await page.goto('https://wellfound.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Angellist Startup. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Angellist Startup',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Angellist Startup order',
      details: 'Completing your order on Angellist Startup',
      amountInr: 0, // Extracted from page at runtime
      description: 'angellist-startup order',
    });
    ...
    log({ step: 'Angellist Startup workflow completed', status: 'completed' });
    log({ message: 'Task completed on Angellist Startup. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'angellist-startup-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'angellist-startup';
