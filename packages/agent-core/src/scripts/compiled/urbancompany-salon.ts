/**
 * Book women's salon at home on Urban Company. Facial, waxing, manicure, pedicure, haircut, bridal makeup.
 *
 * Auto-generated Playwright script for urbancompany-salon.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbancompany Salon ──────────────────────────
    log({ step: 'Opening Urbancompany Salon...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbancompany Salon. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbancompany Salon',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbancompany Salon order',
      details: 'Completing your order on Urbancompany Salon',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbancompany-salon order',
    });
    ...
    log({ step: 'Urbancompany Salon workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbancompany Salon. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbancompany-salon-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbancompany-salon';
