/**
 * Book budget hotels and rooms on OYO — search, compare, book and pay.
 *
 * Auto-generated Playwright script for oyo-hotel.
 * Site: https://www.oyorooms.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Oyo Hotel ──────────────────────────
    log({ step: 'Opening Oyo Hotel...', status: 'running' });
    await page.goto('https://www.oyorooms.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Oyo Hotel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Oyo Hotel order',
      details: 'Completing your order on Oyo Hotel',
      amountInr: 0, // Extracted from page at runtime
      description: 'oyo-hotel order',
    });
    ...
    log({ step: 'Oyo Hotel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Oyo Hotel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'oyo-hotel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'oyo-hotel';
