/**
 * Create a Zoom meeting — set topic, date, time, duration, get shareable meeting link and password.
 *
 * Auto-generated Playwright script for zoom-meeting.
 * Site: https://zoom.us
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Zoom Meeting...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zoom Meeting. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zoom Meeting',
    ...
    log({ step: 'Zoom Meeting workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zoom Meeting. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'zoom-meeting-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'zoom-meeting';
