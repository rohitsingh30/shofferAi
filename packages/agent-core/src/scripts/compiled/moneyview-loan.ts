/**
 * Get instant personal loan on MoneyView — quick approval, minimal documents, disbursal in minutes.
 *
 * Auto-generated Playwright script for moneyview-loan.
 * Site: https://www.moneyview.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Moneyview Loan ──────────────────────────
    log({ step: 'Opening Moneyview Loan...', status: 'running' });
    await page.goto('https://www.moneyview.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Moneyview Loan. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Moneyview Loan',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Moneyview Loan order',
      details: 'Completing your order on Moneyview Loan',
      amountInr: 0, // Extracted from page at runtime
      description: 'moneyview-loan order',
    });
    ...
    log({ step: 'Moneyview Loan workflow completed', status: 'completed' });
    log({ message: 'Task completed on Moneyview Loan. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'moneyview-loan-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'moneyview-loan';
