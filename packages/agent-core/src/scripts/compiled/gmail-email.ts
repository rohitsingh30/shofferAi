/**
 * Compose and send an email via Gmail — write subject, body, add attachments, CC/BCC, and send.
 *
 * Auto-generated Playwright script for gmail-email.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Gmail Email ──────────────────────────
    log({ step: 'Opening Gmail Email...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Gmail Email. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Gmail Email',
    ...
    log({ step: 'Gmail Email workflow completed', status: 'completed' });
    log({ message: 'Task completed on Gmail Email. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'gmail-email-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'gmail-email';
