/**
 * Shop on Nike India — browse, select size, buy shoes, apparel, and accessories, checkout, pay.
 *
 * Auto-generated Playwright script for nike-shoes.
 * Site: https://www.nike.com/in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Nike Shoes ──────────────────────────
    log({ step: 'Opening Nike Shoes...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Nike Shoes. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Nike Shoes',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Nike Shoes order',
      details: 'Completing your order on Nike Shoes',
      amountInr: 0, // Extracted from page at runtime
      description: 'nike-shoes order',
    });
    ...
    log({ step: 'Nike Shoes workflow completed', status: 'completed' });
    log({ message: 'Task completed on Nike Shoes. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'nike-shoes-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'nike-shoes';
