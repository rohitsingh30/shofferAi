/**
 * Order donuts and beverages from Mad Over Donuts — classic donuts, premium donuts, boxes, combos, checkout, pay.
 *
 * Auto-generated Playwright script for mad-over-donuts.
 * Site: https://www.madoverdonuts.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Mad Over Donuts ──────────────────────────
    log({ step: 'Opening Mad Over Donuts...', status: 'running' });
    await page.goto('https://www.madoverdonuts.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Mad Over Donuts. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Mad Over Donuts',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Mad Over Donuts order',
      details: 'Completing your order on Mad Over Donuts',
    ...
    log({ step: 'Mad Over Donuts workflow completed', status: 'completed' });
    log({ message: 'Task completed on Mad Over Donuts. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'mad-over-donuts-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'mad-over-donuts';
