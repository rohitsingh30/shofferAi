 ...
 *
 * Auto-generated Playwright script for bigbasket-grocery.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bigbasket Grocery ──────────────────────────
    log({ step: 'Opening Bigbasket Grocery...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bigbasket Grocery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bigbasket Grocery',
    ...
    const payResp = await requestFromHost({
      ...
      details: 'Completing your order on Bigbasket Grocery',
    ...
    log({ step: 'Bigbasket Grocery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bigbasket Grocery. Page: ' + finalTitle });
