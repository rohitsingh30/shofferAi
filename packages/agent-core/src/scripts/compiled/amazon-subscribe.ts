/**
 * Set up Amazon Subscribe & Save — recurring delivery of groceries, essentials, baby care, pet supplies at discounted prices.
 *
 * Auto-generated Playwright script for amazon-subscribe.
 * Site: https://www.amazon.in/gp/subscribe-and-save/manager
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Amazon Subscribe ──────────────────────────
    log({ step: 'Opening Amazon Subscribe...', status: 'running' });
    await page.goto('https://www.amazon.in/gp/subscribe-and-save/manager');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Amazon Subscribe order',
      details: 'Completing your order on Amazon Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'amazon-subscribe order',
    });
    ...
    log({ step: 'Amazon Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-subscribe';
