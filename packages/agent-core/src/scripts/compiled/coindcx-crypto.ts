/**
 * Trade crypto on CoinDCX — buy Bitcoin, Ethereum, or altcoins with INR, set limit orders, track portfolio.
 *
 * Auto-generated Playwright script for coindcx-crypto.
 * Site: https://coindcx.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Coindcx Crypto ──────────────────────────
    log({ step: 'Opening Coindcx Crypto...', status: 'running' });
    await page.goto('https://coindcx.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Coindcx Crypto. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Coindcx Crypto',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Coindcx Crypto order',
      details: 'Completing your order on Coindcx Crypto',
      amountInr: 0, // Extracted from page at runtime
      description: 'coindcx-crypto order',
    });
    ...
    log({ step: 'Coindcx Crypto workflow completed', status: 'completed' });
    log({ message: 'Task completed on Coindcx Crypto. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'coindcx-crypto-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'coindcx-crypto';
