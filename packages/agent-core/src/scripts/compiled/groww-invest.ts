/**
 * Invest in mutual funds or stocks on Groww — search funds, start SIP, buy stocks, track portfolio.
 *
 * Auto-generated Playwright script for groww-invest.
 * Site: https://groww.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Groww Invest ──────────────────────────
    log({ step: 'Opening Groww Invest...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Groww Invest. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Groww Invest',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Groww Invest order',
      details: 'Completing your order on Groww Invest',
      amountInr: 0, // Extracted from page at runtime
      description: 'groww-invest order',
    });
    ...
    log({ step: 'Groww Invest workflow completed', status: 'completed' });
    log({ message: 'Task completed on Groww Invest. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'groww-invest-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'groww-invest';
