/**
 * Create custom merchandise on Printful — t-shirts, hoodies, hats, print-on-demand products with your design.
 *
 * Auto-generated Playwright script for printful-merch.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Printful Merch ──────────────────────────
    log({ step: 'Opening Printful Merch...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Printful Merch. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Printful Merch',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Printful Merch order',
      details: 'Completing your order on Printful Merch',
      amountInr: 0, // Extracted from page at runtime
      description: 'printful-merch order',
    });
    ...
    log({ step: 'Printful Merch workflow completed', status: 'completed' });
    log({ message: 'Task completed on Printful Merch. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'printful-merch-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'printful-merch';
