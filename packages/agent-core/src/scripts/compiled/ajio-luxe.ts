/**
 * Shop premium fashion on AJIO Luxe — browse designer brands, luxury clothing, shoes, accessories, add to bag, checkout, pay.
 *
 * Auto-generated Playwright script for ajio-luxe.
 * Site: https://www.ajio.com/luxe
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ajio Luxe ──────────────────────────
    log({ step: 'Opening Ajio Luxe...', status: 'running' });
    await page.goto('https://www.ajio.com/luxe');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ajio Luxe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ajio Luxe order',
      details: 'Completing your order on Ajio Luxe',
      amountInr: 0, // Extracted from page at runtime
      description: 'ajio-luxe order',
    });
    ...
    log({ step: 'Ajio Luxe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ajio Luxe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ajio-luxe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'ajio-luxe';
