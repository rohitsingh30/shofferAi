/**
 * Book doctor appointments on Practo — find doctors, check availability, book consultation.
 *
 * Auto-generated Playwright script for practo-doctor.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Practo Doctor ──────────────────────────
    log({ step: 'Opening Practo Doctor...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Practo Doctor. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Practo Doctor',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Practo Doctor order',
      details: 'Completing your order on Practo Doctor',
      amountInr: 0, // Extracted from page at runtime
      description: 'practo-doctor order',
    });
    ...
    log({ step: 'Practo Doctor workflow completed', status: 'completed' });
    log({ message: 'Task completed on Practo Doctor. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'practo-doctor-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'practo-doctor';
