/**
 * Shop on IKEA India — browse furniture, home decor, kitchenware, check delivery, assembly options, checkout, pay.
 *
 * Auto-generated Playwright script for ikea-furniture.
 * Site: https://www.ikea.com/in/en/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ikea Furniture ──────────────────────────
    log({ step: 'Opening Ikea Furniture...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ikea Furniture. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Ikea Furniture',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ikea Furniture order',
      details: 'Completing your order on Ikea Furniture',
      amountInr: 0, // Extracted from page at runtime
      description: 'ikea-furniture order',
    });
    ...
    log({ step: 'Ikea Furniture workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ikea Furniture. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ikea-furniture-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'ikea-furniture';
