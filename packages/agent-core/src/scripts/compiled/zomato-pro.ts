/**
 * Subscribe to Zomato Gold/Pro membership — free delivery, extra discounts, priority service.
 *
 * Auto-generated Playwright script for zomato-pro.
 * Site: https://www.zomato.com/pro
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Zomato Pro ──────────────────────────
    log({ step: 'Opening Zomato Pro...', status: 'running' });
    await page.goto('https://www.zomato.com/pro');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zomato Pro. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zomato Pro',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Zomato Pro order',
      details: 'Completing your order on Zomato Pro',
      amountInr: 0, // Extracted from page at runtime
      description: 'zomato-pro order',
    });
    ...
    log({ step: 'Zomato Pro workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zomato Pro. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'zomato-pro-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'zomato-pro';
