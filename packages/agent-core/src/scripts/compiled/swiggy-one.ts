/**
 * Subscribe to Swiggy One membership — unlimited free delivery, extra discounts on food and Instamart.
 *
 * Auto-generated Playwright script for swiggy-one.
 * Site: https://www.swiggy.com/one
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Swiggy One ──────────────────────────
    log({ step: 'Opening Swiggy One...', status: 'running' });
    await page.goto('https://www.swiggy.com/one');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Swiggy One. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Swiggy One',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Swiggy One order',
      details: 'Completing your order on Swiggy One',
      amountInr: 0, // Extracted from page at runtime
      description: 'swiggy-one order',
    });
    ...
    log({ step: 'Swiggy One workflow completed', status: 'completed' });
    log({ message: 'Task completed on Swiggy One. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'swiggy-one-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'swiggy-one';
