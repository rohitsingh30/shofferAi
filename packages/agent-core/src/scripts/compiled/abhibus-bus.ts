/**
 * Book bus tickets on AbhiBus — search routes, compare operators, select seats, fill details, pay.
 *
 * Auto-generated Playwright script for abhibus-bus.
 * Site: https://www.abhibus.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Abhibus Bus ──────────────────────────
    log({ step: 'Opening Abhibus Bus...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Abhibus Bus. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Abhibus Bus',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Abhibus Bus order',
      details: 'Completing your order on Abhibus Bus',
      amountInr: 0, // Extracted from page at runtime
      description: 'abhibus-bus order',
    });
    ...
    log({ step: 'Abhibus Bus workflow completed', status: 'completed' });
    log({ message: 'Task completed on Abhibus Bus. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'abhibus-bus-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'abhibus-bus';
