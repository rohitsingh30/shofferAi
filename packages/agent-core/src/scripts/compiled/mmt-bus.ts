/**
 * Book bus tickets on MakeMyTrip — search routes, filter by operator/type, select seats, fill details, pay.
 *
 * Auto-generated Playwright script for mmt-bus.
 * Site: https://www.makemytrip.com/bus-tickets
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Mmt Bus ──────────────────────────
    log({ step: 'Opening Mmt Bus...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Mmt Bus. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Mmt Bus order',
      details: 'Completing your order on Mmt Bus',
      amountInr: 0, // Extracted from page at runtime
      description: 'mmt-bus order',
    });
    ...
    log({ step: 'Mmt Bus workflow completed', status: 'completed' });
    log({ message: 'Task completed on Mmt Bus. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'mmt-bus-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
