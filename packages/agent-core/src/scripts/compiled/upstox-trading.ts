/**
 * Trade stocks and F&O on Upstox — search stocks, place orders, manage portfolio, check margins.
 *
 * Auto-generated Playwright script for upstox-trading.
 * Site: https://upstox.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Upstox Trading ──────────────────────────
    log({ step: 'Opening Upstox Trading...', status: 'running' });
    await page.goto('https://upstox.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Upstox Trading. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Upstox Trading',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Upstox Trading order',
      details: 'Completing your order on Upstox Trading',
      amountInr: 0, // Extracted from page at runtime
      description: 'upstox-trading order',
    });
    ...
    log({ step: 'Upstox Trading workflow completed', status: 'completed' });
    log({ message: 'Task completed on Upstox Trading. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'upstox-trading-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'upstox-trading';
