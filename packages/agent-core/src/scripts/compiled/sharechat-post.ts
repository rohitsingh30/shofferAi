/**
 * Create and publish posts on ShareChat — India's regional language social media platform. Text, image, video posts.
 *
 * Auto-generated Playwright script for sharechat-post.
 * Site: https://sharechat.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Sharechat Post ──────────────────────────
    log({ step: 'Opening Sharechat Post...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Sharechat Post. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Sharechat Post',
    ...
    log({ step: 'Sharechat Post workflow completed', status: 'completed' });
    log({ message: 'Task completed on Sharechat Post. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'sharechat-post-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'sharechat-post';
