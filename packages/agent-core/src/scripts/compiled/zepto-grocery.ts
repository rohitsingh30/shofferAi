/**
 * Order groceries from Zepto with 10-15 minute delivery.
 *
 * Auto-generated Playwright script for zepto-grocery.
 * Site: https://www.zeptonow.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Zepto Grocery ──────────────────────────
    log({ step: 'Opening Zepto Grocery...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zepto Grocery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zepto Grocery',
    ...
    const payResp = await requestFromHost({
      ...
      details: 'Completing your order on Zepto Grocery',
    ...
    log({ step: 'Zepto Grocery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zepto Grocery. Page: ' + finalTitle });
