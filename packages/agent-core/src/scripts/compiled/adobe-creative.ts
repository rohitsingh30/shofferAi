/**
 * Subscribe to Adobe Creative Cloud India. Photography plan, Single App, or All Apps — Photoshop, Illustrator, Premiere Pro, and more.
 *
 * Auto-generated Playwright script for adobe-creative.
 * Site: https://www.adobe.com/in/creativecloud/plans.html
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Adobe Creative ──────────────────────────
    log({ step: 'Opening Adobe Creative...', status: 'running' });
    await page.goto('https://www.adobe.com/in/creativecloud/plans.html');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Adobe Creative. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Adobe Creative',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Adobe Creative order',
      details: 'Completing your order on Adobe Creative',
      amountInr: 0, // Extracted from page at runtime
      description: 'adobe-creative order',
    });
    ...
    log({ step: 'Adobe Creative workflow completed', status: 'completed' });
    log({ message: 'Task completed on Adobe Creative. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'adobe-creative-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'adobe-creative';
