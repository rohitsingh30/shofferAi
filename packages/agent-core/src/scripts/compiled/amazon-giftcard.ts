/**
 * Buy Amazon gift cards — select amount, recipient email, add message, pay and deliver instantly.
 *
 * Auto-generated Playwright script for amazon-giftcard.
 * Site: https://www.amazon.in/gift-cards
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Amazon Giftcard ──────────────────────────
    log({ step: 'Opening Amazon Giftcard...', status: 'running' });
    await page.goto('https://www.amazon.in/gift-cards');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Giftcard. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Giftcard',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Amazon Giftcard order',
      details: 'Completing your order on Amazon Giftcard',
      amountInr: 0, // Extracted from page at runtime
      description: 'amazon-giftcard order',
    });
    ...
    log({ step: 'Amazon Giftcard workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Giftcard. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-giftcard-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-giftcard';
