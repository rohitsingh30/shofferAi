/**
 * Book hotels on Agoda — search worldwide destinations, compare deals, book rooms, pay.
 *
 * Auto-generated Playwright script for agoda-hotel.
 * Site: https://www.agoda.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Agoda Hotel ──────────────────────────
    log({ step: 'Opening Agoda Hotel...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Agoda Hotel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Agoda Hotel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Agoda Hotel order',
      details: 'Completing your order on Agoda Hotel',
      amountInr: 0, // Extracted from page at runtime
      description: 'agoda-hotel order',
    });
    ...
    log({ step: 'Agoda Hotel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Agoda Hotel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'agoda-hotel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'agoda-hotel';
