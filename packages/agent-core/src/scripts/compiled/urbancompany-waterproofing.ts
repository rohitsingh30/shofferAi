/**
 * Book waterproofing service on Urban Company — terrace, bathroom, external wall, balcony waterproofing with warranty.
 *
 * Auto-generated Playwright script for urbancompany-waterproofing.
 * Site: https://www.urbancompany.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbancompany Waterproofing ──────────────────────────
    log({ step: 'Opening Urbancompany Waterproofing...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbancompany Waterproofing. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbancompany Waterproofing',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbancompany Waterproofing order',
      details: 'Completing your order on Urbancompany Waterproofing',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbancompany-waterproofing order',
    });
    ...
    log({ step: 'Urbancompany Waterproofing workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbancompany Waterproofing. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbancompany-waterproofing-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbancompany-waterproofing';
