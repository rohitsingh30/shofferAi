/**
 * Shop on FabIndia — buy ethnic wear, kurtas, sarees, home decor, organic food, browse collections, checkout, pay.
 *
 * Auto-generated Playwright script for fabindia-ethnic.
 * Site: https://www.fabindia.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Fabindia Ethnic ──────────────────────────
    log({ step: 'Opening Fabindia Ethnic...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Fabindia Ethnic. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Fabindia Ethnic',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Fabindia Ethnic order',
      details: 'Completing your order on Fabindia Ethnic',
      amountInr: 0, // Extracted from page at runtime
      description: 'fabindia-ethnic order',
    });
    ...
    log({ step: 'Fabindia Ethnic workflow completed', status: 'completed' });
    log({ message: 'Task completed on Fabindia Ethnic. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'fabindia-ethnic-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'fabindia-ethnic';
