/**
 * Order Natural's ice cream — sitaphal, tender coconut, mango, seasonal flavors, packs, delivery, pay.
 *
 * Auto-generated Playwright script for naturals-icecream.
 * Site: https://www.naturalsicecream.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Naturals Icecream ──────────────────────────
    log({ step: 'Opening Naturals Icecream...', status: 'running' });
    await page.goto('https://www.naturalsicecream.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Naturals Icecream. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Naturals Icecream',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Naturals Icecream order',
      details: 'Completing your order on Naturals Icecream',
      amountInr: 0, // Extracted from page at runtime
      description: 'naturals-icecream order',
    });
    ...
    log({ step: 'Naturals Icecream workflow completed', status: 'completed' });
    log({ message: 'Task completed on Naturals Icecream. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'naturals-icecream-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'naturals-icecream';
