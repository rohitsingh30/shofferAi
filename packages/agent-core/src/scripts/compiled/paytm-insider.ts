/**
 * Book entertainment events on Paytm Insider — comedy shows, music concerts, food festivals, nightlife. Browse, select, pay.
 *
 * Auto-generated Playwright script for paytm-insider.
 * Site: https://insider.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Insider ──────────────────────────
    log({ step: 'Opening Paytm Insider...', status: 'running' });
    await page.goto('https://insider.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Insider. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Paytm Insider',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Insider order',
      details: 'Completing your order on Paytm Insider',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-insider order',
    });
    ...
    log({ step: 'Paytm Insider workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Insider. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-insider-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-insider';
