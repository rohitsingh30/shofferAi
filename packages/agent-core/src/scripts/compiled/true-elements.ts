/**
 * Buy healthy snacks, granola, seeds, muesli, and superfoods from True Elements — browse, add to cart, order online.
 *
 * Auto-generated Playwright script for true-elements.
 * Site: https://www.trueelements.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to True Elements ──────────────────────────
    log({ step: 'Opening True Elements...', status: 'running' });
    await page.goto('https://www.trueelements.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on True Elements. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with True Elements',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete True Elements order',
      details: 'Completing your order on True Elements',
    ...
    log({ step: 'True Elements workflow completed', status: 'completed' });
    log({ message: 'Task completed on True Elements. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'true-elements-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'true-elements';
