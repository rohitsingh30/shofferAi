/**
 * Shop on Nykaa — search beauty, skincare, cosmetics, haircare products, add to bag, checkout, pay.
 *
 * Auto-generated Playwright script for nykaa-beauty.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Nykaa Beauty ──────────────────────────
    log({ step: 'Opening Nykaa Beauty...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Nykaa Beauty. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Nykaa Beauty',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Nykaa Beauty order',
      details: 'Completing your order on Nykaa Beauty',
    ...
    log({ step: 'Nykaa Beauty workflow completed', status: 'completed' });
    log({ message: 'Task completed on Nykaa Beauty. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'nykaa-beauty-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'nykaa-beauty';
