/**
 * Buy pantry and household items on Amazon Pantry — search, add to cart, checkout with scheduled delivery.
 *
 * Auto-generated Playwright script for amazon-pantry.
 * Site: https://www.amazon.in/pantry
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Amazon Pantry ──────────────────────────
    log({ step: 'Opening Amazon Pantry...', status: 'running' });
    await page.goto('https://www.amazon.in/pantry');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Pantry. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Pantry',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Amazon Pantry order',
      details: 'Completing your order on Amazon Pantry',
    ...
    log({ step: 'Amazon Pantry workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Pantry. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-pantry-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-pantry';
