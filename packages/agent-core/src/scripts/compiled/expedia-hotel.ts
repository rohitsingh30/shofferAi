/**
 * Book international hotels on Expedia India — search destinations worldwide, compare, book, pay.
 *
 * Auto-generated Playwright script for expedia-hotel.
 * Site: https://www.expedia.co.in/Hotels
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Expedia Hotel ──────────────────────────
    log({ step: 'Opening Expedia Hotel...', status: 'running' });
    await page.goto('https://www.expedia.co.in/Hotels');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Expedia Hotel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Expedia Hotel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Expedia Hotel order',
      details: 'Completing your order on Expedia Hotel',
      amountInr: 0, // Extracted from page at runtime
      description: 'expedia-hotel order',
    });
    ...
    log({ step: 'Expedia Hotel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Expedia Hotel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'expedia-hotel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'expedia-hotel';
