/**
 * Search rental properties on CommonFloor — PG, flats, houses with locality-based search and filters.
 *
 * Auto-generated Playwright script for commonfloor-rental.
 * Site: https://www.commonfloor.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Commonfloor Rental ──────────────────────────
    log({ step: 'Opening Commonfloor Rental...', status: 'running' });
    await page.goto('https://www.commonfloor.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Commonfloor Rental. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Commonfloor Rental',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Commonfloor Rental order',
      details: 'Completing your order on Commonfloor Rental',
      amountInr: 0, // Extracted from page at runtime
      description: 'commonfloor-rental order',
    });
    ...
    log({ step: 'Commonfloor Rental workflow completed', status: 'completed' });
    log({ message: 'Task completed on Commonfloor Rental. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'commonfloor-rental-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'commonfloor-rental';
