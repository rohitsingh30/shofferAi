/**
 * Search and shortlist properties on Housing.com — buy or rent flats, houses, plots with filters.
 *
 * Auto-generated Playwright script for housing-property.
 * Site: https://housing.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Housing Property ──────────────────────────
    log({ step: 'Opening Housing Property...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Housing Property. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Housing Property',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Housing Property order',
      details: 'Completing your order on Housing Property',
      amountInr: 0, // Extracted from page at runtime
      description: 'housing-property order',
    });
    ...
    log({ step: 'Housing Property workflow completed', status: 'completed' });
    log({ message: 'Task completed on Housing Property. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'housing-property-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'housing-property';
