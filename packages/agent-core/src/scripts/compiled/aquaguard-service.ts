/**
 * Book Aquaguard/Eureka Forbes water purifier service — AMC, filter change, repair, installation, annual maintenance.
 *
 * Auto-generated Playwright script for aquaguard-service.
 * Site: https://www.eurekaforbes.com/service
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Aquaguard Service ──────────────────────────
    log({ step: 'Opening Aquaguard Service...', status: 'running' });
    await page.goto('https://www.eurekaforbes.com/service');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Aquaguard Service. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Aquaguard Service',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Aquaguard Service order',
      details: 'Completing your order on Aquaguard Service',
      amountInr: 0, // Extracted from page at runtime
      description: 'aquaguard-service order',
    });
    ...
    log({ step: 'Aquaguard Service workflow completed', status: 'completed' });
    log({ message: 'Task completed on Aquaguard Service. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'aquaguard-service-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'aquaguard-service';
