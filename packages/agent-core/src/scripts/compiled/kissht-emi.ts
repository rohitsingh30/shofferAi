/**
 * Convert purchases to EMI on KissHT — no-cost EMI, buy now pay later, instant credit line, cardless EMI.
 *
 * Auto-generated Playwright script for kissht-emi.
 * Site: https://www.kissht.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Kissht Emi ──────────────────────────
    log({ step: 'Opening Kissht Emi...', status: 'running' });
    await page.goto('https://www.kissht.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Kissht Emi. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Kissht Emi',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Kissht Emi order',
      details: 'Completing your order on Kissht Emi',
      amountInr: 0, // Extracted from page at runtime
      description: 'kissht-emi order',
    });
    ...
    log({ step: 'Kissht Emi workflow completed', status: 'completed' });
    log({ message: 'Task completed on Kissht Emi. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'kissht-emi-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'kissht-emi';
