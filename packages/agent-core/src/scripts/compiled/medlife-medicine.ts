/**
 * Order medicines on Medlife — search by name, upload prescription, add to cart, checkout with delivery.
 *
 * Auto-generated Playwright script for medlife-medicine.
 * Site: https://www.medlife.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Medlife Medicine ──────────────────────────
    log({ step: 'Opening Medlife Medicine...', status: 'running' });
    await page.goto('https://www.medlife.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Medlife Medicine. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Medlife Medicine',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Medlife Medicine order',
      details: 'Completing your order on Medlife Medicine',
    ...
    log({ step: 'Medlife Medicine workflow completed', status: 'completed' });
    log({ message: 'Task completed on Medlife Medicine. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'medlife-medicine-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'medlife-medicine';
