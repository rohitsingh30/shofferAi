/**
 * Order from Popeyes India — Louisiana-style chicken, biscuits, sides, combos, checkout, pay.
 *
 * Auto-generated Playwright script for popeyes-order.
 * Site: https://www.popeyes.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Popeyes Order ──────────────────────────
    log({ step: 'Opening Popeyes Order...', status: 'running' });
    await page.goto('https://www.popeyes.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Popeyes Order. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Popeyes Order',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Popeyes Order order',
      details: 'Completing your order on Popeyes Order',
      amountInr: 0, // Extracted from page at runtime
      description: 'popeyes-order order',
    });
    ...
    log({ step: 'Popeyes Order workflow completed', status: 'completed' });
    log({ message: 'Task completed on Popeyes Order. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'popeyes-order-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'popeyes-order';
