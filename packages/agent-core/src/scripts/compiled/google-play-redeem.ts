/**
 * Redeem a Google Play gift card or promo code. Enter the code, add balance to the Google account, and verify.
 *
 * Auto-generated Playwright script for google-play-redeem.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Google Play Redeem ──────────────────────────
    log({ step: 'Opening Google Play Redeem...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Google Play Redeem. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Google Play Redeem',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Google Play Redeem order',
      details: 'Completing your order on Google Play Redeem',
      amountInr: 0, // Extracted from page at runtime
      description: 'google-play-redeem order',
    });
    ...
    log({ step: 'Google Play Redeem workflow completed', status: 'completed' });
    log({ message: 'Task completed on Google Play Redeem. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'google-play-redeem-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'google-play-redeem';
