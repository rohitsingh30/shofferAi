/**
 * Shop casual fashion on US Polo Assn India — browse polos, t-shirts, jeans, casual shirts, sneakers, accessories, checkout, pay.
 *
 * Auto-generated Playwright script for us-polo.
 * Site: https://www.uspoloassn.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Us Polo ──────────────────────────
    log({ step: 'Opening Us Polo...', status: 'running' });
    await page.goto('https://www.uspoloassn.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Us Polo. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Us Polo order',
      details: 'Completing your order on Us Polo',
      amountInr: 0, // Extracted from page at runtime
      description: 'us-polo order',
    });
    ...
    log({ step: 'Us Polo workflow completed', status: 'completed' });
    log({ message: 'Task completed on Us Polo. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'us-polo-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
