/**
 * Pay credit card bills on CRED — check due amount, pay bills, earn rewards.
 *
 * Auto-generated Playwright script for cred-billpay.
 * Site: https://web.cred.club
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cred Billpay ──────────────────────────
    log({ step: 'Opening Cred Billpay...', status: 'running' });
    await page.goto('https://web.cred.club');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cred Billpay. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cred Billpay',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cred Billpay order',
      details: 'Completing your order on Cred Billpay',
      amountInr: 0, // Extracted from page at runtime
      description: 'cred-billpay order',
    });
    ...
    log({ step: 'Cred Billpay workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cred Billpay. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cred-billpay-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cred-billpay';
