/**
 * Subscribe to Extramarks e-learning platform — K-12, select class, subjects, plan, subscribe, pay.
 *
 * Auto-generated Playwright script for extramarks-learning.
 * Site: https://www.extramarks.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Extramarks Learning ──────────────────────────
    log({ step: 'Opening Extramarks Learning...', status: 'running' });
    await page.goto('https://www.extramarks.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Extramarks Learning. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Extramarks Learning',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Extramarks Learning order',
      details: 'Completing your order on Extramarks Learning',
      amountInr: 0, // Extracted from page at runtime
      description: 'extramarks-learning order',
    });
    ...
    log({ step: 'Extramarks Learning workflow completed', status: 'completed' });
    log({ message: 'Task completed on Extramarks Learning. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'extramarks-learning-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'extramarks-learning';
