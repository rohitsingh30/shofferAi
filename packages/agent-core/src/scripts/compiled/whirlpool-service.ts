/**
 * Book Whirlpool appliance service or repair — AC, washing machine, refrigerator, microwave, water purifier repair.
 *
 * Auto-generated Playwright script for whirlpool-service.
 * Site: https://www.whirlpoolindia.com/support
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Whirlpool Service ──────────────────────────
    log({ step: 'Opening Whirlpool Service...', status: 'running' });
    await page.goto('https://www.whirlpoolindia.com/support');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Whirlpool Service. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Whirlpool Service',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Whirlpool Service order',
      details: 'Completing your order on Whirlpool Service',
      amountInr: 0, // Extracted from page at runtime
      description: 'whirlpool-service order',
    });
    ...
    log({ step: 'Whirlpool Service workflow completed', status: 'completed' });
    log({ message: 'Task completed on Whirlpool Service. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'whirlpool-service-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'whirlpool-service';
