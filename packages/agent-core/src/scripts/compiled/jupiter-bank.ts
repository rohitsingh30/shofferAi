/**
 * Banking on Jupiter — check account balance, UPI payments, manage rewards, savings pots, track spending.
 *
 * Auto-generated Playwright script for jupiter-bank.
 * Site: https://jupiter.money
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Jupiter Bank ──────────────────────────
    log({ step: 'Opening Jupiter Bank...', status: 'running' });
    await page.goto('https://jupiter.money');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Jupiter Bank. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Jupiter Bank',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Jupiter Bank order',
      details: 'Completing your order on Jupiter Bank',
      amountInr: 0, // Extracted from page at runtime
      description: 'jupiter-bank order',
    });
    ...
    log({ step: 'Jupiter Bank workflow completed', status: 'completed' });
    log({ message: 'Task completed on Jupiter Bank. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'jupiter-bank-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'jupiter-bank';
