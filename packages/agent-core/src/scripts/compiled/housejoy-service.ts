/**
 * Book home services on Housejoy — plumber, electrician, cleaning, painting, pest control, carpentry.
 *
 * Auto-generated Playwright script for housejoy-service.
 * Site: https://www.housejoy.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Housejoy Service ──────────────────────────
    log({ step: 'Opening Housejoy Service...', status: 'running' });
    await page.goto('https://www.housejoy.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Housejoy Service. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Housejoy Service',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Housejoy Service order',
      details: 'Completing your order on Housejoy Service',
      amountInr: 0, // Extracted from page at runtime
      description: 'housejoy-service order',
    });
    ...
    log({ step: 'Housejoy Service workflow completed', status: 'completed' });
    log({ message: 'Task completed on Housejoy Service. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'housejoy-service-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'housejoy-service';
