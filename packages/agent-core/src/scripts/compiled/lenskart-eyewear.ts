/**
 * Shop on Lenskart — browse eyeglasses, sunglasses, contact lenses, enter prescription, try-on, checkout, pay.
 *
 * Auto-generated Playwright script for lenskart-eyewear.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Lenskart Eyewear ──────────────────────────
    log({ step: 'Opening Lenskart Eyewear...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Lenskart Eyewear. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Lenskart Eyewear',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Lenskart Eyewear order',
      details: 'Completing your order on Lenskart Eyewear',
    ...
    log({ step: 'Lenskart Eyewear workflow completed', status: 'completed' });
    log({ message: 'Task completed on Lenskart Eyewear. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'lenskart-eyewear-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'lenskart-eyewear';
