/**
 * Buy apps, games, or subscriptions on the Apple App Store via the web. Browse, purchase, and manage subscriptions.
 *
 * Auto-generated Playwright script for apple-appstore.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Apple Appstore ──────────────────────────
    log({ step: 'Opening Apple Appstore...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Apple Appstore. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Apple Appstore',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Apple Appstore order',
      details: 'Completing your order on Apple Appstore',
      amountInr: 0, // Extracted from page at runtime
      description: 'apple-appstore order',
    });
    ...
    log({ step: 'Apple Appstore workflow completed', status: 'completed' });
    log({ message: 'Task completed on Apple Appstore. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'apple-appstore-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'apple-appstore';
