/**
 * Shop men's formal and semi-formal wear on Peter England — browse formal shirts, trousers, suits, blazers, add to bag, checkout, pay.
 *
 * Auto-generated Playwright script for peter-england.
 * Site: https://www.peterengland.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Peter England ──────────────────────────
    log({ step: 'Opening Peter England...', status: 'running' });
    await page.goto('https://www.peterengland.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Peter England. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Peter England',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Peter England order',
      details: 'Completing your order on Peter England',
      amountInr: 0, // Extracted from page at runtime
      description: 'peter-england order',
    });
    ...
    log({ step: 'Peter England workflow completed', status: 'completed' });
    log({ message: 'Task completed on Peter England. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'peter-england-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'peter-england';
