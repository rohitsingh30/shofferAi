/**
 * Register a domain on Namecheap — search availability, compare prices across extensions, purchase and configure.
 *
 * Auto-generated Playwright script for namecheap-domain.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Namecheap Domain ──────────────────────────
    log({ step: 'Opening Namecheap Domain...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Namecheap Domain. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Namecheap Domain',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Namecheap Domain order',
      details: 'Completing your order on Namecheap Domain',
      amountInr: 0, // Extracted from page at runtime
      description: 'namecheap-domain order',
    });
    ...
    log({ step: 'Namecheap Domain workflow completed', status: 'completed' });
    log({ message: 'Task completed on Namecheap Domain. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'namecheap-domain-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'namecheap-domain';
