/**
 * Buy sauces, dips, spreads, and condiments from Wingreens Farms — browse catalog, add to cart, order online.
 *
 * Auto-generated Playwright script for wingreens-sauce.
 * Site: https://www.wingreens.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wingreens Sauce ──────────────────────────
    log({ step: 'Opening Wingreens Sauce...', status: 'running' });
    await page.goto('https://www.wingreens.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wingreens Sauce. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Wingreens Sauce',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wingreens Sauce order',
      details: 'Completing your order on Wingreens Sauce',
      amountInr: 0, // Extracted from page at runtime
      description: 'wingreens-sauce order',
    });
    ...
    log({ step: 'Wingreens Sauce workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wingreens Sauce. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wingreens-sauce-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wingreens-sauce';
