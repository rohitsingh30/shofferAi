/**
 * Buy an audiobook on Audible India — search, preview narration, purchase with credits or money.
 *
 * Auto-generated Playwright script for audible-audiobook.
 * Site: https://www.audible.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Audible Audiobook ──────────────────────────
    log({ step: 'Opening Audible Audiobook...', status: 'running' });
    await page.goto('https://www.audible.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Audible Audiobook. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Audible Audiobook',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Audible Audiobook order',
      details: 'Completing your order on Audible Audiobook',
      amountInr: 0, // Extracted from page at runtime
      description: 'audible-audiobook order',
    });
    ...
    log({ step: 'Audible Audiobook workflow completed', status: 'completed' });
    log({ message: 'Task completed on Audible Audiobook. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'audible-audiobook-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'audible-audiobook';
