/**
 * Apply for premium tech jobs on Hirist — curated software engineering, data, DevOps, and product roles at top companies.
 *
 * Auto-generated Playwright script for hirist-tech.
 * Site: https://www.hirist.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hirist Tech ──────────────────────────
    log({ step: 'Opening Hirist Tech...', status: 'running' });
    await page.goto('https://www.hirist.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hirist Tech. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hirist Tech',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hirist Tech order',
      details: 'Completing your order on Hirist Tech',
      amountInr: 0, // Extracted from page at runtime
      description: 'hirist-tech order',
    });
    ...
    log({ step: 'Hirist Tech workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hirist Tech. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hirist-tech-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hirist-tech';
