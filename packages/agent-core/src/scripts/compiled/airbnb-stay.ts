/**
 * Book homestay, apartment, or experience on Airbnb — search by location/dates/guests, filter, compare listings, book and pay.
 *
 * Auto-generated Playwright script for airbnb-stay.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Airbnb Stay ──────────────────────────
    log({ step: 'Opening Airbnb Stay...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Airbnb Stay. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Airbnb Stay',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Airbnb Stay order',
      details: 'Completing your order on Airbnb Stay',
      amountInr: 0, // Extracted from page at runtime
      description: 'airbnb-stay order',
    });
    ...
    log({ step: 'Airbnb Stay workflow completed', status: 'completed' });
    log({ message: 'Task completed on Airbnb Stay. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'airbnb-stay-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'airbnb-stay';
