/**
 * Book appointment at Fortis Hospital — find specialists, schedule consultation, book health checkup packages.
 *
 * Auto-generated Playwright script for fortis-hospital.
 * Site: https://www.fortishealthcare.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Fortis Hospital ──────────────────────────
    log({ step: 'Opening Fortis Hospital...', status: 'running' });
    await page.goto('https://www.fortishealthcare.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Fortis Hospital. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Fortis Hospital',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Fortis Hospital order',
      details: 'Completing your order on Fortis Hospital',
      amountInr: 0, // Extracted from page at runtime
      description: 'fortis-hospital order',
    });
    ...
    log({ step: 'Fortis Hospital workflow completed', status: 'completed' });
    log({ message: 'Task completed on Fortis Hospital. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'fortis-hospital-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'fortis-hospital';
