/**
 * Shop on Decathlon — buy sports equipment, clothing, shoes, accessories for any sport, checkout, pay.
 *
 * Auto-generated Playwright script for decathlon-sports.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Decathlon Sports ──────────────────────────
    log({ step: 'Opening Decathlon Sports...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Decathlon Sports. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Decathlon Sports',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Decathlon Sports order',
      details: 'Completing your order on Decathlon Sports',
    ...
    log({ step: 'Decathlon Sports workflow completed', status: 'completed' });
    log({ message: 'Task completed on Decathlon Sports. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'decathlon-sports-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'decathlon-sports';
