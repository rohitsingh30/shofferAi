/**
 * Apply for a loan on Bajaj Finserv — personal loan, home loan, business loan, loan against property, gold loan.
 *
 * Auto-generated Playwright script for bajaj-finserv-loan.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bajaj Finserv Loan ──────────────────────────
    log({ step: 'Opening Bajaj Finserv Loan...', status: 'running' });
    await page.goto('https://www.bajajfinserv.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bajaj Finserv Loan. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bajaj Finserv Loan',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bajaj Finserv Loan order',
      details: 'Completing your order on Bajaj Finserv Loan',
      amountInr: 0, // Extracted from page at runtime
      description: 'bajaj-finserv-loan order',
    });
    ...
    log({ step: 'Bajaj Finserv Loan workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bajaj Finserv Loan. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bajaj-finserv-loan-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bajaj-finserv-loan';
