/**
 * Shop on Flipkart — search products, compare options, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for flipkart-shopping.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Flipkart Shopping ──────────────────────────
    log({ step: 'Opening Flipkart Shopping...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Flipkart Shopping. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Flipkart Shopping',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Flipkart Shopping order',
      details: 'Completing your order on Flipkart Shopping',
    ...
    log({ step: 'Flipkart Shopping workflow completed', status: 'completed' });
    log({ message: 'Task completed on Flipkart Shopping. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'flipkart-shopping-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'flipkart-shopping';
