/**
 * Ship packages via Delhivery — pan-India courier service, book shipments, track consignments, express and standard delivery.
 *
 * Auto-generated Playwright script for delhivery-ship.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Delhivery Ship ──────────────────────────
    log({ step: 'Opening Delhivery Ship...', status: 'running' });
    await page.goto('https://www.delhivery.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Delhivery Ship. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Delhivery Ship',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Delhivery Ship order',
      details: 'Completing your order on Delhivery Ship',
      amountInr: 0, // Extracted from page at runtime
      description: 'delhivery-ship order',
    });
    ...
    log({ step: 'Delhivery Ship workflow completed', status: 'completed' });
    log({ message: 'Task completed on Delhivery Ship. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'delhivery-ship-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'delhivery-ship';
