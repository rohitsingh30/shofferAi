/**
 * Shop on Pepperfry — browse furniture, home decor, mattresses, check delivery, checkout, pay.
 *
 * Auto-generated Playwright script for pepperfry-furniture.
 * Site: https://www.pepperfry.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Pepperfry Furniture ──────────────────────────
    log({ step: 'Opening Pepperfry Furniture...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Pepperfry Furniture. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Pepperfry Furniture',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Pepperfry Furniture order',
      details: 'Completing your order on Pepperfry Furniture',
    ...
    log({ step: 'Pepperfry Furniture workflow completed', status: 'completed' });
    log({ message: 'Task completed on Pepperfry Furniture. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'pepperfry-furniture-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'pepperfry-furniture';
