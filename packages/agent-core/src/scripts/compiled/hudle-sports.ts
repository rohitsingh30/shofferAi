/**
 * Book sports facilities on Hudle — courts, turfs, swimming pools. Search venues, pick time slot, pay.
 *
 * Auto-generated Playwright script for hudle-sports.
 * Site: https://hudle.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hudle Sports ──────────────────────────
    log({ step: 'Opening Hudle Sports...', status: 'running' });
    await page.goto('https://hudle.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hudle Sports. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hudle Sports',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hudle Sports order',
      details: 'Completing your order on Hudle Sports',
      amountInr: 0, // Extracted from page at runtime
      description: 'hudle-sports order',
    });
    ...
    log({ step: 'Hudle Sports workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hudle Sports. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hudle-sports-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hudle-sports';
