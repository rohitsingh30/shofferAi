/**
 * Recharge Dish TV DTH. Browse channel packs, select plan, and pay online.
 *
 * Auto-generated Playwright script for dish-tv-recharge.
 * Site: https://www.dishtv.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Dish Tv Recharge ──────────────────────────
    log({ step: 'Opening Dish Tv Recharge...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Dish Tv Recharge. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Dish Tv Recharge',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Dish Tv Recharge order',
      details: 'Completing your order on Dish Tv Recharge',
      amountInr: 0, // Extracted from page at runtime
      description: 'dish-tv-recharge order',
    });
    ...
    log({ step: 'Dish Tv Recharge workflow completed', status: 'completed' });
    log({ message: 'Task completed on Dish Tv Recharge. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'dish-tv-recharge-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'dish-tv-recharge';
