/**
 * Set up WhatsApp Business profile — create business catalog, configure auto-replies, set business hours and greeting messages.
 *
 * Auto-generated Playwright script for whatsapp-business.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Whatsapp Business...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Whatsapp Business. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Whatsapp Business',
    ...
    log({ step: 'Whatsapp Business workflow completed', status: 'completed' });
    log({ message: 'Task completed on Whatsapp Business. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'whatsapp-business-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'whatsapp-business';
