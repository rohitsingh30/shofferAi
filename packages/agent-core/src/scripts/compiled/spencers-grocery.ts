/**
 * Order groceries from Spencer's Retail online — browse products, add to cart, schedule delivery, checkout, pay.
 *
 * Auto-generated Playwright script for spencers-grocery.
 * Site: https://www.spencers.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Spencers Grocery ──────────────────────────
    log({ step: 'Opening Spencers Grocery...', status: 'running' });
    await page.goto('https://www.spencers.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Spencers Grocery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Spencers Grocery',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Spencers Grocery order',
      details: 'Completing your order on Spencers Grocery',
      amountInr: 0, // Extracted from page at runtime
      description: 'spencers-grocery order',
    });
    ...
    log({ step: 'Spencers Grocery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Spencers Grocery. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'spencers-grocery-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'spencers-grocery';
