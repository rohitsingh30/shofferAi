/**
 * Apply for or renew driving license on Parivahan — fill application, upload documents, book slot, pay fee.
 *
 * Auto-generated Playwright script for parivahan-license.
 * Site: https://parivahan.gov.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Parivahan License ──────────────────────────
    log({ step: 'Opening Parivahan License...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Parivahan License. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Parivahan License',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Parivahan License order',
      details: 'Completing your order on Parivahan License',
      amountInr: 0, // Extracted from page at runtime
      description: 'parivahan-license order',
    });
    ...
    log({ step: 'Parivahan License workflow completed', status: 'completed' });
    log({ message: 'Task completed on Parivahan License. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'parivahan-license-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'parivahan-license';
