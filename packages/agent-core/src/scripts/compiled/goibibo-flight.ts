/**
 * Book flights on Goibibo — one-way or round-trip, search by route/dates, compare options, book and pay.
 *
 * Auto-generated Playwright script for goibibo-flight.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Goibibo Flight ──────────────────────────
    log({ step: 'Opening Goibibo Flight...', status: 'running' });
    ...
    const profileEl = page.locator('[class*="user-icon"]').first();
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Goibibo Flight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Goibibo Flight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Goibibo Flight order',
      details: 'Completing your order on Goibibo Flight',
      amountInr: 0, // Extracted from page at runtime
      description: 'goibibo-flight order',
    });
    ...
    log({ step: 'Goibibo Flight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Goibibo Flight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'goibibo-flight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'goibibo-flight';
