/**
 * Check EPF (Employee Provident Fund) balance and passbook on epfindia.gov.in — login, view contributions, download passbook.
 *
 * Auto-generated Playwright script for epfo-balance.
 * Site: https://passbook.epfindia.gov.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Epfo Balance ──────────────────────────
    log({ step: 'Opening Epfo Balance...', status: 'running' });
    await page.goto('https://passbook.epfindia.gov.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Epfo Balance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Epfo Balance',
    ...
    log({ step: 'Epfo Balance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Epfo Balance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'epfo-balance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'epfo-balance';
