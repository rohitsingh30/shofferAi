/**
 * Buy a certified used car on Spinny — browse 200+ point inspected cars, free test drive, 5-day return, buy with confidence.
 *
 * Auto-generated Playwright script for spinny-car.
 * Site: https://www.spinny.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Spinny Car ──────────────────────────
    log({ step: 'Opening Spinny Car...', status: 'running' });
    await page.goto('https://www.spinny.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Spinny Car. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Spinny Car',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Spinny Car order',
      details: 'Completing your order on Spinny Car',
      amountInr: 0, // Extracted from page at runtime
      description: 'spinny-car order',
    });
    ...
    log({ step: 'Spinny Car workflow completed', status: 'completed' });
    log({ message: 'Task completed on Spinny Car. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'spinny-car-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'spinny-car';
