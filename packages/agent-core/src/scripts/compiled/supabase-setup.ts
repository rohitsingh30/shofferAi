/**
 * Set up a Supabase project — create database, configure authentication, storage buckets, API keys, Row Level Security.
 *
 * Auto-generated Playwright script for supabase-setup.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Supabase Setup...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Supabase Setup. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Supabase Setup',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Supabase Setup order',
      details: 'Completing your order on Supabase Setup',
      amountInr: 0, // Extracted from page at runtime
      description: 'supabase-setup order',
    });
    ...
    log({ step: 'Supabase Setup workflow completed', status: 'completed' });
    log({ message: 'Task completed on Supabase Setup. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'supabase-setup-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'supabase-setup';
