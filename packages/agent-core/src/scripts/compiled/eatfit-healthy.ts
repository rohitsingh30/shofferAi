/**
 * Order healthy meals from EatFit — browse by calories, diet type, customize meals, checkout, pay.
 *
 * Auto-generated Playwright script for eatfit-healthy.
 * Site: https://www.eatfit.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Eatfit Healthy ──────────────────────────
    log({ step: 'Opening Eatfit Healthy...', status: 'running' });
    await page.goto('https://www.eatfit.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Eatfit Healthy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Eatfit Healthy',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Eatfit Healthy order',
      details: 'Completing your order on Eatfit Healthy',
      amountInr: 0, // Extracted from page at runtime
      description: 'eatfit-healthy order',
    });
    ...
    log({ step: 'Eatfit Healthy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Eatfit Healthy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'eatfit-healthy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'eatfit-healthy';
