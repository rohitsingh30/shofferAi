/**
 * Buy HP laptops, printers, and accessories on HP India — configure specs, customize, buy.
 *
 * Auto-generated Playwright script for hp-store.
 * Site: https://www.hp.com/in-en
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hp Store ──────────────────────────
    log({ step: 'Opening Hp Store...', status: 'running' });
    await page.goto('https://www.hp.com/in-en');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hp Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hp Store order',
      details: 'Completing your order on Hp Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'hp-store order',
    });
    ...
    log({ step: 'Hp Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hp Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hp-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hp-store';
