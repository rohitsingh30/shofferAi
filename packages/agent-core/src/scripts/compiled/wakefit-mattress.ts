/**
 * Buy a mattress on Wakefit — select size, type, firmness, check trial offer, order and pay.
 *
 * Auto-generated Playwright script for wakefit-mattress.
 * Site: https://www.wakefit.co
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wakefit Mattress ──────────────────────────
    log({ step: 'Opening Wakefit Mattress...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wakefit Mattress. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Wakefit Mattress',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wakefit Mattress order',
      details: 'Completing your order on Wakefit Mattress',
    ...
    log({ step: 'Wakefit Mattress workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wakefit Mattress. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wakefit-mattress-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wakefit-mattress';
