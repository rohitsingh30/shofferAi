/**
 * Search and book wedding venues on ShaadiSaga — filter by city, budget, guest capacity, venue type.
 *
 * Auto-generated Playwright script for shaadisaga-venue.
 * Site: https://www.shaadisaga.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Shaadisaga Venue ──────────────────────────
    log({ step: 'Opening Shaadisaga Venue...', status: 'running' });
    await page.goto('https://www.shaadisaga.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Shaadisaga Venue. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Shaadisaga Venue',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Shaadisaga Venue order',
      details: 'Completing your order on Shaadisaga Venue',
      amountInr: 0, // Extracted from page at runtime
      description: 'shaadisaga-venue order',
    });
    ...
    log({ step: 'Shaadisaga Venue workflow completed', status: 'completed' });
    log({ message: 'Task completed on Shaadisaga Venue. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'shaadisaga-venue-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'shaadisaga-venue';
