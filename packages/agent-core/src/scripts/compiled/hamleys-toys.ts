/**
 * Buy premium toys on Hamleys India — browse by age, category, brand, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for hamleys-toys.
 * Site: https://www.hamleys.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hamleys Toys ──────────────────────────
    log({ step: 'Opening Hamleys Toys...', status: 'running' });
    await page.goto('https://www.hamleys.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hamleys Toys. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hamleys Toys',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hamleys Toys order',
      details: 'Completing your order on Hamleys Toys',
      amountInr: 0, // Extracted from page at runtime
      description: 'hamleys-toys order',
    });
    ...
    log({ step: 'Hamleys Toys workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hamleys Toys. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hamleys-toys-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hamleys-toys';
