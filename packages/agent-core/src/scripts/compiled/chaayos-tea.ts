/**
 * Order chai and snacks from Chaayos — custom chai, combos, maggi, sandwiches, delivery, pay.
 *
 * Auto-generated Playwright script for chaayos-tea.
 * Site: https://www.chaayos.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Chaayos Tea ──────────────────────────
    log({ step: 'Opening Chaayos Tea...', status: 'running' });
    await page.goto('https://www.chaayos.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Chaayos Tea. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Chaayos Tea',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Chaayos Tea order',
      details: 'Completing your order on Chaayos Tea',
      amountInr: 0, // Extracted from page at runtime
      description: 'chaayos-tea order',
    });
    ...
    log({ step: 'Chaayos Tea workflow completed', status: 'completed' });
    log({ message: 'Task completed on Chaayos Tea. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'chaayos-tea-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'chaayos-tea';
