/**
 * Compare and buy insurance on Coverfox — car, bike, health, term life, travel insurance at best prices.
 *
 * Auto-generated Playwright script for coverfox-insurance.
 * Site: https://www.coverfox.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Coverfox Insurance ──────────────────────────
    log({ step: 'Opening Coverfox Insurance...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Coverfox Insurance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Coverfox Insurance',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Coverfox Insurance order',
      details: 'Completing your order on Coverfox Insurance',
      amountInr: 0, // Extracted from page at runtime
      description: 'coverfox-insurance order',
    });
    ...
    log({ step: 'Coverfox Insurance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Coverfox Insurance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'coverfox-insurance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'coverfox-insurance';
