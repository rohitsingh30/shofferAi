/**
 * Book coliving spaces on CoLive — shared apartments, private rooms with community amenities.
 *
 * Auto-generated Playwright script for colive-coliving.
 * Site: https://www.colive.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Colive Coliving ──────────────────────────
    log({ step: 'Opening Colive Coliving...', status: 'running' });
    await page.goto('https://www.colive.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Colive Coliving. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Colive Coliving',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Colive Coliving order',
      details: 'Completing your order on Colive Coliving',
      amountInr: 0, // Extracted from page at runtime
      description: 'colive-coliving order',
    });
    ...
    log({ step: 'Colive Coliving workflow completed', status: 'completed' });
    log({ message: 'Task completed on Colive Coliving. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'colive-coliving-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'colive-coliving';
