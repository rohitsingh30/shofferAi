/**
 * Subscribe to Airtel Xstream Fiber broadband — select plan, check availability, schedule installation, pay.
 *
 * Auto-generated Playwright script for airtel-fiber.
 * Site: https://www.airtel.in/xstream-fiber
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Airtel Fiber ──────────────────────────
    log({ step: 'Opening Airtel Fiber...', status: 'running' });
    await page.goto('https://www.airtel.in/xstream-fiber');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Airtel Fiber. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Airtel Fiber',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Airtel Fiber order',
      details: 'Completing your order on Airtel Fiber',
      amountInr: 0, // Extracted from page at runtime
      description: 'airtel-fiber order',
    });
    ...
    log({ step: 'Airtel Fiber workflow completed', status: 'completed' });
    log({ message: 'Task completed on Airtel Fiber. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'airtel-fiber-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'airtel-fiber';
