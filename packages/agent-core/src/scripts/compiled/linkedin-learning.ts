/**
 * Subscribe to LinkedIn Learning — browse professional courses, start learning paths, earn certificates.
 *
 * Auto-generated Playwright script for linkedin-learning.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Linkedin Learning...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Linkedin Learning. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Linkedin Learning',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Linkedin Learning order',
      details: 'Completing your order on Linkedin Learning',
      amountInr: 0, // Extracted from page at runtime
      description: 'linkedin-learning order',
    });
    ...
    log({ step: 'Linkedin Learning workflow completed', status: 'completed' });
    log({ message: 'Task completed on Linkedin Learning. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'linkedin-learning-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'linkedin-learning';
