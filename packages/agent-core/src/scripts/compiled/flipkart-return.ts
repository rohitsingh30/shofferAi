/**
 * Return or exchange a product on Flipkart — find order, initiate return/exchange, select reason, schedule pickup.
 *
 * Auto-generated Playwright script for flipkart-return.
 * Site: https://www.flipkart.com/account/orders
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Flipkart Return ──────────────────────────
    log({ step: 'Opening Flipkart Return...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Flipkart Return. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Flipkart Return',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Flipkart Return order',
      details: 'Completing your order on Flipkart Return',
      amountInr: 0, // Extracted from page at runtime
      description: 'flipkart-return order',
    });
    ...
    log({ step: 'Flipkart Return workflow completed', status: 'completed' });
    log({ message: 'Task completed on Flipkart Return. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'flipkart-return-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'flipkart-return';
