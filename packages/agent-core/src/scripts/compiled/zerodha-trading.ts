/**
 * Trade stocks on Zerodha Kite — search stocks, place buy/sell orders, manage portfolio, track positions.
 *
 * Auto-generated Playwright script for zerodha-trading.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Zerodha Trading ──────────────────────────
    log({ step: 'Opening Zerodha Trading...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zerodha Trading. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zerodha Trading',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Zerodha Trading order',
      details: 'Completing your order on Zerodha Trading',
      amountInr: 0, // Extracted from page at runtime
      description: 'zerodha-trading order',
    });
    ...
    log({ step: 'Zerodha Trading workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zerodha Trading. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'zerodha-trading-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'zerodha-trading';
