/**
 * Book concert, comedy show, or live event tickets on BookMyShow — search events, select seats, pay.
 *
 * Auto-generated Playwright script for bookmyshow-event.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bookmyshow Event ──────────────────────────
    log({ step: 'Opening Bookmyshow Event...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bookmyshow Event. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bookmyshow Event',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bookmyshow Event order',
      details: 'Completing your order on Bookmyshow Event',
      amountInr: 0, // Extracted from page at runtime
      description: 'bookmyshow-event order',
    });
    ...
    log({ step: 'Bookmyshow Event workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bookmyshow Event. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bookmyshow-event-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bookmyshow-event';
