/**
 * Search and book flights on Booking.com — compare airlines, prices, layovers, select flight, fill passenger details, and pay.
 *
 * Auto-generated Playwright script for booking-flight.
 * Site: https://www.booking.com/flights
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Booking Flight ──────────────────────────
    log({ step: 'Opening Booking Flight...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Booking Flight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Booking Flight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Booking Flight order',
      details: 'Completing your order on Booking Flight',
      amountInr: 0, // Extracted from page at runtime
      description: 'booking-flight order',
    });
    ...
    log({ step: 'Booking Flight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Booking Flight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'booking-flight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'booking-flight';
