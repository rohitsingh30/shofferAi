/**
 * Order sweets and snacks from local sweet shops via Swiggy or Zomato — mithai, namkeen, chaat, delivery, pay.
 *
 * Auto-generated Playwright script for bhagat-halwai.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bhagat Halwai ──────────────────────────
    log({ step: 'Opening Bhagat Halwai...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bhagat Halwai. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bhagat Halwai',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bhagat Halwai order',
      details: 'Completing your order on Bhagat Halwai',
      amountInr: 0, // Extracted from page at runtime
      description: 'bhagat-halwai order',
    });
    ...
    log({ step: 'Bhagat Halwai workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bhagat Halwai. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bhagat-halwai-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bhagat-halwai';
