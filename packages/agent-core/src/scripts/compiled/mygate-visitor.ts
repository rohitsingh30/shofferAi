/**
 * Register visitor or delivery on MyGate — enter guest details, select flat, generate digital visitor pass.
 *
 * Auto-generated Playwright script for mygate-visitor.
 * Site: https://mygate.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Mygate Visitor ──────────────────────────
    log({ step: 'Opening Mygate Visitor...', status: 'running' });
    await page.goto('https://mygate.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Mygate Visitor. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Mygate Visitor',
    ...
    log({ step: 'Mygate Visitor workflow completed', status: 'completed' });
    log({ message: 'Task completed on Mygate Visitor. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'mygate-visitor-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'mygate-visitor';
