/**
 * Join Skillshare classes — search creative courses, enroll in free trial or premium membership.
 *
 * Auto-generated Playwright script for skillshare-course.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Skillshare Course ──────────────────────────
    log({ step: 'Opening Skillshare Course...', status: 'running' });
    await page.goto('https://www.skillshare.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Skillshare Course. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Skillshare Course',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Skillshare Course order',
      details: 'Completing your order on Skillshare Course',
      amountInr: 0, // Extracted from page at runtime
      description: 'skillshare-course order',
    });
    ...
    log({ step: 'Skillshare Course workflow completed', status: 'completed' });
    log({ message: 'Task completed on Skillshare Course. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'skillshare-course-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'skillshare-course';
