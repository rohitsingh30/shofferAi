/**
 * Subscribe to Midjourney AI art generation. Choose plan (Basic, Standard, Pro, Mega), pay, and start creating.
 *
 * Auto-generated Playwright script for midjourney-subscribe.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Midjourney Subscribe ──────────────────────────
    log({ step: 'Opening Midjourney Subscribe...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Midjourney Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Midjourney Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Midjourney Subscribe order',
      details: 'Completing your order on Midjourney Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'midjourney-subscribe order',
    });
    ...
    log({ step: 'Midjourney Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Midjourney Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'midjourney-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'midjourney-subscribe';
