/**
 * Search and book hotels on Booking.com — compare prices, select rooms, fill guest details, handle payment.
 *
 * Auto-generated Playwright script for booking-hotel.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Booking Hotel ──────────────────────────
    log({ step: 'Opening Booking Hotel...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Booking Hotel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Booking Hotel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Booking Hotel order',
      details: 'Completing your order on Booking Hotel',
      amountInr: 0, // Extracted from page at runtime
      description: 'booking-hotel order',
    });
    ...
    log({ step: 'Booking Hotel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Booking Hotel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'booking-hotel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'booking-hotel';
