/**
 * Recharge FASTag on Paytm — enter vehicle number, select amount, pay toll charges.
 *
 * Auto-generated Playwright script for paytm-fastag.
 * Site: https://paytm.com/fastag-recharge
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Fastag ──────────────────────────
    log({ step: 'Opening Paytm Fastag...', status: 'running' });
    await page.goto('https://paytm.com/fastag-recharge');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Fastag. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Paytm Fastag',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Fastag order',
      details: 'Completing your order on Paytm Fastag',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-fastag order',
    });
    ...
    log({ step: 'Paytm Fastag workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Fastag. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-fastag-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-fastag';
