/**
 * Shop on Westside (Tata) — buy fashion, clothing, footwear, home furnishing, browse collections, checkout, pay.
 *
 * Auto-generated Playwright script for westside-fashion.
 * Site: https://www.westside.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Westside Fashion ──────────────────────────
    log({ step: 'Opening Westside Fashion...', status: 'running' });
    await page.goto('https://www.westside.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Westside Fashion. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Westside Fashion',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Westside Fashion order',
      details: 'Completing your order on Westside Fashion',
    ...
    log({ step: 'Westside Fashion workflow completed', status: 'completed' });
    log({ message: 'Task completed on Westside Fashion. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'westside-fashion-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'westside-fashion';
