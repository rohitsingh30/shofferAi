/**
 * Order food from Box8 — wraps, meals, thalis, combos, sides, delivery, pay.
 *
 * Auto-generated Playwright script for boxeight-food.
 * Site: https://www.box8.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Boxeight Food ──────────────────────────
    log({ step: 'Opening Boxeight Food...', status: 'running' });
    await page.goto('https://www.box8.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Boxeight Food. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Boxeight Food',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Boxeight Food order',
      details: 'Completing your order on Boxeight Food',
      amountInr: 0, // Extracted from page at runtime
      description: 'boxeight-food order',
    });
    ...
    log({ step: 'Boxeight Food workflow completed', status: 'completed' });
    log({ message: 'Task completed on Boxeight Food. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'boxeight-food-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'boxeight-food';
