/**
 * Book vet appointment on MarsVet/Wiggles — pet health consultation, grooming, vaccination, and wellness services.
 *
 * Auto-generated Playwright script for marsvet-pet.
 * Site: https://www.wiggles.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Marsvet Pet ──────────────────────────
    log({ step: 'Opening Marsvet Pet...', status: 'running' });
    await page.goto('https://www.wiggles.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Marsvet Pet. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Marsvet Pet',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Marsvet Pet order',
      details: 'Completing your order on Marsvet Pet',
      amountInr: 0, // Extracted from page at runtime
      description: 'marsvet-pet order',
    });
    ...
    log({ step: 'Marsvet Pet workflow completed', status: 'completed' });
    log({ message: 'Task completed on Marsvet Pet. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'marsvet-pet-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'marsvet-pet';
