/**
 * Create Facebook ad campaign — set objective, target audience, creative, budget, and launch via Meta Ads Manager.
 *
 * Auto-generated Playwright script for facebook-ads.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Facebook Ads ──────────────────────────
    log({ step: 'Opening Facebook Ads...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Facebook Ads. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Facebook Ads',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Facebook Ads order',
      details: 'Completing your order on Facebook Ads',
      amountInr: 0, // Extracted from page at runtime
      description: 'facebook-ads order',
    });
    ...
    log({ step: 'Facebook Ads workflow completed', status: 'completed' });
    log({ message: 'Task completed on Facebook Ads. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'facebook-ads-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'facebook-ads';
