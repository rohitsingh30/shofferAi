/**
 * Donate to causes on Ketto India — crowdfunding for medical, education, disaster relief, and social causes.
 *
 * Auto-generated Playwright script for ketto-donate.
 * Site: https://www.ketto.org
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ketto Donate ──────────────────────────
    log({ step: 'Opening Ketto Donate...', status: 'running' });
    await page.goto('https://www.ketto.org');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ketto Donate. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Ketto Donate',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ketto Donate order',
      details: 'Completing your order on Ketto Donate',
      amountInr: 0, // Extracted from page at runtime
      description: 'ketto-donate order',
    });
    ...
    log({ step: 'Ketto Donate workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ketto Donate. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ketto-donate-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'ketto-donate';
