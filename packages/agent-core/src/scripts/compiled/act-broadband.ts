/**
 * Pay ACT Fibernet broadband bill online. Login, view bill, select plan, and pay.
 *
 * Auto-generated Playwright script for act-broadband.
 * Site: https://selfcare.actcorp.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Act Broadband ──────────────────────────
    log({ step: 'Opening Act Broadband...', status: 'running' });
    await page.goto('https://selfcare.actcorp.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Act Broadband. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Act Broadband',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Act Broadband order',
      details: 'Completing your order on Act Broadband',
      amountInr: 0, // Extracted from page at runtime
      description: 'act-broadband order',
    });
    ...
    log({ step: 'Act Broadband workflow completed', status: 'completed' });
    log({ message: 'Task completed on Act Broadband. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'act-broadband-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'act-broadband';
