/**
 * Shop on Adidas India — browse, select size, buy shoes, apparel, and accessories, checkout, pay.
 *
 * Auto-generated Playwright script for adidas-shoes.
 * Site: https://www.adidas.co.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Adidas Shoes ──────────────────────────
    log({ step: 'Opening Adidas Shoes...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Adidas Shoes. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Adidas Shoes',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Adidas Shoes order',
      details: 'Completing your order on Adidas Shoes',
      amountInr: 0, // Extracted from page at runtime
      description: 'adidas-shoes order',
    });
    ...
    log({ step: 'Adidas Shoes workflow completed', status: 'completed' });
    log({ message: 'Task completed on Adidas Shoes. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'adidas-shoes-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'adidas-shoes';
