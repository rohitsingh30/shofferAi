/**
 * Subscribe to Kindle Unlimited India on Amazon — unlimited reading from 2M+ ebooks, audiobooks, and magazines for a monthly fee.
 *
 * Auto-generated Playwright script for amazon-kindle-unlimited.
 * Site: https://www.amazon.in/kindle-dbs/hz/subscribe/ku
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Amazon Kindle Unlimited ──────────────────────────
    log({ step: 'Opening Amazon Kindle Unlimited...', status: 'running' });
    await page.goto('https://www.amazon.in/kindle-dbs/hz/subscribe/ku');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Kindle Unlimited. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Kindle Unlimited',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Amazon Kindle Unlimited order',
      details: 'Completing your order on Amazon Kindle Unlimited',
      amountInr: 0, // Extracted from page at runtime
      description: 'amazon-kindle-unlimited order',
    });
    ...
    log({ step: 'Amazon Kindle Unlimited workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Kindle Unlimited. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-kindle-unlimited-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-kindle-unlimited';
