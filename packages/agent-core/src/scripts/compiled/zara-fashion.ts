/**
 * Buy Zara fashion on zara.com/in — browse clothing, shoes, accessories, select size, add to bag, checkout, pay.
 *
 * Auto-generated Playwright script for zara-fashion.
 * Site: https://www.zara.com/in/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Zara Fashion ──────────────────────────
    log({ step: 'Opening Zara Fashion...', status: 'running' });
    await page.goto('https://www.zara.com/in/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zara Fashion. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zara Fashion',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Zara Fashion order',
      details: 'Completing your order on Zara Fashion',
    ...
    log({ step: 'Zara Fashion workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zara Fashion. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'zara-fashion-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'zara-fashion';
