/**
 * Find managed rental homes on NestAway — furnished flats, PG, shared rooms with zero brokerage.
 *
 * Auto-generated Playwright script for nestaway-rental.
 * Site: https://www.nestaway.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Nestaway Rental ──────────────────────────
    log({ step: 'Opening Nestaway Rental...', status: 'running' });
    await page.goto('https://www.nestaway.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Nestaway Rental. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Nestaway Rental',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Nestaway Rental order',
      details: 'Completing your order on Nestaway Rental',
      amountInr: 0, // Extracted from page at runtime
      description: 'nestaway-rental order',
    });
    ...
    log({ step: 'Nestaway Rental workflow completed', status: 'completed' });
    log({ message: 'Task completed on Nestaway Rental. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'nestaway-rental-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'nestaway-rental';
