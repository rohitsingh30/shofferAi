/**
 * Check ITR refund status on incometax.gov.in — login, view filing status, track refund, check processing.
 *
 * Auto-generated Playwright script for incometax-refund.
 * Site: https://www.incometax.gov.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Incometax Refund ──────────────────────────
    log({ step: 'Opening Incometax Refund...', status: 'running' });
    await page.goto('https://www.incometax.gov.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Incometax Refund. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Incometax Refund',
    ...
    log({ step: 'Incometax Refund workflow completed', status: 'completed' });
    log({ message: 'Task completed on Incometax Refund. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'incometax-refund-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'incometax-refund';
