/**
 * Shop on Woodland India — browse, select size, buy outdoor shoes, boots, sandals, and apparel, checkout, pay.
 *
 * Auto-generated Playwright script for woodland-shoes.
 * Site: https://www.woodland.co.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Woodland Shoes ──────────────────────────
    log({ step: 'Opening Woodland Shoes...', status: 'running' });
    await page.goto('https://www.woodland.co.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Woodland Shoes. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Woodland Shoes',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Woodland Shoes order',
      details: 'Completing your order on Woodland Shoes',
      amountInr: 0, // Extracted from page at runtime
      description: 'woodland-shoes order',
    });
    ...
    log({ step: 'Woodland Shoes workflow completed', status: 'completed' });
    log({ message: 'Task completed on Woodland Shoes. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'woodland-shoes-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'woodland-shoes';
