/**
 * Buy organic and clean-label food from Conscious Food — specialty grains, flours, sweeteners, oils, and health foods.
 *
 * Auto-generated Playwright script for conscious-food.
 * Site: https://www.consciousfood.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Conscious Food ──────────────────────────
    log({ step: 'Opening Conscious Food...', status: 'running' });
    await page.goto('https://www.consciousfood.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Conscious Food. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Conscious Food',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Conscious Food order',
      details: 'Completing your order on Conscious Food',
    ...
    log({ step: 'Conscious Food workflow completed', status: 'completed' });
    log({ message: 'Task completed on Conscious Food. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'conscious-food-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'conscious-food';
