/**
 * Book Uber ride — set pickup/drop, select ride type (Go/Premier/XL), confirm ride, pay.
 *
 * Auto-generated Playwright script for uber-cab.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Uber Cab ──────────────────────────
    log({ step: 'Opening Uber Cab...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Uber Cab. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Uber Cab order',
      details: 'Completing your order on Uber Cab',
      amountInr: 0, // Extracted from page at runtime
      description: 'uber-cab order',
    });
    ...
    log({ step: 'Uber Cab workflow completed', status: 'completed' });
    log({ message: 'Task completed on Uber Cab. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'uber-cab-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'uber-cab';
