/**
 * Book appointment at Max Hospital — find specialists by department, schedule in-person or video consultation.
 *
 * Auto-generated Playwright script for max-healthcare.
 * Site: https://www.maxhealthcare.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Max Healthcare ──────────────────────────
    log({ step: 'Opening Max Healthcare...', status: 'running' });
    await page.goto('https://www.maxhealthcare.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Max Healthcare. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Max Healthcare',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Max Healthcare order',
      details: 'Completing your order on Max Healthcare',
      amountInr: 0, // Extracted from page at runtime
      description: 'max-healthcare order',
    });
    ...
    log({ step: 'Max Healthcare workflow completed', status: 'completed' });
    log({ message: 'Task completed on Max Healthcare. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'max-healthcare-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'max-healthcare';
