/**
 * File GST return on ClearTax — upload invoices, reconcile data, review summary, and file GSTR-1/GSTR-3B with payment.
 *
 * Auto-generated Playwright script for cleartax-gst.
 * Site: https://cleartax.in/gst
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cleartax Gst ──────────────────────────
    log({ step: 'Opening Cleartax Gst...', status: 'running' });
    await page.goto('https://cleartax.in/gst');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cleartax Gst. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cleartax Gst',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cleartax Gst order',
      details: 'Completing your order on Cleartax Gst',
      amountInr: 0, // Extracted from page at runtime
      description: 'cleartax-gst order',
    });
    ...
    log({ step: 'Cleartax Gst workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cleartax Gst. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cleartax-gst-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cleartax-gst';
