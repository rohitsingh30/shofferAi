/**
 * Create a rent agreement online on StampReady or NoBroker — draft agreement, e-stamp paper, notarization, and doorstep delivery.
 *
 * Auto-generated Playwright script for stampready-agreement.
 * Site: https://www.nobroker.in/rent-agreement
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Stampready Agreement ──────────────────────────
    log({ step: 'Opening Stampready Agreement...', status: 'running' });
    await page.goto('https://www.nobroker.in/rent-agreement');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Stampready Agreement. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Stampready Agreement',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Stampready Agreement order',
      details: 'Completing your order on Stampready Agreement',
      amountInr: 0, // Extracted from page at runtime
      description: 'stampready-agreement order',
    });
    ...
    log({ step: 'Stampready Agreement workflow completed', status: 'completed' });
    log({ message: 'Task completed on Stampready Agreement. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'stampready-agreement-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'stampready-agreement';
