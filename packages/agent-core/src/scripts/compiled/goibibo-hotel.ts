/**
 * Book hotels on Goibibo — search by city/dates, filter by price/rating, compare options, book and pay.
 *
 * Auto-generated Playwright script for goibibo-hotel.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Goibibo Hotel ──────────────────────────
    log({ step: 'Opening Goibibo Hotel...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Goibibo Hotel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Goibibo Hotel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Goibibo Hotel order',
      details: 'Completing your order on Goibibo Hotel',
      amountInr: 0, // Extracted from page at runtime
      description: 'goibibo-hotel order',
    });
    ...
    log({ step: 'Goibibo Hotel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Goibibo Hotel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'goibibo-hotel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'goibibo-hotel';
