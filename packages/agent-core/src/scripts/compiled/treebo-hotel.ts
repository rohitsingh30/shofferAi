/**
 * Book budget hotels on Treebo — search by city/dates, compare quality-assured properties, select room, pay.
 *
 * Auto-generated Playwright script for treebo-hotel.
 * Site: https://www.treebo.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Treebo Hotel ──────────────────────────
    log({ step: 'Opening Treebo Hotel...', status: 'running' });
    await page.goto('https://www.treebo.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Treebo Hotel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Treebo Hotel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Treebo Hotel order',
      details: 'Completing your order on Treebo Hotel',
      amountInr: 0, // Extracted from page at runtime
      description: 'treebo-hotel order',
    });
    ...
    log({ step: 'Treebo Hotel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Treebo Hotel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'treebo-hotel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'treebo-hotel';
