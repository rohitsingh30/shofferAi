/**
 * Book appointment at Narayana Health — find specialists, schedule in-person or teleconsultation, book health checkups.
 *
 * Auto-generated Playwright script for narayana-health.
 * Site: https://www.narayanahealth.org
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Narayana Health ──────────────────────────
    log({ step: 'Opening Narayana Health...', status: 'running' });
    await page.goto('https://www.narayanahealth.org');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Narayana Health. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Narayana Health',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Narayana Health order',
      details: 'Completing your order on Narayana Health',
      amountInr: 0, // Extracted from page at runtime
      description: 'narayana-health order',
    });
    ...
    log({ step: 'Narayana Health workflow completed', status: 'completed' });
    log({ message: 'Task completed on Narayana Health. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'narayana-health-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'narayana-health';
