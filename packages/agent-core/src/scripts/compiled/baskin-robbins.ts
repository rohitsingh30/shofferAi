/**
 * Order ice cream from Baskin Robbins India — flavors, sundaes, cakes, packs, delivery, pay.
 *
 * Auto-generated Playwright script for baskin-robbins.
 * Site: https://www.baskinrobbinsindia.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Baskin Robbins ──────────────────────────
    log({ step: 'Opening Baskin Robbins...', status: 'running' });
    await page.goto('https://www.baskinrobbinsindia.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Baskin Robbins. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Baskin Robbins',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Baskin Robbins order',
      details: 'Completing your order on Baskin Robbins',
      amountInr: 0, // Extracted from page at runtime
      description: 'baskin-robbins order',
    });
    ...
    log({ step: 'Baskin Robbins workflow completed', status: 'completed' });
    log({ message: 'Task completed on Baskin Robbins. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'baskin-robbins-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'baskin-robbins';
