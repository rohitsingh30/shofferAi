/**
 * Get a diet or fitness plan on HealthifyMe — set goals, explore plans, subscribe, track meals and workouts.
 *
 * Auto-generated Playwright script for healthifyme-diet.
 * Site: https://www.healthifyme.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Healthifyme Diet ──────────────────────────
    log({ step: 'Opening Healthifyme Diet...', status: 'running' });
    await page.goto('https://www.healthifyme.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Healthifyme Diet. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Healthifyme Diet',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Healthifyme Diet order',
      details: 'Completing your order on Healthifyme Diet',
      amountInr: 0, // Extracted from page at runtime
      description: 'healthifyme-diet order',
    });
    ...
    log({ step: 'Healthifyme Diet workflow completed', status: 'completed' });
    log({ message: 'Task completed on Healthifyme Diet. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'healthifyme-diet-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'healthifyme-diet';
