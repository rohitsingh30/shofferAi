/**
 * Find travel itineraries on Tripoto — community travel plans, trip ideas, guided packages, blog-based destination research.
 *
 * Auto-generated Playwright script for tripoto-travel.
 * Site: https://www.tripoto.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Tripoto Travel ──────────────────────────
    log({ step: 'Opening Tripoto Travel...', status: 'running' });
    await page.goto('https://www.tripoto.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Tripoto Travel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Tripoto Travel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Tripoto Travel order',
      details: 'Completing your order on Tripoto Travel',
      amountInr: 0, // Extracted from page at runtime
      description: 'tripoto-travel order',
    });
    ...
    log({ step: 'Tripoto Travel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Tripoto Travel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'tripoto-travel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'tripoto-travel';
