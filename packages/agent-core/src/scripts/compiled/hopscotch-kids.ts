/**
 * Buy kids clothing on Hopscotch — browse by age, gender, category, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for hopscotch-kids.
 * Site: https://www.hopscotch.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hopscotch Kids ──────────────────────────
    log({ step: 'Opening Hopscotch Kids...', status: 'running' });
    await page.goto('https://www.hopscotch.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hopscotch Kids. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hopscotch Kids',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hopscotch Kids order',
      details: 'Completing your order on Hopscotch Kids',
      amountInr: 0, // Extracted from page at runtime
      description: 'hopscotch-kids order',
    });
    ...
    log({ step: 'Hopscotch Kids workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hopscotch Kids. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hopscotch-kids-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hopscotch-kids';
