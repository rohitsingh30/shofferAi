/**
 * Shop on NurseryLive — browse plants, seeds, pots, gardening tools, fertilizers, checkout, pay.
 *
 * Auto-generated Playwright script for nursery-live-plants.
 * Site: https://www.nurserylive.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Nursery Live Plants ──────────────────────────
    log({ step: 'Opening Nursery Live Plants...', status: 'running' });
    await page.goto('https://www.nurserylive.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Nursery Live Plants. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Nursery Live Plants',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Nursery Live Plants order',
      details: 'Completing your order on Nursery Live Plants',
      amountInr: 0, // Extracted from page at runtime
      description: 'nursery-live-plants order',
    });
    ...
    log({ step: 'Nursery Live Plants workflow completed', status: 'completed' });
    log({ message: 'Task completed on Nursery Live Plants. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'nursery-live-plants-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'nursery-live-plants';
