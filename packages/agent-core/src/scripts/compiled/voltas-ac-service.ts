/**
 * Book Voltas AC service — installation, repair, gas refill, AMC, deep cleaning for Voltas and Voltas Beko products.
 *
 * Auto-generated Playwright script for voltas-ac-service.
 * Site: https://www.voltas.com/customer-support
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Voltas Ac Service ──────────────────────────
    log({ step: 'Opening Voltas Ac Service...', status: 'running' });
    await page.goto('https://www.voltas.com/customer-support');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Voltas Ac Service. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Voltas Ac Service',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Voltas Ac Service order',
      details: 'Completing your order on Voltas Ac Service',
      amountInr: 0, // Extracted from page at runtime
      description: 'voltas-ac-service order',
    });
    ...
    log({ step: 'Voltas Ac Service workflow completed', status: 'completed' });
    log({ message: 'Task completed on Voltas Ac Service. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'voltas-ac-service-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'voltas-ac-service';
