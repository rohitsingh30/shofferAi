/**
 * Start learning on Khan Academy — select subject, create personalized learning plan, track progress.
 *
 * Auto-generated Playwright script for khan-academy.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Khan Academy...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Khan Academy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Khan Academy',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Khan Academy order',
      details: 'Completing your order on Khan Academy',
      amountInr: 0, // Extracted from page at runtime
      description: 'khan-academy order',
    });
    ...
    log({ step: 'Khan Academy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Khan Academy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'khan-academy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'khan-academy';
