/**
 * Shop on Reliance Digital — buy electronics, appliances, gadgets, compare specs, checkout with installation, pay.
 *
 * Auto-generated Playwright script for reliance-digital.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Reliance Digital ──────────────────────────
    log({ step: 'Opening Reliance Digital...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Reliance Digital. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Reliance Digital',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Reliance Digital order',
      details: 'Completing your order on Reliance Digital',
    ...
    log({ step: 'Reliance Digital workflow completed', status: 'completed' });
    log({ message: 'Task completed on Reliance Digital. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'reliance-digital-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'reliance-digital';
