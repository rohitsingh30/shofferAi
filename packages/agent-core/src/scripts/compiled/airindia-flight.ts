/**
 * Book Air India flights on airindia.com — search routes, select class, fill details, pay.
 *
 * Auto-generated Playwright script for airindia-flight.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Airindia Flight ──────────────────────────
    log({ step: 'Opening Airindia Flight...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Airindia Flight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Airindia Flight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Airindia Flight order',
      details: 'Completing your order on Airindia Flight',
      amountInr: 0, // Extracted from page at runtime
      description: 'airindia-flight order',
    });
    ...
    log({ step: 'Airindia Flight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Airindia Flight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'airindia-flight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'airindia-flight';
