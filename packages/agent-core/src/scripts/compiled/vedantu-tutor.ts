/**
 * Book live tutoring sessions on Vedantu — select subject, grade, and schedule 1-on-1 or group classes.
 *
 * Auto-generated Playwright script for vedantu-tutor.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Vedantu Tutor ──────────────────────────
    log({ step: 'Opening Vedantu Tutor...', status: 'running' });
    await page.goto('https://www.vedantu.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Vedantu Tutor. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Vedantu Tutor',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Vedantu Tutor order',
      details: 'Completing your order on Vedantu Tutor',
      amountInr: 0, // Extracted from page at runtime
      description: 'vedantu-tutor order',
    });
    ...
    log({ step: 'Vedantu Tutor workflow completed', status: 'completed' });
    log({ message: 'Task completed on Vedantu Tutor. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'vedantu-tutor-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'vedantu-tutor';
