/**
 * Search rental apartments on NoBroker — filter by BHK, budget, locality, shortlist properties, contact owners directly.
 *
 * Auto-generated Playwright script for nobroker-rental.
 * Site: https://www.nobroker.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Nobroker Rental ──────────────────────────
    log({ step: 'Opening Nobroker Rental...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Nobroker Rental. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Nobroker Rental',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Nobroker Rental order',
      details: 'Completing your order on Nobroker Rental',
      amountInr: 0, // Extracted from page at runtime
      description: 'nobroker-rental order',
    });
    ...
    log({ step: 'Nobroker Rental workflow completed', status: 'completed' });
    log({ message: 'Task completed on Nobroker Rental. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'nobroker-rental-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'nobroker-rental';
