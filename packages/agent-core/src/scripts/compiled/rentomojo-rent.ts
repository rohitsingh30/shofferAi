/**
 * Rent furniture and appliances on Rentomojo — browse items, select tenure, subscribe, pay deposit.
 *
 * Auto-generated Playwright script for rentomojo-rent.
 * Site: https://www.rentomojo.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Rentomojo Rent ──────────────────────────
    log({ step: 'Opening Rentomojo Rent...', status: 'running' });
    await page.goto('https://www.rentomojo.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Rentomojo Rent. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Rentomojo Rent',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Rentomojo Rent order',
      details: 'Completing your order on Rentomojo Rent',
      amountInr: 0, // Extracted from page at runtime
      description: 'rentomojo-rent order',
    });
    ...
    log({ step: 'Rentomojo Rent workflow completed', status: 'completed' });
    log({ message: 'Task completed on Rentomojo Rent. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'rentomojo-rent-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'rentomojo-rent';
