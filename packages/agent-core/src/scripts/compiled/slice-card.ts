/**
 * Manage Slice credit line — pay bills, track spending, check rewards, manage EMI, view statements.
 *
 * Auto-generated Playwright script for slice-card.
 * Site: https://www.sliceit.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Slice Card ──────────────────────────
    log({ step: 'Opening Slice Card...', status: 'running' });
    await page.goto('https://www.sliceit.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Slice Card. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Slice Card',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Slice Card order',
      details: 'Completing your order on Slice Card',
      amountInr: 0, // Extracted from page at runtime
      description: 'slice-card order',
    });
    ...
    log({ step: 'Slice Card workflow completed', status: 'completed' });
    log({ message: 'Task completed on Slice Card. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'slice-card-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'slice-card';
