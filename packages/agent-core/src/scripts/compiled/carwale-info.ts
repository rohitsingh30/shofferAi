/**
 * Research cars on CarWale — compare specifications, prices, reviews, calculate EMI, find dealers, check on-road price.
 *
 * Auto-generated Playwright script for carwale-info.
 * Site: https://www.carwale.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Carwale Info ──────────────────────────
    log({ step: 'Opening Carwale Info...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Carwale Info. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Carwale Info',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Carwale Info order',
      details: 'Completing your order on Carwale Info',
      amountInr: 0, // Extracted from page at runtime
      description: 'carwale-info order',
    });
    ...
    log({ step: 'Carwale Info workflow completed', status: 'completed' });
    log({ message: 'Task completed on Carwale Info. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'carwale-info-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'carwale-info';
