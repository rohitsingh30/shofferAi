/**
 * Buy LG appliances on LG India — TVs, ACs, refrigerators, washing machines, microwaves, air purifiers.
 *
 * Auto-generated Playwright script for lg-store.
 * Site: https://www.lg.com/in/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Lg Store ──────────────────────────
    log({ step: 'Opening Lg Store...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Lg Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Lg Store order',
      details: 'Completing your order on Lg Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'lg-store order',
    });
    ...
    log({ step: 'Lg Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Lg Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'lg-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'lg-store';
