/**
 * Buy natural skincare and haircare on Mamaearth — browse toxin-free products, select variants, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for mama-earth.
 * Site: https://mamaearth.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Mama Earth ──────────────────────────
    log({ step: 'Opening Mama Earth...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Mama Earth. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Mama Earth',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Mama Earth order',
      details: 'Completing your order on Mama Earth',
      amountInr: 0, // Extracted from page at runtime
      description: 'mama-earth order',
    });
    ...
    log({ step: 'Mama Earth workflow completed', status: 'completed' });
    log({ message: 'Task completed on Mama Earth. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'mama-earth-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'mama-earth';
