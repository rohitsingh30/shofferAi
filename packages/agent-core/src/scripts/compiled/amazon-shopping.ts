/**
 * Shop on Amazon.in — search products, compare options, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for amazon-shopping.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Amazon Shopping ──────────────────────────
    log({ step: 'Opening Amazon Shopping...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Shopping. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Shopping',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Amazon Shopping order',
      details: 'Completing your order on Amazon Shopping',
    ...
    log({ step: 'Amazon Shopping workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Shopping. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-shopping-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-shopping';
