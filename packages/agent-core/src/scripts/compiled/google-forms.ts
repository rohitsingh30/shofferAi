/**
 * Create a Google Form survey — add questions, configure settings, customize theme, and share the form link.
 *
 * Auto-generated Playwright script for google-forms.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Google Forms...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Google Forms. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Google Forms',
    ...
    log({ step: 'Google Forms workflow completed', status: 'completed' });
    log({ message: 'Task completed on Google Forms. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'google-forms-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'google-forms';
