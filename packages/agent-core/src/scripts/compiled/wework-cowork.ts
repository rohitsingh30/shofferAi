/**
 * Book coworking space on WeWork India — day passes, hot desks, dedicated desks, private offices.
 *
 * Auto-generated Playwright script for wework-cowork.
 * Site: https://www.wework.co.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wework Cowork ──────────────────────────
    log({ step: 'Opening Wework Cowork...', status: 'running' });
    await page.goto('https://www.wework.co.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wework Cowork. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Wework Cowork',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wework Cowork order',
      details: 'Completing your order on Wework Cowork',
      amountInr: 0, // Extracted from page at runtime
      description: 'wework-cowork order',
    });
    ...
    log({ step: 'Wework Cowork workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wework Cowork. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wework-cowork-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wework-cowork';
