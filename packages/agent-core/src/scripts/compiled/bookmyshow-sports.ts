/**
 * Book sports event tickets on BookMyShow — IPL, ISL, PKL, WPL cricket and football matches. Search, select seats, pay.
 *
 * Auto-generated Playwright script for bookmyshow-sports.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bookmyshow Sports ──────────────────────────
    log({ step: 'Opening Bookmyshow Sports...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bookmyshow Sports. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bookmyshow Sports',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bookmyshow Sports order',
      details: 'Completing your order on Bookmyshow Sports',
      amountInr: 0, // Extracted from page at runtime
      description: 'bookmyshow-sports order',
    });
    ...
    log({ step: 'Bookmyshow Sports workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bookmyshow Sports. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bookmyshow-sports-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bookmyshow-sports';
