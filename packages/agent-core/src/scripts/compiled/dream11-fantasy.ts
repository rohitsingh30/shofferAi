/**
 * Create fantasy sports team on Dream11 — select match, pick players, set captain/vice-captain, join contest.
 *
 * Auto-generated Playwright script for dream11-fantasy.
 * Site: https://www.dream11.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Dream11 Fantasy ──────────────────────────
    log({ step: 'Opening Dream11 Fantasy...', status: 'running' });
    await page.goto('https://www.dream11.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Dream11 Fantasy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Dream11 Fantasy',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Dream11 Fantasy order',
      details: 'Completing your order on Dream11 Fantasy',
      amountInr: 0, // Extracted from page at runtime
      description: 'dream11-fantasy order',
    });
    ...
    log({ step: 'Dream11 Fantasy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Dream11 Fantasy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'dream11-fantasy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'dream11-fantasy';
