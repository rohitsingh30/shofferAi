/**
 * Return or exchange fashion items on Myntra — find order, initiate return/exchange, select reason, schedule pickup.
 *
 * Auto-generated Playwright script for myntra-return.
 * Site: https://www.myntra.com/my/orders
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Myntra Return ──────────────────────────
    log({ step: 'Opening Myntra Return...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Myntra Return. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Myntra Return',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Myntra Return order',
      details: 'Completing your order on Myntra Return',
      amountInr: 0, // Extracted from page at runtime
      description: 'myntra-return order',
    });
    ...
    log({ step: 'Myntra Return workflow completed', status: 'completed' });
    log({ message: 'Task completed on Myntra Return. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'myntra-return-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'myntra-return';
