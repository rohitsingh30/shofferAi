/**
 * Enroll kids in BYJU'S Early Learn program — K-3 learning, select class, plan, subscribe, pay.
 *
 * Auto-generated Playwright script for byju-kids.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Byju Kids ──────────────────────────
    log({ step: 'Opening Byju Kids...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Byju Kids. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Byju Kids order',
      details: 'Completing your order on Byju Kids',
      amountInr: 0, // Extracted from page at runtime
      description: 'byju-kids order',
    });
    ...
    log({ step: 'Byju Kids workflow completed', status: 'completed' });
    log({ message: 'Task completed on Byju Kids. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'byju-kids-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'byju-kids';
