/**
 * Order anything via Dunzo — groceries, food, packages, pick & drop, essentials delivery.
 *
 * Auto-generated Playwright script for dunzo-delivery.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Dunzo Delivery ──────────────────────────
    log({ step: 'Opening Dunzo Delivery...', status: 'running' });
    await page.goto('https://www.dunzo.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Dunzo Delivery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Dunzo Delivery',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Dunzo Delivery order',
      details: 'Completing your order on Dunzo Delivery',
    ...
    log({ step: 'Dunzo Delivery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Dunzo Delivery. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'dunzo-delivery-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'dunzo-delivery';
