/**
 * Subscribe to Sony LIV India. Sports (cricket, football, WWE), shows, movies, and originals.
 *
 * Auto-generated Playwright script for sony-liv-subscribe.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Sony Liv Subscribe ──────────────────────────
    log({ step: 'Opening Sony Liv Subscribe...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Sony Liv Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Sony Liv Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Sony Liv Subscribe order',
      details: 'Completing your order on Sony Liv Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'sony-liv-subscribe order',
    });
    ...
    log({ step: 'Sony Liv Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Sony Liv Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'sony-liv-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'sony-liv-subscribe';
