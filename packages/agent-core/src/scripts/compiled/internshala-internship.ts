/**
 * Apply for internships on Internshala — search, filter by domain/stipend/location, and apply.
 *
 * Auto-generated Playwright script for internshala-internship.
 * Site: https://internshala.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Internshala Internship ──────────────────────────
    log({ step: 'Opening Internshala Internship...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Internshala Internship. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Internshala Internship',
    ...
    log({ step: 'Internshala Internship workflow completed', status: 'completed' });
    log({ message: 'Task completed on Internshala Internship. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'internshala-internship-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'internshala-internship';
