/**
 * Buy online courses on Udemy — search courses, compare ratings and reviews, purchase.
 *
 * Auto-generated Playwright script for udemy-course.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Udemy Course ──────────────────────────
    log({ step: 'Opening Udemy Course...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Udemy Course. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Udemy Course',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Udemy Course order',
      details: 'Completing your order on Udemy Course',
      amountInr: 0, // Extracted from page at runtime
      description: 'udemy-course order',
    });
    ...
    log({ step: 'Udemy Course workflow completed', status: 'completed' });
    log({ message: 'Task completed on Udemy Course. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'udemy-course-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'udemy-course';
