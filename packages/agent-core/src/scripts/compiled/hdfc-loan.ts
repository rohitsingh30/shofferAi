/**
 * Apply for HDFC Bank loan — personal loan, home loan, car loan, two-wheeler loan, gold loan, education loan.
 *
 * Auto-generated Playwright script for hdfc-loan.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hdfc Loan ──────────────────────────
    log({ step: 'Opening Hdfc Loan...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hdfc Loan. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hdfc Loan order',
      details: 'Completing your order on Hdfc Loan',
      amountInr: 0, // Extracted from page at runtime
      description: 'hdfc-loan order',
    });
    ...
    log({ step: 'Hdfc Loan workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hdfc Loan. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hdfc-loan-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hdfc-loan';
