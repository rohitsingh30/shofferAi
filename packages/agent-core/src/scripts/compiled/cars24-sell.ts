/**
 * Sell your used car on Cars24 — enter car details, get instant valuation, schedule free inspection at home.
 *
 * Auto-generated Playwright script for cars24-sell.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cars24 Sell ──────────────────────────
    log({ step: 'Opening Cars24 Sell...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cars24 Sell. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cars24 Sell',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cars24 Sell order',
      details: 'Completing your order on Cars24 Sell',
      amountInr: 0, // Extracted from page at runtime
      description: 'cars24-sell order',
    });
    ...
    log({ step: 'Cars24 Sell workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cars24 Sell. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cars24-sell-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cars24-sell';
