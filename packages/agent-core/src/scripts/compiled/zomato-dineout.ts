/**
 * Book a restaurant table on Zomato — search restaurants, select date/time/guests, reserve.
 *
 * Auto-generated Playwright script for zomato-dineout.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Zomato Dineout ──────────────────────────
    log({ step: 'Opening Zomato Dineout...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zomato Dineout. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zomato Dineout',
    ...
    log({ step: 'Zomato Dineout workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zomato Dineout. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'zomato-dineout-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'zomato-dineout';
