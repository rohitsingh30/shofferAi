/**
 * Recharge Vi (Vodafone Idea) prepaid or postpaid on myvi.in. Browse plans, select, and pay.
 *
 * Auto-generated Playwright script for vi-recharge.
 * Site: https://www.myvi.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Vi Recharge ──────────────────────────
    log({ step: 'Opening Vi Recharge...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Vi Recharge. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Vi Recharge',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Vi Recharge order',
      details: 'Completing your order on Vi Recharge',
      amountInr: 0, // Extracted from page at runtime
      description: 'vi-recharge order',
    });
    ...
    log({ step: 'Vi Recharge workflow completed', status: 'completed' });
    log({ message: 'Task completed on Vi Recharge. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'vi-recharge-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'vi-recharge';
