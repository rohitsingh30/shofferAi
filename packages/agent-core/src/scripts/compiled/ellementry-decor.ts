/**
 * Shop on Ellementry — browse handcrafted home decor, kitchen, tableware, storage, checkout, pay.
 *
 * Auto-generated Playwright script for ellementry-decor.
 * Site: https://www.ellementry.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ellementry Decor ──────────────────────────
    log({ step: 'Opening Ellementry Decor...', status: 'running' });
    await page.goto('https://www.ellementry.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ellementry Decor. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Ellementry Decor',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ellementry Decor order',
      details: 'Completing your order on Ellementry Decor',
      amountInr: 0, // Extracted from page at runtime
      description: 'ellementry-decor order',
    });
    ...
    log({ step: 'Ellementry Decor workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ellementry Decor. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ellementry-decor-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'ellementry-decor';
