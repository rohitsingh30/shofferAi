/**
 * Order flowers, cakes, and gifts from Ferns N Petals — select arrangement, set delivery date, recipient address, checkout, pay.
 *
 * Auto-generated Playwright script for ferns-flowers.
 * Site: https://www.fnp.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ferns Flowers ──────────────────────────
    log({ step: 'Opening Ferns Flowers...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ferns Flowers. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Ferns Flowers',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ferns Flowers order',
      details: 'Completing your order on Ferns Flowers',
      amountInr: 0, // Extracted from page at runtime
      description: 'ferns-flowers order',
    });
    ...
    log({ step: 'Ferns Flowers workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ferns Flowers. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ferns-flowers-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'ferns-flowers';
