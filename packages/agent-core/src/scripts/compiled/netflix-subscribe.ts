/**
 * Subscribe to Netflix India. Choose plan (Mobile, Basic, Standard, Premium), enter payment, and activate.
 *
 * Auto-generated Playwright script for netflix-subscribe.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Netflix Subscribe ──────────────────────────
    log({ step: 'Opening Netflix Subscribe...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Netflix Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Netflix Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Netflix Subscribe order',
      details: 'Completing your order on Netflix Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'netflix-subscribe order',
    });
    ...
    log({ step: 'Netflix Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Netflix Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'netflix-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'netflix-subscribe';
