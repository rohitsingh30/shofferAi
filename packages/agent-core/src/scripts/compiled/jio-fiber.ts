/**
 * Subscribe to JioFiber broadband plans — select plan, enter address, schedule installation, pay.
 *
 * Auto-generated Playwright script for jio-fiber.
 * Site: https://www.jio.com/fiber
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Jio Fiber ──────────────────────────
    log({ step: 'Opening Jio Fiber...', status: 'running' });
    await page.goto('https://www.jio.com/fiber');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Jio Fiber. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Jio Fiber order',
      details: 'Completing your order on Jio Fiber',
      amountInr: 0, // Extracted from page at runtime
      description: 'jio-fiber order',
    });
    ...
    log({ step: 'Jio Fiber workflow completed', status: 'completed' });
    log({ message: 'Task completed on Jio Fiber. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'jio-fiber-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'jio-fiber';
