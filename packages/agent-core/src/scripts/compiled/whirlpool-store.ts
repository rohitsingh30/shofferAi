/**
 * Buy Whirlpool appliances on Whirlpool India — refrigerators, washing machines, ACs, microwaves, water purifiers.
 *
 * Auto-generated Playwright script for whirlpool-store.
 * Site: https://www.whirlpoolindia.com/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Whirlpool Store ──────────────────────────
    log({ step: 'Opening Whirlpool Store...', status: 'running' });
    await page.goto('https://www.whirlpoolindia.com/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Whirlpool Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Whirlpool Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Whirlpool Store order',
      details: 'Completing your order on Whirlpool Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'whirlpool-store order',
    });
    ...
    log({ step: 'Whirlpool Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Whirlpool Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'whirlpool-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'whirlpool-store';
