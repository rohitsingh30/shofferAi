/**
 * Manage money on Fi (neobank) — savings account, smart deposits, mutual fund investments, spending insights.
 *
 * Auto-generated Playwright script for fi-money.
 * Site: https://fi.money
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Fi Money ──────────────────────────
    log({ step: 'Opening Fi Money...', status: 'running' });
    await page.goto('https://fi.money');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Fi Money. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Fi Money order',
      details: 'Completing your order on Fi Money',
      amountInr: 0, // Extracted from page at runtime
      description: 'fi-money order',
    });
    ...
    log({ step: 'Fi Money workflow completed', status: 'completed' });
    log({ message: 'Task completed on Fi Money. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'fi-money-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'fi-money';
