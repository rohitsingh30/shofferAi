/**
 * Order sweets, snacks, and dry fruits from Haldiram's online — namkeen, sweets, gift packs, delivery, pay.
 *
 * Auto-generated Playwright script for haldiram-snacks.
 * Site: https://www.haldirams.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Haldiram Snacks ──────────────────────────
    log({ step: 'Opening Haldiram Snacks...', status: 'running' });
    await page.goto('https://www.haldirams.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Haldiram Snacks. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Haldiram Snacks',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Haldiram Snacks order',
      details: 'Completing your order on Haldiram Snacks',
      amountInr: 0, // Extracted from page at runtime
      description: 'haldiram-snacks order',
    });
    ...
    log({ step: 'Haldiram Snacks workflow completed', status: 'completed' });
    log({ message: 'Task completed on Haldiram Snacks. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'haldiram-snacks-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'haldiram-snacks';
