/**
 * Ship packages via DTDC — domestic and international courier, book shipments, track consignments, economy and express options.
 *
 * Auto-generated Playwright script for dtdc-ship.
 * Site: https://www.dtdc.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Dtdc Ship ──────────────────────────
    log({ step: 'Opening Dtdc Ship...', status: 'running' });
    await page.goto('https://www.dtdc.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Dtdc Ship. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Dtdc Ship order',
      details: 'Completing your order on Dtdc Ship',
      amountInr: 0, // Extracted from page at runtime
      description: 'dtdc-ship order',
    });
    ...
    log({ step: 'Dtdc Ship workflow completed', status: 'completed' });
    log({ message: 'Task completed on Dtdc Ship. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'dtdc-ship-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'dtdc-ship';
