/**
 * Track Flipkart order status — find order, show delivery status, courier details, and estimated delivery.
 *
 * Auto-generated Playwright script for flipkart-track.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Flipkart Track ──────────────────────────
    log({ step: 'Opening Flipkart Track...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Flipkart Track. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Flipkart Track',
    ...
    log({ step: 'Flipkart Track workflow completed', status: 'completed' });
    log({ message: 'Task completed on Flipkart Track. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'flipkart-track-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'flipkart-track';
