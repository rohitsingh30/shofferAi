/**
 * Book online doctor consultation on MFine — find specialists, schedule video consult, get prescriptions.
 *
 * Auto-generated Playwright script for mfine-doctor.
 * Site: https://www.mfine.co
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Mfine Doctor ──────────────────────────
    log({ step: 'Opening Mfine Doctor...', status: 'running' });
    await page.goto('https://www.mfine.co');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Mfine Doctor. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Mfine Doctor',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Mfine Doctor order',
      details: 'Completing your order on Mfine Doctor',
      amountInr: 0, // Extracted from page at runtime
      description: 'mfine-doctor order',
    });
    ...
    log({ step: 'Mfine Doctor workflow completed', status: 'completed' });
    log({ message: 'Task completed on Mfine Doctor. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'mfine-doctor-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'mfine-doctor';
