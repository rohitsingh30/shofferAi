/**
 * Recharge Tata Play (Tata Sky) DTH. Browse channel packs, add-ons, and recharge.
 *
 * Auto-generated Playwright script for tatasky-recharge.
 * Site: https://www.tataplay.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Tatasky Recharge ──────────────────────────
    log({ step: 'Opening Tatasky Recharge...', status: 'running' });
    await page.goto('https://www.tataplay.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Tatasky Recharge. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Tatasky Recharge',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Tatasky Recharge order',
      details: 'Completing your order on Tatasky Recharge',
      amountInr: 0, // Extracted from page at runtime
      description: 'tatasky-recharge order',
    });
    ...
    log({ step: 'Tatasky Recharge workflow completed', status: 'completed' });
    log({ message: 'Task completed on Tatasky Recharge. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'tatasky-recharge-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'tatasky-recharge';
