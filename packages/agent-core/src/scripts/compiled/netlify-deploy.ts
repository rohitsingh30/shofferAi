/**
 * Deploy website on Netlify — import from GitHub, configure build settings, set up custom domain, enable HTTPS.
 *
 * Auto-generated Playwright script for netlify-deploy.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Netlify Deploy ──────────────────────────
    log({ step: 'Opening Netlify Deploy...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Netlify Deploy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Netlify Deploy',
    ...
    log({ step: 'Netlify Deploy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Netlify Deploy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'netlify-deploy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'netlify-deploy';
