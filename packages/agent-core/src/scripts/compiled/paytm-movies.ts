/**
 * Book movie tickets on Paytm Movies — select movie, showtime, seats, pay.
 *
 * Auto-generated Playwright script for paytm-movies.
 * Site: https://paytm.com/movies
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Movies ──────────────────────────
    log({ step: 'Opening Paytm Movies...', status: 'running' });
    await page.goto('https://paytm.com/movies');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Movies. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const resultCards = page.locator('[class*="item-card"]');
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Paytm Movies',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Movies order',
      details: 'Completing your order on Paytm Movies',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-movies order',
    });
    ...
    log({ step: 'Paytm Movies workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Movies. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-movies-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-movies';
