/**
 * Order pizza from Mojo Pizza (via EatSure) — unique flavors, combos, sides, checkout, pay.
 *
 * Auto-generated Playwright script for mojo-pizza.
 * Site: https://www.eatsure.com/mojo-pizza
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Mojo Pizza ──────────────────────────
    log({ step: 'Opening Mojo Pizza...', status: 'running' });
    await page.goto('https://www.eatsure.com/mojo-pizza');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Mojo Pizza. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Mojo Pizza',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Mojo Pizza order',
      details: 'Completing your order on Mojo Pizza',
    ...
    log({ step: 'Mojo Pizza workflow completed', status: 'completed' });
    log({ message: 'Task completed on Mojo Pizza. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'mojo-pizza-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'mojo-pizza';
