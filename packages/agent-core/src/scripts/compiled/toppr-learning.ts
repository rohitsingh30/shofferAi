/**
 * Subscribe to Toppr learning platform — K-12, exam prep, select class, plan, subscribe, pay.
 *
 * Auto-generated Playwright script for toppr-learning.
 * Site: https://www.toppr.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Toppr Learning ──────────────────────────
    log({ step: 'Opening Toppr Learning...', status: 'running' });
    await page.goto('https://www.toppr.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Toppr Learning. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Toppr Learning',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Toppr Learning order',
      details: 'Completing your order on Toppr Learning',
      amountInr: 0, // Extracted from page at runtime
      description: 'toppr-learning order',
    });
    ...
    log({ step: 'Toppr Learning workflow completed', status: 'completed' });
    log({ message: 'Task completed on Toppr Learning. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'toppr-learning-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'toppr-learning';
