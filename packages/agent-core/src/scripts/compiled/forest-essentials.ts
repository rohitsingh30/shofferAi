/**
 * Buy luxury Ayurvedic beauty products on Forest Essentials — browse skincare, haircare, body care, select variants, checkout, pay.
 *
 * Auto-generated Playwright script for forest-essentials.
 * Site: https://www.forestessentialsindia.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Forest Essentials ──────────────────────────
    log({ step: 'Opening Forest Essentials...', status: 'running' });
    await page.goto('https://www.forestessentialsindia.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Forest Essentials. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Forest Essentials',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Forest Essentials order',
      details: 'Completing your order on Forest Essentials',
      amountInr: 0, // Extracted from page at runtime
      description: 'forest-essentials order',
    });
    ...
    log({ step: 'Forest Essentials workflow completed', status: 'completed' });
    log({ message: 'Task completed on Forest Essentials. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'forest-essentials-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'forest-essentials';
