/**
 * Search and apply for jobs on Indeed India — filter by role, location, salary, company, and submit applications.
 *
 * Auto-generated Playwright script for indeed-jobs.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Indeed Jobs...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Indeed Jobs. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Indeed Jobs',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Indeed Jobs order',
      details: 'Completing your order on Indeed Jobs',
      amountInr: 0, // Extracted from page at runtime
      description: 'indeed-jobs order',
    });
    ...
    log({ step: 'Indeed Jobs workflow completed', status: 'completed' });
    log({ message: 'Task completed on Indeed Jobs. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'indeed-jobs-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'indeed-jobs';
