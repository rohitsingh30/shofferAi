/**
 * Recharge Airtel prepaid or postpaid, pay Airtel bills on airtel.in. Browse plans and pay.
 *
 * Auto-generated Playwright script for airtel-recharge.
 * Site: https://www.airtel.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Airtel Recharge ──────────────────────────
    log({ step: 'Opening Airtel Recharge...', status: 'running' });
    await page.goto('https://www.airtel.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Airtel Recharge. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Airtel Recharge',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Airtel Recharge order',
      details: 'Completing your order on Airtel Recharge',
      amountInr: 0, // Extracted from page at runtime
      description: 'airtel-recharge order',
    });
    ...
    log({ step: 'Airtel Recharge workflow completed', status: 'completed' });
    log({ message: 'Task completed on Airtel Recharge. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'airtel-recharge-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'airtel-recharge';
