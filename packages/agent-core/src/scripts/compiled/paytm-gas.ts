/**
 * Pay piped gas bill on Paytm. Select gas provider, enter customer ID, fetch bill, and pay.
 *
 * Auto-generated Playwright script for paytm-gas.
 * Site: https://paytm.com/gas-bill-payment
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Gas ──────────────────────────
    log({ step: 'Opening Paytm Gas...', status: 'running' });
    await page.goto('https://paytm.com/gas-bill-payment');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Gas. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Gas order',
      details: 'Completing your order on Paytm Gas',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-gas order',
    });
    ...
    log({ step: 'Paytm Gas workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Gas. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-gas-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-gas';
