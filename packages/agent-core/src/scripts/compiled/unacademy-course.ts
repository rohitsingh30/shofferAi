/**
 * Subscribe to Unacademy Plus for exam prep — UPSC, CAT, JEE, NEET, SSC courses and live classes.
 *
 * Auto-generated Playwright script for unacademy-course.
 * Site: https://unacademy.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Unacademy Course ──────────────────────────
    log({ step: 'Opening Unacademy Course...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Unacademy Course. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Unacademy Course',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Unacademy Course order',
      details: 'Completing your order on Unacademy Course',
      amountInr: 0, // Extracted from page at runtime
      description: 'unacademy-course order',
    });
    ...
    log({ step: 'Unacademy Course workflow completed', status: 'completed' });
    log({ message: 'Task completed on Unacademy Course. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'unacademy-course-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'unacademy-course';
