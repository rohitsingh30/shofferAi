/**
 * Hire a driver on DriveU — hourly, one-way, round-trip, outstation, airport transfer.
 *
 * Auto-generated Playwright script for driveu-driver.
 * Site: https://www.driveu.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Driveu Driver ──────────────────────────
    log({ step: 'Opening Driveu Driver...', status: 'running' });
    await page.goto('https://www.driveu.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Driveu Driver. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Driveu Driver',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Driveu Driver order',
      details: 'Completing your order on Driveu Driver',
      amountInr: 0, // Extracted from page at runtime
      description: 'driveu-driver order',
    });
    ...
    log({ step: 'Driveu Driver workflow completed', status: 'completed' });
    log({ message: 'Task completed on Driveu Driver. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'driveu-driver-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'driveu-driver';
