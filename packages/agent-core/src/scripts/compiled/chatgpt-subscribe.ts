/**
 * Subscribe to ChatGPT Plus or Pro plan on OpenAI. Select plan, enter payment, and activate premium access.
 *
 * Auto-generated Playwright script for chatgpt-subscribe.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Chatgpt Subscribe ──────────────────────────
    log({ step: 'Opening Chatgpt Subscribe...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Chatgpt Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Chatgpt Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Chatgpt Subscribe order',
      details: 'Completing your order on Chatgpt Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'chatgpt-subscribe order',
    });
    ...
    log({ step: 'Chatgpt Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Chatgpt Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'chatgpt-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'chatgpt-subscribe';
