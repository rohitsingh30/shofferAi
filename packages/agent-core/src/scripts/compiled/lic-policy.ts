/**
 * Buy LIC policy online on licindia.in — term insurance, endowment, ULIP, pension, and money-back plans.
 *
 * Auto-generated Playwright script for lic-policy.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Lic Policy ──────────────────────────
    log({ step: 'Opening Lic Policy...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Lic Policy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Lic Policy',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Lic Policy order',
      details: 'Completing your order on Lic Policy',
      amountInr: 0, // Extracted from page at runtime
      description: 'lic-policy order',
    });
    ...
    log({ step: 'Lic Policy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Lic Policy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'lic-policy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'lic-policy';
