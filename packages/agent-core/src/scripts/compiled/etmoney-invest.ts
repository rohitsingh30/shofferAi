/**
 * Invest in mutual funds on ET Money — search funds, compare returns, start SIP or lump sum investment.
 *
 * Auto-generated Playwright script for etmoney-invest.
 * Site: https://www.etmoney.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Etmoney Invest ──────────────────────────
    log({ step: 'Opening Etmoney Invest...', status: 'running' });
    await page.goto('https://www.etmoney.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Etmoney Invest. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Etmoney Invest',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Etmoney Invest order',
      details: 'Completing your order on Etmoney Invest',
      amountInr: 0, // Extracted from page at runtime
      description: 'etmoney-invest order',
    });
    ...
    log({ step: 'Etmoney Invest workflow completed', status: 'completed' });
    log({ message: 'Task completed on Etmoney Invest. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'etmoney-invest-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'etmoney-invest';
