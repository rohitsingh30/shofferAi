/**
 * Find and apply for jobs on Apna — delivery, sales, support, telecalling, data entry, and other entry-level roles.
 *
 * Auto-generated Playwright script for apna-jobs.
 * Site: https://apna.co
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Apna Jobs ──────────────────────────
    log({ step: 'Opening Apna Jobs...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Apna Jobs. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Apna Jobs order',
      details: 'Completing your order on Apna Jobs',
      amountInr: 0, // Extracted from page at runtime
      description: 'apna-jobs order',
    });
    ...
    log({ step: 'Apna Jobs workflow completed', status: 'completed' });
    log({ message: 'Task completed on Apna Jobs. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'apna-jobs-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'apna-jobs';
