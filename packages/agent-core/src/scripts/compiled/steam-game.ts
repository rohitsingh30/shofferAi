/**
 * Buy or gift games on Steam. Search the store, check reviews and pricing, add to cart, and complete purchase.
 *
 * Auto-generated Playwright script for steam-game.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Steam Game ──────────────────────────
    log({ step: 'Opening Steam Game...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Steam Game. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Steam Game',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Steam Game order',
      details: 'Completing your order on Steam Game',
      amountInr: 0, // Extracted from page at runtime
      description: 'steam-game order',
    });
    ...
    log({ step: 'Steam Game workflow completed', status: 'completed' });
    log({ message: 'Task completed on Steam Game. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'steam-game-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'steam-game';
