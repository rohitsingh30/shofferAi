/**
 * Book Vistara flights on airvistara.com — search routes, select premium economy/business, fill details, pay.
 *
 * Auto-generated Playwright script for vistara-flight.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Vistara Flight ──────────────────────────
    log({ step: 'Opening Vistara Flight...', status: 'running' });
    await page.goto('https://www.airvistara.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Vistara Flight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Vistara Flight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Vistara Flight order',
      details: 'Completing your order on Vistara Flight',
      amountInr: 0, // Extracted from page at runtime
      description: 'vistara-flight order',
    });
    ...
    log({ step: 'Vistara Flight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Vistara Flight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'vistara-flight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'vistara-flight';
