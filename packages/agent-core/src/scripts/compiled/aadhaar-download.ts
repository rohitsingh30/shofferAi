/**
 * Download Aadhaar card from UIDAI — enter Aadhaar number, verify via OTP, download e-Aadhaar PDF.
 *
 * Auto-generated Playwright script for aadhaar-download.
 * Site: https://myaadhaar.uidai.gov.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Aadhaar Download ──────────────────────────
    log({ step: 'Opening Aadhaar Download...', status: 'running' });
    await page.goto('https://myaadhaar.uidai.gov.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Aadhaar Download. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Aadhaar Download',
    ...
    log({ step: 'Aadhaar Download workflow completed', status: 'completed' });
    log({ message: 'Task completed on Aadhaar Download. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'aadhaar-download-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'aadhaar-download';
