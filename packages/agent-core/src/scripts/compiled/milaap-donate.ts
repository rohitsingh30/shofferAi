/**
 * Donate to causes on Milaap — social impact crowdfunding for medical, education, livelihood, and community projects in India.
 *
 * Auto-generated Playwright script for milaap-donate.
 * Site: https://www.milaap.org
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Milaap Donate ──────────────────────────
    log({ step: 'Opening Milaap Donate...', status: 'running' });
    await page.goto('https://www.milaap.org');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Milaap Donate. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Milaap Donate',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Milaap Donate order',
      details: 'Completing your order on Milaap Donate',
      amountInr: 0, // Extracted from page at runtime
      description: 'milaap-donate order',
    });
    ...
    log({ step: 'Milaap Donate workflow completed', status: 'completed' });
    log({ message: 'Task completed on Milaap Donate. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'milaap-donate-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'milaap-donate';
