/**
 * Buy an ebook on Amazon Kindle store — search books, preview, purchase and deliver to Kindle device.
 *
 * Auto-generated Playwright script for kindle-ebook.
 * Site: https://www.amazon.in/kindle-dbs/storefront
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Kindle Ebook ──────────────────────────
    log({ step: 'Opening Kindle Ebook...', status: 'running' });
    await page.goto('https://www.amazon.in/kindle-dbs/storefront');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Kindle Ebook. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Kindle Ebook',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Kindle Ebook order',
      details: 'Completing your order on Kindle Ebook',
      amountInr: 0, // Extracted from page at runtime
      description: 'kindle-ebook order',
    });
    ...
    log({ step: 'Kindle Ebook workflow completed', status: 'completed' });
    log({ message: 'Task completed on Kindle Ebook. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'kindle-ebook-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'kindle-ebook';
