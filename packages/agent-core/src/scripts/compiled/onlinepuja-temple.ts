/**
 * Book online puja at temples via OnlinePrasad or DevDarshan — virtual puja, prasad delivery, and temple donations across India.
 *
 * Auto-generated Playwright script for onlinepuja-temple.
 * Site: https://www.onlineprasad.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Onlinepuja Temple ──────────────────────────
    log({ step: 'Opening Onlinepuja Temple...', status: 'running' });
    await page.goto('https://www.onlineprasad.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Onlinepuja Temple. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Onlinepuja Temple',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Onlinepuja Temple order',
      details: 'Completing your order on Onlinepuja Temple',
      amountInr: 0, // Extracted from page at runtime
      description: 'onlinepuja-temple order',
    });
    ...
    log({ step: 'Onlinepuja Temple workflow completed', status: 'completed' });
    log({ message: 'Task completed on Onlinepuja Temple. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'onlinepuja-temple-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'onlinepuja-temple';
