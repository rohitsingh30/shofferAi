/**
 * Sell items on OLX — create listing with photos, price, description, manage buyer enquiries.
 *
 * Auto-generated Playwright script for olx-sell.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Olx Sell ──────────────────────────
    log({ step: 'Opening Olx Sell...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Olx Sell. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Olx Sell order',
      details: 'Completing your order on Olx Sell',
      amountInr: 0, // Extracted from page at runtime
      description: 'olx-sell order',
    });
    ...
    log({ step: 'Olx Sell workflow completed', status: 'completed' });
    log({ message: 'Task completed on Olx Sell. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'olx-sell-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'olx-sell';
