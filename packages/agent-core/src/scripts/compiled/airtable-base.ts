/**
 * Create Airtable bases with tables, fields, views, and records — flexible database and project tracking.
 *
 * Auto-generated Playwright script for airtable-base.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Airtable Base...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Airtable Base. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Airtable Base',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Airtable Base order',
      details: 'Completing your order on Airtable Base',
      amountInr: 0, // Extracted from page at runtime
      description: 'airtable-base order',
    });
    ...
    log({ step: 'Airtable Base workflow completed', status: 'completed' });
    log({ message: 'Task completed on Airtable Base. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'airtable-base-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'airtable-base';
