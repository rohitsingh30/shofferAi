/**
 * Book hostel beds or private rooms on Zostel — backpacker hostels across India, dorms and privates, community stays.
 *
 * Auto-generated Playwright script for zostel-hostel.
 * Site: https://www.zostel.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Zostel Hostel ──────────────────────────
    log({ step: 'Opening Zostel Hostel...', status: 'running' });
    await page.goto('https://www.zostel.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zostel Hostel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zostel Hostel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Zostel Hostel order',
      details: 'Completing your order on Zostel Hostel',
      amountInr: 0, // Extracted from page at runtime
      description: 'zostel-hostel order',
    });
    ...
    log({ step: 'Zostel Hostel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zostel Hostel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'zostel-hostel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'zostel-hostel';
