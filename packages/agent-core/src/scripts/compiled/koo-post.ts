/**
 * Create and publish posts on Koo — Indian multilingual social media platform. Write, attach media, share.
 *
 * Auto-generated Playwright script for koo-post.
 * Site: https://www.kooapp.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Koo Post ──────────────────────────
    log({ step: 'Opening Koo Post...', status: 'running' });
    await page.goto('https://www.kooapp.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Koo Post. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    log({ step: 'Koo Post workflow completed', status: 'completed' });
    log({ message: 'Task completed on Koo Post. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'koo-post-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'koo-post';
