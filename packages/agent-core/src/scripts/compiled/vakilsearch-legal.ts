/**
 * Register a company, trademark, or other legal service on VakilSearch — select service, fill details, submit application with payment.
 *
 * Auto-generated Playwright script for vakilsearch-legal.
 * Site: https://vakilsearch.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Vakilsearch Legal ──────────────────────────
    log({ step: 'Opening Vakilsearch Legal...', status: 'running' });
    await page.goto('https://vakilsearch.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Vakilsearch Legal. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Vakilsearch Legal',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Vakilsearch Legal order',
      details: 'Completing your order on Vakilsearch Legal',
      amountInr: 0, // Extracted from page at runtime
      description: 'vakilsearch-legal order',
    });
    ...
    log({ step: 'Vakilsearch Legal workflow completed', status: 'completed' });
    log({ message: 'Task completed on Vakilsearch Legal. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'vakilsearch-legal-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'vakilsearch-legal';
