/**
 * Order business cards or invitations on Vistaprint — choose design, customize text, select paper/finish, order and pay.
 *
 * Auto-generated Playwright script for vistaprint-cards.
 * Site: https://www.vistaprint.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Vistaprint Cards ──────────────────────────
    log({ step: 'Opening Vistaprint Cards...', status: 'running' });
    await page.goto('https://www.vistaprint.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Vistaprint Cards. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Vistaprint Cards',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Vistaprint Cards order',
      details: 'Completing your order on Vistaprint Cards',
      amountInr: 0, // Extracted from page at runtime
      description: 'vistaprint-cards order',
    });
    ...
    log({ step: 'Vistaprint Cards workflow completed', status: 'completed' });
    log({ message: 'Task completed on Vistaprint Cards. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'vistaprint-cards-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'vistaprint-cards';
