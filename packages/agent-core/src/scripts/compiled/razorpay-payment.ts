/**
 * Create a Razorpay payment link — set amount, description, customer details, and share the payment link.
 *
 * Auto-generated Playwright script for razorpay-payment.
 * Site: https://dashboard.razorpay.com
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Razorpay Payment...', status: 'running' });
    await page.goto('https://dashboard.razorpay.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Razorpay Payment. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Razorpay Payment',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Razorpay Payment order',
      details: 'Completing your order on Razorpay Payment',
      amountInr: 0, // Extracted from page at runtime
      description: 'razorpay-payment order',
    });
    ...
    log({ step: 'Razorpay Payment workflow completed', status: 'completed' });
    log({ message: 'Task completed on Razorpay Payment. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'razorpay-payment-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'razorpay-payment';
