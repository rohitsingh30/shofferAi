/**
 * Buy wellness, ayurvedic, and nutrition products on Tata 1mg — vitamins, immunity boosters, skin care, supplements.
 *
 * Auto-generated Playwright script for tata-1mg-wellness.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Tata 1mg Wellness ──────────────────────────
    log({ step: 'Opening Tata 1mg Wellness...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Tata 1mg Wellness. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Tata 1mg Wellness',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Tata 1mg Wellness order',
      details: 'Completing your order on Tata 1mg Wellness',
    ...
    log({ step: 'Tata 1mg Wellness workflow completed', status: 'completed' });
    log({ message: 'Task completed on Tata 1mg Wellness. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'tata-1mg-wellness-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'tata-1mg-wellness';
