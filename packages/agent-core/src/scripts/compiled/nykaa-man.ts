/**
 * Buy men's grooming products on Nykaa Man — beard care, skincare, perfume, haircare, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for nykaa-man.
 * Site: https://www.nykaaman.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Nykaa Man ──────────────────────────
    log({ step: 'Opening Nykaa Man...', status: 'running' });
    await page.goto('https://www.nykaaman.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Nykaa Man. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Nykaa Man order',
      details: 'Completing your order on Nykaa Man',
      amountInr: 0, // Extracted from page at runtime
      description: 'nykaa-man order',
    });
    ...
    log({ step: 'Nykaa Man workflow completed', status: 'completed' });
    log({ message: 'Task completed on Nykaa Man. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'nykaa-man-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'nykaa-man';
