/**
 * Hire freelancers on Freelancer.com — post a project, receive bids from freelancers, review profiles, and award the project.
 *
 * Auto-generated Playwright script for freelancer-hire.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Freelancer Hire ──────────────────────────
    log({ step: 'Opening Freelancer Hire...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Freelancer Hire. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Freelancer Hire',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Freelancer Hire order',
      details: 'Completing your order on Freelancer Hire',
      amountInr: 0, // Extracted from page at runtime
      description: 'freelancer-hire order',
    });
    ...
    log({ step: 'Freelancer Hire workflow completed', status: 'completed' });
    log({ message: 'Task completed on Freelancer Hire. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'freelancer-hire-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'freelancer-hire';
