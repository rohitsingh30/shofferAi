/**
 * Subscribe to Gaana Plus. Music streaming, podcasts, ad-free listening, and offline downloads.
 *
 * Auto-generated Playwright script for gaana-music.
 * Site: https://gaana.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Gaana Music ──────────────────────────
    log({ step: 'Opening Gaana Music...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Gaana Music. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Gaana Music',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Gaana Music order',
      details: 'Completing your order on Gaana Music',
      amountInr: 0, // Extracted from page at runtime
      description: 'gaana-music order',
    });
    ...
    log({ step: 'Gaana Music workflow completed', status: 'completed' });
    log({ message: 'Task completed on Gaana Music. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'gaana-music-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'gaana-music';
