/**
 * Subscribe to Medium membership — unlimited access to all articles, stories, and publications from writers and experts worldwide.
 *
 * Auto-generated Playwright script for medium-subscribe.
 * Site: https://medium.com/plans
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Medium Subscribe ──────────────────────────
    log({ step: 'Opening Medium Subscribe...', status: 'running' });
    await page.goto('https://medium.com/plans');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Medium Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Medium Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Medium Subscribe order',
      details: 'Completing your order on Medium Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'medium-subscribe order',
    });
    ...
    log({ step: 'Medium Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Medium Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'medium-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'medium-subscribe';
