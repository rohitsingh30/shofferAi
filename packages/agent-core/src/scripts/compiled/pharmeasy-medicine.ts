/**
 * Order medicines on PharmEasy — search by name or upload prescription, add to cart, checkout.
 *
 * Auto-generated Playwright script for pharmeasy-medicine.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Pharmeasy Medicine ──────────────────────────
    log({ step: 'Opening Pharmeasy Medicine...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Pharmeasy Medicine. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Pharmeasy Medicine',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Pharmeasy Medicine order',
      details: 'Completing your order on Pharmeasy Medicine',
    ...
    log({ step: 'Pharmeasy Medicine workflow completed', status: 'completed' });
    log({ message: 'Task completed on Pharmeasy Medicine. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'pharmeasy-medicine-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'pharmeasy-medicine';
