/**
 * Book holiday packages on Thomas Cook India — international/domestic tour packages, flights+hotel combos, guided tours, custom itineraries.
 *
 * Auto-generated Playwright script for thomascook-holiday.
 * Site: https://www.thomascook.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Thomascook Holiday ──────────────────────────
    log({ step: 'Opening Thomascook Holiday...', status: 'running' });
    await page.goto('https://www.thomascook.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Thomascook Holiday. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Thomascook Holiday',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Thomascook Holiday order',
      details: 'Completing your order on Thomascook Holiday',
      amountInr: 0, // Extracted from page at runtime
      description: 'thomascook-holiday order',
    });
    ...
    log({ step: 'Thomascook Holiday workflow completed', status: 'completed' });
    log({ message: 'Task completed on Thomascook Holiday. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'thomascook-holiday-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'thomascook-holiday';
