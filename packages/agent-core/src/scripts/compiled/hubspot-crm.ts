/**
 * Add contacts, create deals, and manage leads in HubSpot CRM — sales pipeline and contact management.
 *
 * Auto-generated Playwright script for hubspot-crm.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Hubspot Crm...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hubspot Crm. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hubspot Crm',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hubspot Crm order',
      details: 'Completing your order on Hubspot Crm',
      amountInr: 0, // Extracted from page at runtime
      description: 'hubspot-crm order',
    });
    ...
    log({ step: 'Hubspot Crm workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hubspot Crm. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hubspot-crm-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hubspot-crm';
