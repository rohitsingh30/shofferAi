/**
 * Buy Star Health insurance — family floater, individual, senior citizen, and accident care health plans.
 *
 * Auto-generated Playwright script for star-health-insurance.
 * Site: https://www.starhealth.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Star Health Insurance ──────────────────────────
    log({ step: 'Opening Star Health Insurance...', status: 'running' });
    await page.goto('https://www.starhealth.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Star Health Insurance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Star Health Insurance',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Star Health Insurance order',
      details: 'Completing your order on Star Health Insurance',
      amountInr: 0, // Extracted from page at runtime
      description: 'star-health-insurance order',
    });
    ...
    log({ step: 'Star Health Insurance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Star Health Insurance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'star-health-insurance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'star-health-insurance';
