/**
 * Shop on Crocs India — browse, select size, buy clogs, sandals, slides, and footwear, checkout, pay.
 *
 * Auto-generated Playwright script for crocs-shoes.
 * Site: https://www.crocs.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Crocs Shoes ──────────────────────────
    log({ step: 'Opening Crocs Shoes...', status: 'running' });
    await page.goto('https://www.crocs.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Crocs Shoes. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Crocs Shoes',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Crocs Shoes order',
      details: 'Completing your order on Crocs Shoes',
      amountInr: 0, // Extracted from page at runtime
      description: 'crocs-shoes order',
    });
    ...
    log({ step: 'Crocs Shoes workflow completed', status: 'completed' });
    log({ message: 'Task completed on Crocs Shoes. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'crocs-shoes-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'crocs-shoes';
