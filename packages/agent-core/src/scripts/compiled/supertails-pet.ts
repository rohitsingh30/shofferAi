/**
 * Order pet food and supplies from Supertails — dog/cat food, accessories, medicines, grooming, checkout, pay.
 *
 * Auto-generated Playwright script for supertails-pet.
 * Site: https://supertails.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Supertails Pet ──────────────────────────
    log({ step: 'Opening Supertails Pet...', status: 'running' });
    await page.goto('https://supertails.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Supertails Pet. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Supertails Pet',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Supertails Pet order',
      details: 'Completing your order on Supertails Pet',
      amountInr: 0, // Extracted from page at runtime
      description: 'supertails-pet order',
    });
    ...
    log({ step: 'Supertails Pet workflow completed', status: 'completed' });
    log({ message: 'Task completed on Supertails Pet. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'supertails-pet-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'supertails-pet';
