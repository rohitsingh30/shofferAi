/**
 * Order food from Pizza Hut India — browse menu, customize pizza, add sides, checkout, pay.
 *
 * Auto-generated Playwright script for pizzahut-order.
 * Site: https://www.pizzahut.co.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Pizzahut Order ──────────────────────────
    log({ step: 'Opening Pizzahut Order...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Pizzahut Order. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Pizzahut Order',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Pizzahut Order order',
      details: 'Completing your order on Pizzahut Order',
      amountInr: 0, // Extracted from page at runtime
      description: 'pizzahut-order order',
    });
    ...
    log({ step: 'Pizzahut Order workflow completed', status: 'completed' });
    log({ message: 'Task completed on Pizzahut Order. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'pizzahut-order-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'pizzahut-order';
