/**
 * Book movie tickets on BookMyShow — pick movie, cinema, showtime, seats, pay.
 *
 * Auto-generated Playwright script for bookmyshow-movie.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bookmyshow Movie ──────────────────────────
    log({ step: 'Opening Bookmyshow Movie...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bookmyshow Movie. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bookmyshow Movie',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bookmyshow Movie order',
      details: 'Completing your order on Bookmyshow Movie',
      amountInr: 0, // Extracted from page at runtime
      description: 'bookmyshow-movie order',
    });
    ...
    log({ step: 'Bookmyshow Movie workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bookmyshow Movie. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bookmyshow-movie-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bookmyshow-movie';
