/**
 * Pay traffic challan/fine on eChallan Parivahan — search by vehicle or challan number, view violations, pay online.
 *
 * Auto-generated Playwright script for echallan-fine.
 * Site: https://echallan.parivahan.gov.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Echallan Fine ──────────────────────────
    log({ step: 'Opening Echallan Fine...', status: 'running' });
    await page.goto('https://echallan.parivahan.gov.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Echallan Fine. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Echallan Fine',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Echallan Fine order',
      details: 'Completing your order on Echallan Fine',
      amountInr: 0, // Extracted from page at runtime
      description: 'echallan-fine order',
    });
    ...
    log({ step: 'Echallan Fine workflow completed', status: 'completed' });
    log({ message: 'Task completed on Echallan Fine. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'echallan-fine-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'echallan-fine';
