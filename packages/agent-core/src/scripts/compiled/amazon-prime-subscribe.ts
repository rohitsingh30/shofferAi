/**
 * Subscribe to Amazon Prime India. Choose annual or monthly plan, pay and activate.
 *
 * Auto-generated Playwright script for amazon-prime-subscribe.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Amazon Prime Subscribe ──────────────────────────
    log({ step: 'Opening Amazon Prime Subscribe...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Prime Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Prime Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Amazon Prime Subscribe order',
      details: 'Completing your order on Amazon Prime Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'amazon-prime-subscribe order',
    });
    ...
    log({ step: 'Amazon Prime Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Prime Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-prime-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-prime-subscribe';
