/**
 * Buy baby products on Babyoye/FirstCry — diapers, baby food, gear, clothes, toys, checkout, pay.
 *
 * Auto-generated Playwright script for babyoye-baby.
 * Site: https://www.babyoye.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Babyoye Baby ──────────────────────────
    log({ step: 'Opening Babyoye Baby...', status: 'running' });
    await page.goto('https://www.babyoye.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Babyoye Baby. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Babyoye Baby',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Babyoye Baby order',
      details: 'Completing your order on Babyoye Baby',
      amountInr: 0, // Extracted from page at runtime
      description: 'babyoye-baby order',
    });
    ...
    log({ step: 'Babyoye Baby workflow completed', status: 'completed' });
    log({ message: 'Task completed on Babyoye Baby. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'babyoye-baby-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'babyoye-baby';
