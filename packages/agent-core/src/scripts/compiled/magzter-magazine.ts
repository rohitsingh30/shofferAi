/**
 * Subscribe to digital magazines on Magzter — browse All-Access Gold plan or buy individual magazine issues.
 *
 * Auto-generated Playwright script for magzter-magazine.
 * Site: https://www.magzter.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Magzter Magazine ──────────────────────────
    log({ step: 'Opening Magzter Magazine...', status: 'running' });
    await page.goto('https://www.magzter.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Magzter Magazine. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Magzter Magazine',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Magzter Magazine order',
      details: 'Completing your order on Magzter Magazine',
      amountInr: 0, // Extracted from page at runtime
      description: 'magzter-magazine order',
    });
    ...
    log({ step: 'Magzter Magazine workflow completed', status: 'completed' });
    log({ message: 'Task completed on Magzter Magazine. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'magzter-magazine-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'magzter-magazine';
