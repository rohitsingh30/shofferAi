/**
 * Book pest control on Urban Company — cockroach, termite, bed bugs, mosquito, rat control for homes.
 *
 * Auto-generated Playwright script for urbancompany-pest.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbancompany Pest ──────────────────────────
    log({ step: 'Opening Urbancompany Pest...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbancompany Pest. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbancompany Pest',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbancompany Pest order',
      details: 'Completing your order on Urbancompany Pest',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbancompany-pest order',
    });
    ...
    log({ step: 'Urbancompany Pest workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbancompany Pest. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbancompany-pest-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbancompany-pest';
