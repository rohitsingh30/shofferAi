/**
 * Book flights on EaseMyTrip — budget-friendly airline search, zero convenience fee, compare and book cheapest flights.
 *
 * Auto-generated Playwright script for easemytrip-flight.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Easemytrip Flight ──────────────────────────
    log({ step: 'Opening Easemytrip Flight...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Easemytrip Flight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Easemytrip Flight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Easemytrip Flight order',
      details: 'Completing your order on Easemytrip Flight',
      amountInr: 0, // Extracted from page at runtime
      description: 'easemytrip-flight order',
    });
    ...
    log({ step: 'Easemytrip Flight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Easemytrip Flight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'easemytrip-flight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'easemytrip-flight';
