/**
 * Book passport appointment on Passport Seva — select office, choose date/slot, fill application, pay fee.
 *
 * Auto-generated Playwright script for passport-appointment.
 * Site: https://www.passportindia.gov.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Passport Appointment ──────────────────────────
    log({ step: 'Opening Passport Appointment...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Passport Appointment. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Passport Appointment',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Passport Appointment order',
      details: 'Completing your order on Passport Appointment',
      amountInr: 0, // Extracted from page at runtime
      description: 'passport-appointment order',
    });
    ...
    log({ step: 'Passport Appointment workflow completed', status: 'completed' });
    log({ message: 'Task completed on Passport Appointment. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'passport-appointment-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'passport-appointment';
