/**
 * Buy PS5 and PS4 games on PlayStation Store India. Browse, purchase digital games, DLC, and PS Plus subscriptions.
 *
 * Auto-generated Playwright script for playstation-store.
 * Site: https://store.playstation.com/en-in
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Playstation Store...', status: 'running' });
    await page.goto('https://store.playstation.com/en-in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Playstation Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Playstation Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Playstation Store order',
      details: 'Completing your order on Playstation Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'playstation-store order',
    });
    ...
    log({ step: 'Playstation Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Playstation Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'playstation-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'playstation-store';
