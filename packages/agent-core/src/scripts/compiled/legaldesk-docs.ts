/**
 * Create legal documents on LegalDesk — rent agreements, affidavits, power of attorney, wills, and other notarized documents with doorstep delivery.
 *
 * Auto-generated Playwright script for legaldesk-docs.
 * Site: https://legaldesk.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Legaldesk Docs ──────────────────────────
    log({ step: 'Opening Legaldesk Docs...', status: 'running' });
    await page.goto('https://legaldesk.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Legaldesk Docs. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Legaldesk Docs',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Legaldesk Docs order',
      details: 'Completing your order on Legaldesk Docs',
      amountInr: 0, // Extracted from page at runtime
      description: 'legaldesk-docs order',
    });
    ...
    log({ step: 'Legaldesk Docs workflow completed', status: 'completed' });
    log({ message: 'Task completed on Legaldesk Docs. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'legaldesk-docs-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'legaldesk-docs';
