/**
 * Book hotels on Cleartrip — search by destination, compare options, select room, fill details, pay.
 *
 * Auto-generated Playwright script for cleartrip-hotel.
 * Site: https://www.cleartrip.com/hotels
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cleartrip Hotel ──────────────────────────
    log({ step: 'Opening Cleartrip Hotel...', status: 'running' });
    await page.goto('https://www.cleartrip.com/hotels');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cleartrip Hotel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cleartrip Hotel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cleartrip Hotel order',
      details: 'Completing your order on Cleartrip Hotel',
      amountInr: 0, // Extracted from page at runtime
      description: 'cleartrip-hotel order',
    });
    ...
    log({ step: 'Cleartrip Hotel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cleartrip Hotel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cleartrip-hotel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cleartrip-hotel';
