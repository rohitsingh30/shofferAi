/**
 * Research and buy new or used cars on CarDekho — compare specs, check prices, calculate EMI, find dealers, book test drive.
 *
 * Auto-generated Playwright script for cardekho-car.
 * Site: https://www.cardekho.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cardekho Car ──────────────────────────
    log({ step: 'Opening Cardekho Car...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cardekho Car. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cardekho Car',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cardekho Car order',
      details: 'Completing your order on Cardekho Car',
      amountInr: 0, // Extracted from page at runtime
      description: 'cardekho-car order',
    });
    ...
    log({ step: 'Cardekho Car workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cardekho Car. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cardekho-car-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cardekho-car';
