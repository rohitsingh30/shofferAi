/**
 * Book gym session, yoga class, or fitness class on cult.fit — select center, class type, time slot, book.
 *
 * Auto-generated Playwright script for cultfit-fitness.
 * Site: https://www.cult.fit
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cultfit Fitness ──────────────────────────
    log({ step: 'Opening Cultfit Fitness...', status: 'running' });
    await page.goto('https://www.cult.fit');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cultfit Fitness. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cultfit Fitness',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cultfit Fitness order',
      details: 'Completing your order on Cultfit Fitness',
      amountInr: 0, // Extracted from page at runtime
      description: 'cultfit-fitness order',
    });
    ...
    log({ step: 'Cultfit Fitness workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cultfit Fitness. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cultfit-fitness-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cultfit-fitness';
