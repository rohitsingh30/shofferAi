/**
 * Shop on Tata CLiQ — buy electronics, fashion, luxury brands, redeem Tata Neu coins, checkout, pay.
 *
 * Auto-generated Playwright script for tatacliq-shopping.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Tatacliq Shopping ──────────────────────────
    log({ step: 'Opening Tatacliq Shopping...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Tatacliq Shopping. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Tatacliq Shopping',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Tatacliq Shopping order',
      details: 'Completing your order on Tatacliq Shopping',
      amountInr: 0, // Extracted from page at runtime
      description: 'tatacliq-shopping order',
    });
    ...
    log({ step: 'Tatacliq Shopping workflow completed', status: 'completed' });
    log({ message: 'Task completed on Tatacliq Shopping. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'tatacliq-shopping-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'tatacliq-shopping';
