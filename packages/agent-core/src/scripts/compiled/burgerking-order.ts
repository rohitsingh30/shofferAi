/**
 * Order food from Burger King India — browse menu, combos, customize burgers, add sides, checkout, pay.
 *
 * Auto-generated Playwright script for burgerking-order.
 * Site: https://www.burgerking.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Burgerking Order ──────────────────────────
    log({ step: 'Opening Burgerking Order...', status: 'running' });
    await page.goto('https://www.burgerking.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Burgerking Order. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Burgerking Order',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Burgerking Order order',
      details: 'Completing your order on Burgerking Order',
      amountInr: 0, // Extracted from page at runtime
      description: 'burgerking-order order',
    });
    ...
    log({ step: 'Burgerking Order workflow completed', status: 'completed' });
    log({ message: 'Task completed on Burgerking Order. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'burgerking-order-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'burgerking-order';
