/**
 * Buy kids fashion on Myntra Kids section — browse, select size/color, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for myntra-kids.
 * Site: https://www.myntra.com/kids
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Myntra Kids ──────────────────────────
    log({ step: 'Opening Myntra Kids...', status: 'running' });
    await page.goto('https://www.myntra.com/kids');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Myntra Kids. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Myntra Kids',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Myntra Kids order',
      details: 'Completing your order on Myntra Kids',
      amountInr: 0, // Extracted from page at runtime
      description: 'myntra-kids order',
    });
    ...
    log({ step: 'Myntra Kids workflow completed', status: 'completed' });
    log({ message: 'Task completed on Myntra Kids. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'myntra-kids-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'myntra-kids';
