/**
 * Book coworking on 91springboard — community-driven spaces with desks, offices, events, and networking.
 *
 * Auto-generated Playwright script for 91springboard-cowork.
 * Site: https://www.91springboard.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to 91springboard Cowork ──────────────────────────
    log({ step: 'Opening 91springboard Cowork...', status: 'running' });
    await page.goto('https://www.91springboard.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on 91springboard Cowork. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with 91springboard Cowork',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete 91springboard Cowork order',
      details: 'Completing your order on 91springboard Cowork',
      amountInr: 0, // Extracted from page at runtime
      description: '91springboard-cowork order',
    });
    ...
    log({ step: '91springboard Cowork workflow completed', status: 'completed' });
    log({ message: 'Task completed on 91springboard Cowork. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, '91springboard-cowork-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = '91springboard-cowork';
