/**
 * Shop luxury Indian designer fashion on Aza Fashions — browse couture, bridal, festive, and contemporary designer wear, checkout, pay.
 *
 * Auto-generated Playwright script for aza-fashion.
 * Site: https://www.azafashions.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Aza Fashion ──────────────────────────
    log({ step: 'Opening Aza Fashion...', status: 'running' });
    await page.goto('https://www.azafashions.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Aza Fashion. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Aza Fashion',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Aza Fashion order',
      details: 'Completing your order on Aza Fashion',
      amountInr: 0, // Extracted from page at runtime
      description: 'aza-fashion order',
    });
    ...
    log({ step: 'Aza Fashion workflow completed', status: 'completed' });
    log({ message: 'Task completed on Aza Fashion. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'aza-fashion-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'aza-fashion';
