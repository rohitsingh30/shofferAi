/**
 * Ship packages via BlueDart — premium express courier, domestic and international, priority delivery, track shipments.
 *
 * Auto-generated Playwright script for bluedart-ship.
 * Site: https://www.bluedart.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bluedart Ship ──────────────────────────
    log({ step: 'Opening Bluedart Ship...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bluedart Ship. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bluedart Ship',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bluedart Ship order',
      details: 'Completing your order on Bluedart Ship',
      amountInr: 0, // Extracted from page at runtime
      description: 'bluedart-ship order',
    });
    ...
    log({ step: 'Bluedart Ship workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bluedart Ship. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bluedart-ship-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bluedart-ship';
