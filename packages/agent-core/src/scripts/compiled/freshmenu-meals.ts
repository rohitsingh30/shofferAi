/**
 * Order chef-cooked meals from FreshMenu — browse daily menu, bowls, wraps, desserts, delivery, pay.
 *
 * Auto-generated Playwright script for freshmenu-meals.
 * Site: https://www.freshmenu.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Freshmenu Meals ──────────────────────────
    log({ step: 'Opening Freshmenu Meals...', status: 'running' });
    await page.goto('https://www.freshmenu.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Freshmenu Meals. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Freshmenu Meals',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Freshmenu Meals order',
      details: 'Completing your order on Freshmenu Meals',
      amountInr: 0, // Extracted from page at runtime
      description: 'freshmenu-meals order',
    });
    ...
    log({ step: 'Freshmenu Meals workflow completed', status: 'completed' });
    log({ message: 'Task completed on Freshmenu Meals. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'freshmenu-meals-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'freshmenu-meals';
