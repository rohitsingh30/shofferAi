/**
 * Create personalized photo gifts on Shutterfly — mugs, calendars, prints, cards, blankets, phone cases.
 *
 * Auto-generated Playwright script for shutterfly-photo.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Shutterfly Photo ──────────────────────────
    log({ step: 'Opening Shutterfly Photo...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Shutterfly Photo. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Shutterfly Photo',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Shutterfly Photo order',
      details: 'Completing your order on Shutterfly Photo',
      amountInr: 0, // Extracted from page at runtime
      description: 'shutterfly-photo order',
    });
    ...
    log({ step: 'Shutterfly Photo workflow completed', status: 'completed' });
    log({ message: 'Task completed on Shutterfly Photo. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'shutterfly-photo-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'shutterfly-photo';
