/**
 * Shop on Campus Shoes — browse, select size, buy running, training, and sports shoes, checkout, pay.
 *
 * Auto-generated Playwright script for campus-shoes.
 * Site: https://www.campusshoes.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Campus Shoes ──────────────────────────
    log({ step: 'Opening Campus Shoes...', status: 'running' });
    await page.goto('https://www.campusshoes.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Campus Shoes. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Campus Shoes',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Campus Shoes order',
      details: 'Completing your order on Campus Shoes',
    ...
    log({ step: 'Campus Shoes workflow completed', status: 'completed' });
    log({ message: 'Task completed on Campus Shoes. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'campus-shoes-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'campus-shoes';
