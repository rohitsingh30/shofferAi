/**
 * Shop on SleepyCat — browse mattresses, select size/type, check trial offer, checkout, pay.
 *
 * Auto-generated Playwright script for sleepycat-mattress.
 * Site: https://www.sleepycat.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Sleepycat Mattress ──────────────────────────
    log({ step: 'Opening Sleepycat Mattress...', status: 'running' });
    await page.goto('https://www.sleepycat.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Sleepycat Mattress. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Sleepycat Mattress',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Sleepycat Mattress order',
      details: 'Completing your order on Sleepycat Mattress',
    ...
    log({ step: 'Sleepycat Mattress workflow completed', status: 'completed' });
    log({ message: 'Task completed on Sleepycat Mattress. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'sleepycat-mattress-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'sleepycat-mattress';
