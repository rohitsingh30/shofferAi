/**
 * Buy certified organic groceries from Organic Tattva — dal, rice, spices, flours, oils, and pantry essentials.
 *
 * Auto-generated Playwright script for organic-tattva.
 * Site: https://www.organictattva.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Organic Tattva ──────────────────────────
    log({ step: 'Opening Organic Tattva...', status: 'running' });
    await page.goto('https://www.organictattva.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Organic Tattva. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Organic Tattva',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Organic Tattva order',
      details: 'Completing your order on Organic Tattva',
      amountInr: 0, // Extracted from page at runtime
      description: 'organic-tattva order',
    });
    ...
    log({ step: 'Organic Tattva workflow completed', status: 'completed' });
    log({ message: 'Task completed on Organic Tattva. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'organic-tattva-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'organic-tattva';
