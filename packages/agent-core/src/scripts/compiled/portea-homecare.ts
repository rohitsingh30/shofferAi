/**
 * Book home healthcare services on Portea — nursing care, physiotherapy, lab tests, doctor visits at home.
 *
 * Auto-generated Playwright script for portea-homecare.
 * Site: https://www.portea.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Portea Homecare ──────────────────────────
    log({ step: 'Opening Portea Homecare...', status: 'running' });
    await page.goto('https://www.portea.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Portea Homecare. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Portea Homecare',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Portea Homecare order',
      details: 'Completing your order on Portea Homecare',
      amountInr: 0, // Extracted from page at runtime
      description: 'portea-homecare order',
    });
    ...
    log({ step: 'Portea Homecare workflow completed', status: 'completed' });
    log({ message: 'Task completed on Portea Homecare. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'portea-homecare-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'portea-homecare';
