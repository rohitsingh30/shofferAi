/**
 * Book India Post services online — Speed Post, Registered Post, track consignments, book pickups, calculate postage rates.
 *
 * Auto-generated Playwright script for indiapost-ship.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Indiapost Ship ──────────────────────────
    log({ step: 'Opening Indiapost Ship...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Indiapost Ship. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Indiapost Ship',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Indiapost Ship order',
      details: 'Completing your order on Indiapost Ship',
      amountInr: 0, // Extracted from page at runtime
      description: 'indiapost-ship order',
    });
    ...
    log({ step: 'Indiapost Ship workflow completed', status: 'completed' });
    log({ message: 'Task completed on Indiapost Ship. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'indiapost-ship-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'indiapost-ship';
