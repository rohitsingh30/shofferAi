/**
 * Find local services on Quikr — search home services, compare providers, get quotes, contact or book.
 *
 * Auto-generated Playwright script for quikr-service.
 * Site: https://www.quikr.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Quikr Service ──────────────────────────
    log({ step: 'Opening Quikr Service...', status: 'running' });
    await page.goto('https://www.quikr.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Quikr Service. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Quikr Service',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Quikr Service order',
      details: 'Completing your order on Quikr Service',
      amountInr: 0, // Extracted from page at runtime
      description: 'quikr-service order',
    });
    ...
    log({ step: 'Quikr Service workflow completed', status: 'completed' });
    log({ message: 'Task completed on Quikr Service. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'quikr-service-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'quikr-service';
