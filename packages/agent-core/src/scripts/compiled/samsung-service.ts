/**
 * Book Samsung device service or repair on samsung.com — phone, TV, refrigerator, washing machine, AC repair.
 *
 * Auto-generated Playwright script for samsung-service.
 * Site: https://www.samsung.com/in/support/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Samsung Service ──────────────────────────
    log({ step: 'Opening Samsung Service...', status: 'running' });
    await page.goto('https://www.samsung.com/in/support/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Samsung Service. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Samsung Service',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Samsung Service order',
      details: 'Completing your order on Samsung Service',
      amountInr: 0, // Extracted from page at runtime
      description: 'samsung-service order',
    });
    ...
    log({ step: 'Samsung Service workflow completed', status: 'completed' });
    log({ message: 'Task completed on Samsung Service. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'samsung-service-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'samsung-service';
