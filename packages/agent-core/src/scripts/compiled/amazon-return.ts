/**
 * Return a product on Amazon.in — find order, select return reason, choose refund method, schedule pickup or drop-off.
 *
 * Auto-generated Playwright script for amazon-return.
 * Site: https://www.amazon.in/gp/your-account/order-history
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Amazon Return ──────────────────────────
    log({ step: 'Opening Amazon Return...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Return. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Return',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Amazon Return order',
      details: 'Completing your order on Amazon Return',
      amountInr: 0, // Extracted from page at runtime
      description: 'amazon-return order',
    });
    ...
    log({ step: 'Amazon Return workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Return. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-return-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-return';
