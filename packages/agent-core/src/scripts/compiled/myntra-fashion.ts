/**
 * Shop fashion on Myntra — browse clothing, shoes, accessories, add to bag, checkout, pay.
 *
 * Auto-generated Playwright script for myntra-fashion.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Myntra Fashion ──────────────────────────
    log({ step: 'Opening Myntra Fashion...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Myntra Fashion. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Myntra Fashion',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Myntra Fashion order',
      details: 'Completing your order on Myntra Fashion',
    ...
    log({ step: 'Myntra Fashion workflow completed', status: 'completed' });
    log({ message: 'Task completed on Myntra Fashion. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'myntra-fashion-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'myntra-fashion';
