/**
 * Register a company, file compliance, or get legal services on IndiaFilings — company incorporation, annual filings, trademark, ISO certification.
 *
 * Auto-generated Playwright script for indiafilings-legal.
 * Site: https://www.indiafilings.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Indiafilings Legal ──────────────────────────
    log({ step: 'Opening Indiafilings Legal...', status: 'running' });
    await page.goto('https://www.indiafilings.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Indiafilings Legal. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Indiafilings Legal',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Indiafilings Legal order',
      details: 'Completing your order on Indiafilings Legal',
      amountInr: 0, // Extracted from page at runtime
      description: 'indiafilings-legal order',
    });
    ...
    log({ step: 'Indiafilings Legal workflow completed', status: 'completed' });
    log({ message: 'Task completed on Indiafilings Legal. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'indiafilings-legal-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'indiafilings-legal';
