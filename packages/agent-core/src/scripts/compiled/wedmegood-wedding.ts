/**
 * Find and book wedding vendors on WedMeGood — photographers, venues, decorators, makeup artists, caterers.
 *
 * Auto-generated Playwright script for wedmegood-wedding.
 * Site: https://www.wedmegood.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wedmegood Wedding ──────────────────────────
    log({ step: 'Opening Wedmegood Wedding...', status: 'running' });
    await page.goto('https://www.wedmegood.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wedmegood Wedding. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Wedmegood Wedding',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wedmegood Wedding order',
      details: 'Completing your order on Wedmegood Wedding',
      amountInr: 0, // Extracted from page at runtime
      description: 'wedmegood-wedding order',
    });
    ...
    log({ step: 'Wedmegood Wedding workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wedmegood Wedding. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wedmegood-wedding-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wedmegood-wedding';
