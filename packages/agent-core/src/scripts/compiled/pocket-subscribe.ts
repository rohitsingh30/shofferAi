/**
 * Subscribe to Pocket Premium — save articles, videos, and web pages to read later with permanent library, full-text search, and ad-free reading.
 *
 * Auto-generated Playwright script for pocket-subscribe.
 * Site: https://getpocket.com/premium
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Pocket Subscribe ──────────────────────────
    log({ step: 'Opening Pocket Subscribe...', status: 'running' });
    await page.goto('https://getpocket.com/premium');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Pocket Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Pocket Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Pocket Subscribe order',
      details: 'Completing your order on Pocket Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'pocket-subscribe order',
    });
    ...
    log({ step: 'Pocket Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Pocket Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'pocket-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'pocket-subscribe';
