/**
 * Book home deep cleaning on Urban Company. Full home, bathroom, kitchen, sofa — by BHK size and schedule.
 *
 * Auto-generated Playwright script for urbancompany-cleaning.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbancompany Cleaning ──────────────────────────
    log({ step: 'Opening Urbancompany Cleaning...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbancompany Cleaning. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbancompany Cleaning',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbancompany Cleaning order',
      details: 'Completing your order on Urbancompany Cleaning',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbancompany-cleaning order',
    });
    ...
    log({ step: 'Urbancompany Cleaning workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbancompany Cleaning. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbancompany-cleaning-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbancompany-cleaning';
