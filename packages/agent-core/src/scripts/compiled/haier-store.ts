/**
 * Buy Haier appliances on Haier India — ACs, washing machines, refrigerators, water heaters, deep freezers.
 *
 * Auto-generated Playwright script for haier-store.
 * Site: https://www.haier.com/in/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Haier Store ──────────────────────────
    log({ step: 'Opening Haier Store...', status: 'running' });
    await page.goto('https://www.haier.com/in/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Haier Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Haier Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Haier Store order',
      details: 'Completing your order on Haier Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'haier-store order',
    });
    ...
    log({ step: 'Haier Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Haier Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'haier-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'haier-store';
