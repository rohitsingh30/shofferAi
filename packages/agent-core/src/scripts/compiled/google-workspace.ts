/**
 * Create and manage Google Workspace documents — Docs, Sheets, Slides, Forms — with templates and sharing.
 *
 * Auto-generated Playwright script for google-workspace.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Google Workspace...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Google Workspace. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Google Workspace',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Google Workspace order',
      details: 'Completing your order on Google Workspace',
      amountInr: 0, // Extracted from page at runtime
      description: 'google-workspace order',
    });
    ...
    log({ step: 'Google Workspace workflow completed', status: 'completed' });
    log({ message: 'Task completed on Google Workspace. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'google-workspace-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'google-workspace';
