/**
 * Book car service or repair on GoMechanic — periodic service, AC repair, denting, battery, tyres.
 *
 * Auto-generated Playwright script for gomechanic-car.
 * Site: https://www.gomechanic.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Gomechanic Car ──────────────────────────
    log({ step: 'Opening Gomechanic Car...', status: 'running' });
    await page.goto('https://www.gomechanic.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Gomechanic Car. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Gomechanic Car',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Gomechanic Car order',
      details: 'Completing your order on Gomechanic Car',
      amountInr: 0, // Extracted from page at runtime
      description: 'gomechanic-car order',
    });
    ...
    log({ step: 'Gomechanic Car workflow completed', status: 'completed' });
    log({ message: 'Task completed on Gomechanic Car. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'gomechanic-car-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'gomechanic-car';
