/**
 * Buy Bajaj appliances on Bajaj Electricals — fans, heaters, coolers, kitchen appliances, water heaters, lighting.
 *
 * Auto-generated Playwright script for bajaj-appliances.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bajaj Appliances ──────────────────────────
    log({ step: 'Opening Bajaj Appliances...', status: 'running' });
    await page.goto('https://www.bajajelectricals.com/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bajaj Appliances. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bajaj Appliances',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bajaj Appliances order',
      details: 'Completing your order on Bajaj Appliances',
      amountInr: 0, // Extracted from page at runtime
      description: 'bajaj-appliances order',
    });
    ...
    log({ step: 'Bajaj Appliances workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bajaj Appliances. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bajaj-appliances-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bajaj-appliances';
