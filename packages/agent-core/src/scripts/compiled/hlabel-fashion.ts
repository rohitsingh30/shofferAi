/**
 * Buy H&M fashion on hm.com/in — browse clothing, select size and color, add to bag, checkout, pay.
 *
 * Auto-generated Playwright script for hlabel-fashion.
 * Site: https://www2.hm.com/en_in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hlabel Fashion ──────────────────────────
    log({ step: 'Opening Hlabel Fashion...', status: 'running' });
    await page.goto('https://www2.hm.com/en_in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hlabel Fashion. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hlabel Fashion',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hlabel Fashion order',
      details: 'Completing your order on Hlabel Fashion',
      amountInr: 0, // Extracted from page at runtime
      description: 'hlabel-fashion order',
    });
    ...
    log({ step: 'Hlabel Fashion workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hlabel Fashion. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hlabel-fashion-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hlabel-fashion';
