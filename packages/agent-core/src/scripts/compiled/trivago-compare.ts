/**
 * Compare hotel prices across booking sites on Trivago — find cheapest deal, redirect to best site, book.
 *
 * Auto-generated Playwright script for trivago-compare.
 * Site: https://www.trivago.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Trivago Compare ──────────────────────────
    log({ step: 'Opening Trivago Compare...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Trivago Compare. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Trivago Compare',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Trivago Compare order',
      details: 'Completing your order on Trivago Compare',
      amountInr: 0, // Extracted from page at runtime
      description: 'trivago-compare order',
    });
    ...
    log({ step: 'Trivago Compare workflow completed', status: 'completed' });
    log({ message: 'Task completed on Trivago Compare. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'trivago-compare-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'trivago-compare';
