/**
 * Book home painting service on Urban Company — select room size, paint type, color, schedule painters, pay.
 *
 * Auto-generated Playwright script for urbancompany-painting.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbancompany Painting ──────────────────────────
    log({ step: 'Opening Urbancompany Painting...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbancompany Painting. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbancompany Painting',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbancompany Painting order',
      details: 'Completing your order on Urbancompany Painting',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbancompany-painting order',
    });
    ...
    log({ step: 'Urbancompany Painting workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbancompany Painting. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbancompany-painting-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbancompany-painting';
