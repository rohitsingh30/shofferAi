/**
 * Buy jewelry on CaratLane — browse gold, diamond, and gemstone jewelry, try-on virtually, order and pay.
 *
 * Auto-generated Playwright script for caratlane-jewelry.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Caratlane Jewelry ──────────────────────────
    log({ step: 'Opening Caratlane Jewelry...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Caratlane Jewelry. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Caratlane Jewelry',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Caratlane Jewelry order',
      details: 'Completing your order on Caratlane Jewelry',
    ...
    log({ step: 'Caratlane Jewelry workflow completed', status: 'completed' });
    log({ message: 'Task completed on Caratlane Jewelry. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'caratlane-jewelry-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'caratlane-jewelry';
