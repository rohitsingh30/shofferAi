/**
 * Create X/Twitter ad campaign — promoted tweets, follower growth, website traffic with targeting and budget controls.
 *
 * Auto-generated Playwright script for twitter-ads.
 * Site: https://ads.x.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Twitter Ads ──────────────────────────
    log({ step: 'Opening Twitter Ads...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Twitter Ads. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Twitter Ads',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Twitter Ads order',
      details: 'Completing your order on Twitter Ads',
      amountInr: 0, // Extracted from page at runtime
      description: 'twitter-ads order',
    });
    ...
    log({ step: 'Twitter Ads workflow completed', status: 'completed' });
    log({ message: 'Task completed on Twitter Ads. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'twitter-ads-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'twitter-ads';
