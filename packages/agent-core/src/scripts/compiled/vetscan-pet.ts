/**
 * Book online vet consultation on VetScan — dog, cat, and pet health, video consult with veterinarians.
 *
 * Auto-generated Playwright script for vetscan-pet.
 * Site: https://www.vetscan.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Vetscan Pet ──────────────────────────
    log({ step: 'Opening Vetscan Pet...', status: 'running' });
    await page.goto('https://www.vetscan.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Vetscan Pet. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Vetscan Pet',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Vetscan Pet order',
      details: 'Completing your order on Vetscan Pet',
      amountInr: 0, // Extracted from page at runtime
      description: 'vetscan-pet order',
    });
    ...
    log({ step: 'Vetscan Pet workflow completed', status: 'completed' });
    log({ message: 'Task completed on Vetscan Pet. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'vetscan-pet-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'vetscan-pet';
