/**
 * Book Ola cab for airport transfer, city ride, or outstation — set pickup/drop, select cab type, confirm and pay.
 *
 * Auto-generated Playwright script for ola-cab.
 * Site: https://www.olacabs.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Ola Cab ──────────────────────────
    log({ step: 'Opening Ola Cab...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Ola Cab. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Ola Cab order',
      details: 'Completing your order on Ola Cab',
      amountInr: 0, // Extracted from page at runtime
      description: 'ola-cab order',
    });
    ...
    log({ step: 'Ola Cab workflow completed', status: 'completed' });
    log({ message: 'Task completed on Ola Cab. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'ola-cab-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
