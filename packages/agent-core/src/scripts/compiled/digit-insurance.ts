/**
 * Buy car or bike insurance on Digit Insurance — get quick quote, compare plans, buy policy online.
 *
 * Auto-generated Playwright script for digit-insurance.
 * Site: https://www.godigit.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Digit Insurance ──────────────────────────
    log({ step: 'Opening Digit Insurance...', status: 'running' });
    await page.goto('https://www.godigit.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Digit Insurance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Digit Insurance',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Digit Insurance order',
      details: 'Completing your order on Digit Insurance',
      amountInr: 0, // Extracted from page at runtime
      description: 'digit-insurance order',
    });
    ...
    log({ step: 'Digit Insurance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Digit Insurance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'digit-insurance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'digit-insurance';
