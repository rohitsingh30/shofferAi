/**
 * Invest in Smallcases (thematic stock portfolios) — browse themes, buy baskets, track performance, rebalance.
 *
 * Auto-generated Playwright script for smallcase-invest.
 * Site: https://www.smallcase.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Smallcase Invest ──────────────────────────
    log({ step: 'Opening Smallcase Invest...', status: 'running' });
    await page.goto('https://www.smallcase.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Smallcase Invest. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Smallcase Invest',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Smallcase Invest order',
      details: 'Completing your order on Smallcase Invest',
      amountInr: 0, // Extracted from page at runtime
      description: 'smallcase-invest order',
    });
    ...
    log({ step: 'Smallcase Invest workflow completed', status: 'completed' });
    log({ message: 'Task completed on Smallcase Invest. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'smallcase-invest-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'smallcase-invest';
