/**
 * Shop Sparx shoes on Relaxo Footwear — browse, select size, buy sports shoes, sandals, slippers, checkout, pay.
 *
 * Auto-generated Playwright script for sparx-shoes.
 * Site: https://www.relaxofootwear.com/sparx
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Sparx Shoes ──────────────────────────
    log({ step: 'Opening Sparx Shoes...', status: 'running' });
    await page.goto('https://www.relaxofootwear.com/sparx');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Sparx Shoes. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Sparx Shoes',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Sparx Shoes order',
      details: 'Completing your order on Sparx Shoes',
      amountInr: 0, // Extracted from page at runtime
      description: 'sparx-shoes order',
    });
    ...
    log({ step: 'Sparx Shoes workflow completed', status: 'completed' });
    log({ message: 'Task completed on Sparx Shoes. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'sparx-shoes-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'sparx-shoes';
