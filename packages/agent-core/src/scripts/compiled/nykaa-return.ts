/**
 * Return beauty products on Nykaa — find order, request return, select reason, schedule pickup or arrange refund.
 *
 * Auto-generated Playwright script for nykaa-return.
 * Site: https://www.nykaa.com/orders
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Nykaa Return ──────────────────────────
    log({ step: 'Opening Nykaa Return...', status: 'running' });
    await page.goto('https://www.nykaa.com/orders');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Nykaa Return. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Nykaa Return',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Nykaa Return order',
      details: 'Completing your order on Nykaa Return',
      amountInr: 0, // Extracted from page at runtime
      description: 'nykaa-return order',
    });
    ...
    log({ step: 'Nykaa Return workflow completed', status: 'completed' });
    log({ message: 'Task completed on Nykaa Return. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'nykaa-return-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'nykaa-return';
