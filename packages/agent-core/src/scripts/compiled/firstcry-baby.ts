/**
 * Buy baby products from FirstCry — diapers, baby food, clothes, toys, gear, checkout, pay.
 *
 * Auto-generated Playwright script for firstcry-baby.
 * Site: https://www.firstcry.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Firstcry Baby ──────────────────────────
    log({ step: 'Opening Firstcry Baby...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Firstcry Baby. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Firstcry Baby',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Firstcry Baby order',
      details: 'Completing your order on Firstcry Baby',
      amountInr: 0, // Extracted from page at runtime
      description: 'firstcry-baby order',
    });
    ...
    log({ step: 'Firstcry Baby workflow completed', status: 'completed' });
    log({ message: 'Task completed on Firstcry Baby. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'firstcry-baby-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'firstcry-baby';
