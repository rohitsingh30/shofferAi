/**
 * Buy OnePlus products on OnePlus India — phones, earbuds, watches, accessories.
 *
 * Auto-generated Playwright script for oneplus-store.
 * Site: https://www.oneplus.in/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Oneplus Store ──────────────────────────
    log({ step: 'Opening Oneplus Store...', status: 'running' });
    await page.goto('https://www.oneplus.in/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Oneplus Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Oneplus Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Oneplus Store order',
      details: 'Completing your order on Oneplus Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'oneplus-store order',
    });
    ...
    log({ step: 'Oneplus Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Oneplus Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'oneplus-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'oneplus-store';
