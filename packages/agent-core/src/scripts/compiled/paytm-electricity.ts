/**
 * Pay electricity bill on Paytm. Select state, choose provider, enter consumer number, and pay.
 *
 * Auto-generated Playwright script for paytm-electricity.
 * Site: https://paytm.com/electricity-bill-payment
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Paytm Electricity ──────────────────────────
    log({ step: 'Opening Paytm Electricity...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Paytm Electricity. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Paytm Electricity',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Paytm Electricity order',
      details: 'Completing your order on Paytm Electricity',
      amountInr: 0, // Extracted from page at runtime
      description: 'paytm-electricity order',
    });
    ...
    log({ step: 'Paytm Electricity workflow completed', status: 'completed' });
    log({ message: 'Task completed on Paytm Electricity. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'paytm-electricity-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'paytm-electricity';
