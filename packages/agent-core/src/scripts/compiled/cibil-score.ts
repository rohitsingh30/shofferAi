/**
 * Check CIBIL credit score and report on cibil.com — enter details, verify identity, view full credit report.
 *
 * Auto-generated Playwright script for cibil-score.
 * Site: https://www.cibil.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cibil Score ──────────────────────────
    log({ step: 'Opening Cibil Score...', status: 'running' });
    await page.goto('https://www.cibil.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cibil Score. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cibil Score',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cibil Score order',
      details: 'Completing your order on Cibil Score',
      amountInr: 0, // Extracted from page at runtime
      description: 'cibil-score order',
    });
    ...
    log({ step: 'Cibil Score workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cibil Score. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cibil-score-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cibil-score';
