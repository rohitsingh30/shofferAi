/**
 * Order cakes, pastries, and brownies from Theobroma — brownies, cakes, cookies, sandwiches, delivery, pay.
 *
 * Auto-generated Playwright script for theobroma-bakery.
 * Site: https://www.theobroma.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Theobroma Bakery ──────────────────────────
    log({ step: 'Opening Theobroma Bakery...', status: 'running' });
    await page.goto('https://www.theobroma.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Theobroma Bakery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Theobroma Bakery',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Theobroma Bakery order',
      details: 'Completing your order on Theobroma Bakery',
    ...
    log({ step: 'Theobroma Bakery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Theobroma Bakery. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'theobroma-bakery-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'theobroma-bakery';
