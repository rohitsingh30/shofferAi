/**
 * Shop on Chumbak — browse quirky home decor, mugs, bags, accessories, furniture, checkout, pay.
 *
 * Auto-generated Playwright script for chumbak-decor.
 * Site: https://www.chumbak.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Chumbak Decor ──────────────────────────
    log({ step: 'Opening Chumbak Decor...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Chumbak Decor. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Chumbak Decor',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Chumbak Decor order',
      details: 'Completing your order on Chumbak Decor',
      amountInr: 0, // Extracted from page at runtime
      description: 'chumbak-decor order',
    });
    ...
    log({ step: 'Chumbak Decor workflow completed', status: 'completed' });
    log({ message: 'Task completed on Chumbak Decor. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'chumbak-decor-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'chumbak-decor';
