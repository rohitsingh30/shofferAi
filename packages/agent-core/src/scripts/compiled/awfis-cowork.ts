/**
 * Book coworking on Awfis — meeting rooms, hot desks, dedicated desks, private cabins across India.
 *
 * Auto-generated Playwright script for awfis-cowork.
 * Site: https://www.awfis.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Awfis Cowork ──────────────────────────
    log({ step: 'Opening Awfis Cowork...', status: 'running' });
    await page.goto('https://www.awfis.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Awfis Cowork. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Awfis Cowork',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Awfis Cowork order',
      details: 'Completing your order on Awfis Cowork',
      amountInr: 0, // Extracted from page at runtime
      description: 'awfis-cowork order',
    });
    ...
    log({ step: 'Awfis Cowork workflow completed', status: 'completed' });
    log({ message: 'Task completed on Awfis Cowork. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'awfis-cowork-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'awfis-cowork';
