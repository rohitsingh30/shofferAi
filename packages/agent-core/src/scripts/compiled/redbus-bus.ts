/**
 * Book bus tickets on RedBus — search routes, compare operators/seat types, pick seats, book and pay.
 *
 * Auto-generated Playwright script for redbus-bus.
 * Site: https://www.redbus.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Redbus Bus ──────────────────────────
    log({ step: 'Opening Redbus Bus...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Redbus Bus. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Redbus Bus',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Redbus Bus order',
      details: 'Completing your order on Redbus Bus',
      amountInr: 0, // Extracted from page at runtime
      description: 'redbus-bus order',
    });
    ...
    log({ step: 'Redbus Bus workflow completed', status: 'completed' });
    log({ message: 'Task completed on Redbus Bus. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'redbus-bus-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'redbus-bus';
