/**
 * Buy exam prep courses on Adda247 — government exam courses, test series, ebooks for SSC, Banking, Railways.
 *
 * Auto-generated Playwright script for adda247-exam.
 * Site: https://www.adda247.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Adda247 Exam ──────────────────────────
    log({ step: 'Opening Adda247 Exam...', status: 'running' });
    await page.goto('https://www.adda247.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Adda247 Exam. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Adda247 Exam',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Adda247 Exam order',
      details: 'Completing your order on Adda247 Exam',
      amountInr: 0, // Extracted from page at runtime
      description: 'adda247-exam order',
    });
    ...
    log({ step: 'Adda247 Exam workflow completed', status: 'completed' });
    log({ message: 'Task completed on Adda247 Exam. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'adda247-exam-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'adda247-exam';
