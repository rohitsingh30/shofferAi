/**
 * Create Asana projects with tasks, sections, assignees, and due dates — team project management and tracking.
 *
 * Auto-generated Playwright script for asana-project.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Asana Project ──────────────────────────
    log({ step: 'Opening Asana Project...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Asana Project. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Asana Project',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Asana Project order',
      details: 'Completing your order on Asana Project',
      amountInr: 0, // Extracted from page at runtime
      description: 'asana-project order',
    });
    ...
    log({ step: 'Asana Project workflow completed', status: 'completed' });
    log({ message: 'Task completed on Asana Project. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'asana-project-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'asana-project';
