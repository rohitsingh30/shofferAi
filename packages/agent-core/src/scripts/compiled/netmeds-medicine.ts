/**
 * Order medicines on Netmeds — search by name, check generic alternatives, upload prescription, order.
 *
 * Auto-generated Playwright script for netmeds-medicine.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Netmeds Medicine ──────────────────────────
    log({ step: 'Opening Netmeds Medicine...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Netmeds Medicine. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Netmeds Medicine',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Netmeds Medicine order',
      details: 'Completing your order on Netmeds Medicine',
    ...
    log({ step: 'Netmeds Medicine workflow completed', status: 'completed' });
    log({ message: 'Task completed on Netmeds Medicine. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'netmeds-medicine-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'netmeds-medicine';
