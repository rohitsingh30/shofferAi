/**
 * Set up a Firebase project — configure Authentication, Firestore database, Cloud Storage, Hosting, and Cloud Functions.
 *
 * Auto-generated Playwright script for firebase-setup.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Firebase Setup ──────────────────────────
    log({ step: 'Opening Firebase Setup...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Firebase Setup. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Firebase Setup',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Firebase Setup order',
      details: 'Completing your order on Firebase Setup',
      amountInr: 0, // Extracted from page at runtime
      description: 'firebase-setup order',
    });
    ...
    log({ step: 'Firebase Setup workflow completed', status: 'completed' });
    log({ message: 'Task completed on Firebase Setup. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'firebase-setup-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'firebase-setup';
