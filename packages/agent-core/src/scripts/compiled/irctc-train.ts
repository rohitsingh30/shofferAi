/**
 * Book train tickets on IRCTC — search trains, select class, book seats, pay.
 *
 * Auto-generated Playwright script for irctc-train.
 * Site: https://www.irctc.co.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Irctc Train ──────────────────────────
    log({ step: 'Opening Irctc Train...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Irctc Train. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Irctc Train',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Irctc Train order',
      details: 'Completing your order on Irctc Train',
      amountInr: 0, // Extracted from page at runtime
      description: 'irctc-train order',
    });
    ...
    log({ step: 'Irctc Train workflow completed', status: 'completed' });
    log({ message: 'Task completed on Irctc Train. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'irctc-train-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'irctc-train';
