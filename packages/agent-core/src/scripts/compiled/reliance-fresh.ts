/**
 * Order groceries from Reliance Fresh/Smart online via JioMart — browse, add to cart, schedule delivery, checkout, pay.
 *
 * Auto-generated Playwright script for reliance-fresh.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Reliance Fresh ──────────────────────────
    log({ step: 'Opening Reliance Fresh...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Reliance Fresh. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Reliance Fresh',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Reliance Fresh order',
      details: 'Completing your order on Reliance Fresh',
      amountInr: 0, // Extracted from page at runtime
      description: 'reliance-fresh order',
    });
    ...
    log({ step: 'Reliance Fresh workflow completed', status: 'completed' });
    log({ message: 'Task completed on Reliance Fresh. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'reliance-fresh-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'reliance-fresh';
