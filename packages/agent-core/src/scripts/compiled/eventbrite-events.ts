/**
 * Find and book events on Eventbrite India — workshops, meetups, conferences, networking events. Browse, register, pay.
 *
 * Auto-generated Playwright script for eventbrite-events.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Eventbrite Events ──────────────────────────
    log({ step: 'Opening Eventbrite Events...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Eventbrite Events. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Eventbrite Events',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Eventbrite Events order',
      details: 'Completing your order on Eventbrite Events',
      amountInr: 0, // Extracted from page at runtime
      description: 'eventbrite-events order',
    });
    ...
    log({ step: 'Eventbrite Events workflow completed', status: 'completed' });
    log({ message: 'Task completed on Eventbrite Events. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'eventbrite-events-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'eventbrite-events';
