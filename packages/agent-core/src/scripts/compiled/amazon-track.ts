/**
 * Track Amazon.in order status — find order, show real-time tracking, delivery estimate, and carrier details.
 *
 * Auto-generated Playwright script for amazon-track.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Amazon Track ──────────────────────────
    log({ step: 'Opening Amazon Track...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Amazon Track. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Amazon Track',
    ...
    log({ step: 'Amazon Track workflow completed', status: 'completed' });
    log({ message: 'Task completed on Amazon Track. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'amazon-track-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'amazon-track';
