/**
 * Book washing machine, refrigerator, AC, geyser, or microwave repair on Urban Company.
 *
 * Auto-generated Playwright script for urbancompany-appliance.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Urbancompany Appliance ──────────────────────────
    log({ step: 'Opening Urbancompany Appliance...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Urbancompany Appliance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Urbancompany Appliance',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Urbancompany Appliance order',
      details: 'Completing your order on Urbancompany Appliance',
      amountInr: 0, // Extracted from page at runtime
      description: 'urbancompany-appliance order',
    });
    ...
    log({ step: 'Urbancompany Appliance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Urbancompany Appliance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'urbancompany-appliance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'urbancompany-appliance';
