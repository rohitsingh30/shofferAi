/**
 * Order fresh milk and dairy from Country Delight — subscription or one-time, farm-fresh products, doorstep delivery.
 *
 * Auto-generated Playwright script for country-delight.
 * Site: https://www.countrydelight.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Country Delight ──────────────────────────
    log({ step: 'Opening Country Delight...', status: 'running' });
    await page.goto('https://www.countrydelight.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Country Delight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Country Delight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Country Delight order',
      details: 'Completing your order on Country Delight',
      amountInr: 0, // Extracted from page at runtime
      description: 'country-delight order',
    });
    ...
    log({ step: 'Country Delight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Country Delight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'country-delight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'country-delight';
