/**
 * Create Instagram ad campaign — set objective, target audience, budget, ad creative, and launch via Meta Ads Manager.
 *
 * Auto-generated Playwright script for instagram-ads.
 * Site: https://adsmanager.facebook.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Instagram Ads ──────────────────────────
    log({ step: 'Opening Instagram Ads...', status: 'running' });
    await page.goto('https://adsmanager.facebook.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Instagram Ads. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Instagram Ads',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Instagram Ads order',
      details: 'Completing your order on Instagram Ads',
      amountInr: 0, // Extracted from page at runtime
      description: 'instagram-ads order',
    });
    ...
    log({ step: 'Instagram Ads workflow completed', status: 'completed' });
    log({ message: 'Task completed on Instagram Ads. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'instagram-ads-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'instagram-ads';
