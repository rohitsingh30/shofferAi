/**
 * Shop on boAt — buy earbuds, headphones, speakers, smartwatches, cables, browse collections, checkout, pay.
 *
 * Auto-generated Playwright script for boat-electronics.
 * Site: https://www.boat-lifestyle.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Boat Electronics ──────────────────────────
    log({ step: 'Opening Boat Electronics...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Boat Electronics. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Boat Electronics',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Boat Electronics order',
      details: 'Completing your order on Boat Electronics',
      amountInr: 0, // Extracted from page at runtime
      description: 'boat-electronics order',
    });
    ...
    log({ step: 'Boat Electronics workflow completed', status: 'completed' });
    log({ message: 'Task completed on Boat Electronics. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'boat-electronics-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'boat-electronics';
