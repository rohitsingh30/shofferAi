/**
 * Book hotels on MakeMyTrip — search by city/dates/budget, compare options, book and pay.
 *
 * Auto-generated Playwright script for makemytrip-hotel.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Makemytrip Hotel ──────────────────────────
    log({ step: 'Opening Makemytrip Hotel...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Makemytrip Hotel. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Makemytrip Hotel',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Makemytrip Hotel order',
      details: 'Completing your order on Makemytrip Hotel',
      amountInr: 0, // Extracted from page at runtime
      description: 'makemytrip-hotel order',
    });
    ...
    log({ step: 'Makemytrip Hotel workflow completed', status: 'completed' });
    log({ message: 'Task completed on Makemytrip Hotel. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'makemytrip-hotel-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'makemytrip-hotel';
