/**
 * Buy Lenovo/ThinkPad products on Lenovo India — laptops, desktops, tablets, monitors.
 *
 * Auto-generated Playwright script for lenovo-store.
 * Site: https://www.lenovo.com/in/en/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Lenovo Store ──────────────────────────
    log({ step: 'Opening Lenovo Store...', status: 'running' });
    await page.goto('https://www.lenovo.com/in/en/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Lenovo Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Lenovo Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Lenovo Store order',
      details: 'Completing your order on Lenovo Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'lenovo-store order',
    });
    ...
    log({ step: 'Lenovo Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Lenovo Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'lenovo-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'lenovo-store';
