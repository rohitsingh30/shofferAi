/**
 * Order from Starbucks India for delivery — select store, customize drinks, add food, checkout, pay.
 *
 * Auto-generated Playwright script for starbucks-order.
 * Site: https://www.starbucks.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Starbucks Order ──────────────────────────
    log({ step: 'Opening Starbucks Order...', status: 'running' });
    await page.goto('https://www.starbucks.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Starbucks Order. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Starbucks Order',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Starbucks Order order',
      details: 'Completing your order on Starbucks Order',
      amountInr: 0, // Extracted from page at runtime
      description: 'starbucks-order order',
    });
    ...
    log({ step: 'Starbucks Order workflow completed', status: 'completed' });
    log({ message: 'Task completed on Starbucks Order. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'starbucks-order-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'starbucks-order';
