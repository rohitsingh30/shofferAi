/**
 * Book appointment at Manipal Hospital — find specialists by department, schedule in-person or video consultation.
 *
 * Auto-generated Playwright script for manipal-hospital.
 * Site: https://www.manipalhospitals.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Manipal Hospital ──────────────────────────
    log({ step: 'Opening Manipal Hospital...', status: 'running' });
    await page.goto('https://www.manipalhospitals.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Manipal Hospital. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Manipal Hospital',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Manipal Hospital order',
      details: 'Completing your order on Manipal Hospital',
      amountInr: 0, // Extracted from page at runtime
      description: 'manipal-hospital order',
    });
    ...
    log({ step: 'Manipal Hospital workflow completed', status: 'completed' });
    log({ message: 'Task completed on Manipal Hospital. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'manipal-hospital-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'manipal-hospital';
