/**
 * Order groceries from Grofers (now Blinkit) — browse products, add to cart, checkout, pay with quick delivery.
 *
 * Auto-generated Playwright script for grofers-grocery.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Grofers Grocery ──────────────────────────
    log({ step: 'Opening Grofers Grocery...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Grofers Grocery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Grofers Grocery',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Grofers Grocery order',
      details: 'Completing your order on Grofers Grocery',
      amountInr: 0, // Extracted from page at runtime
      description: 'grofers-grocery order',
    });
    ...
    log({ step: 'Grofers Grocery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Grofers Grocery. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'grofers-grocery-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'grofers-grocery';
