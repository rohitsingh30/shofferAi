/**
 * Book sports venues (badminton, cricket, football turfs) on Playo — search venues, pick time slot, pay.
 *
 * Auto-generated Playwright script for playo-sports.
 * Site: https://playo.co
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Playo Sports ──────────────────────────
    log({ step: 'Opening Playo Sports...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Playo Sports. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Playo Sports',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Playo Sports order',
      details: 'Completing your order on Playo Sports',
      amountInr: 0, // Extracted from page at runtime
      description: 'playo-sports order',
    });
    ...
    log({ step: 'Playo Sports workflow completed', status: 'completed' });
    log({ message: 'Task completed on Playo Sports. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'playo-sports-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'playo-sports';
