/**
 * Book holiday packages on MakeMyTrip — flights+hotel combos, domestic/international tours, customizable and group packages.
 *
 * Auto-generated Playwright script for makemytrip-holiday.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Makemytrip Holiday ──────────────────────────
    log({ step: 'Opening Makemytrip Holiday...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Makemytrip Holiday. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Makemytrip Holiday',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Makemytrip Holiday order',
      details: 'Completing your order on Makemytrip Holiday',
      amountInr: 0, // Extracted from page at runtime
      description: 'makemytrip-holiday order',
    });
    ...
    log({ step: 'Makemytrip Holiday workflow completed', status: 'completed' });
    log({ message: 'Task completed on Makemytrip Holiday. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'makemytrip-holiday-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'makemytrip-holiday';
