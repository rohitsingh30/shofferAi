/**
 * Order flowers and cakes on FlowerAura — same-day delivery, bouquets, combos, personalized gifts for all occasions.
 *
 * Auto-generated Playwright script for floweraura-flowers.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Floweraura Flowers ──────────────────────────
    log({ step: 'Opening Floweraura Flowers...', status: 'running' });
    await page.goto('https://www.floweraura.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Floweraura Flowers. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Floweraura Flowers',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Floweraura Flowers order',
      details: 'Completing your order on Floweraura Flowers',
      amountInr: 0, // Extracted from page at runtime
      description: 'floweraura-flowers order',
    });
    ...
    log({ step: 'Floweraura Flowers workflow completed', status: 'completed' });
    log({ message: 'Task completed on Floweraura Flowers. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'floweraura-flowers-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'floweraura-flowers';
