/**
 * Recharge BSNL prepaid or pay BSNL postpaid/landline bills on bsnl.co.in. Browse plans and pay.
 *
 * Auto-generated Playwright script for bsnl-recharge.
 * Site: https://portal.bsnl.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bsnl Recharge ──────────────────────────
    log({ step: 'Opening Bsnl Recharge...', status: 'running' });
    await page.goto('https://portal.bsnl.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bsnl Recharge. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bsnl Recharge',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bsnl Recharge order',
      details: 'Completing your order on Bsnl Recharge',
      amountInr: 0, // Extracted from page at runtime
      description: 'bsnl-recharge order',
    });
    ...
    log({ step: 'Bsnl Recharge workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bsnl Recharge. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bsnl-recharge-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bsnl-recharge';
