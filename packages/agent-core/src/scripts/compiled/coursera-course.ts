/**
 * Enroll in Coursera courses or specializations — search, compare, enroll free or paid.
 *
 * Auto-generated Playwright script for coursera-course.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Coursera Course ──────────────────────────
    log({ step: 'Opening Coursera Course...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Coursera Course. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Coursera Course',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Coursera Course order',
      details: 'Completing your order on Coursera Course',
      amountInr: 0, // Extracted from page at runtime
      description: 'coursera-course order',
    });
    ...
    log({ step: 'Coursera Course workflow completed', status: 'completed' });
    log({ message: 'Task completed on Coursera Course. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'coursera-course-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'coursera-course';
