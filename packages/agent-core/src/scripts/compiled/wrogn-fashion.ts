/**
 * Buy Virat Kohli's Wrogn fashion on wrogn.com — browse casual wear, streetwear, active wear, select size, checkout, pay.
 *
 * Auto-generated Playwright script for wrogn-fashion.
 * Site: https://www.wrogn.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Wrogn Fashion ──────────────────────────
    log({ step: 'Opening Wrogn Fashion...', status: 'running' });
    await page.goto('https://www.wrogn.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Wrogn Fashion. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Wrogn Fashion',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Wrogn Fashion order',
      details: 'Completing your order on Wrogn Fashion',
    ...
    log({ step: 'Wrogn Fashion workflow completed', status: 'completed' });
    log({ message: 'Task completed on Wrogn Fashion. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'wrogn-fashion-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'wrogn-fashion';
