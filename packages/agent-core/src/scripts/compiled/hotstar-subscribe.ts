/**
 * Subscribe to Disney+ Hotstar India. Choose plan (Mobile, Super, Premium), pay and activate.
 *
 * Auto-generated Playwright script for hotstar-subscribe.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hotstar Subscribe ──────────────────────────
    log({ step: 'Opening Hotstar Subscribe...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hotstar Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hotstar Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hotstar Subscribe order',
      details: 'Completing your order on Hotstar Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'hotstar-subscribe order',
    });
    ...
    log({ step: 'Hotstar Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hotstar Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hotstar-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hotstar-subscribe';
