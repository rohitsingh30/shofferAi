/**
 * Apply for tech and startup jobs on Cuvette — software engineering, data science, product, design roles at high-growth startups.
 *
 * Auto-generated Playwright script for cuvette-tech-jobs.
 * Site: https://cuvette.tech
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cuvette Tech Jobs ──────────────────────────
    log({ step: 'Opening Cuvette Tech Jobs...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cuvette Tech Jobs. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cuvette Tech Jobs',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cuvette Tech Jobs order',
      details: 'Completing your order on Cuvette Tech Jobs',
      amountInr: 0, // Extracted from page at runtime
      description: 'cuvette-tech-jobs order',
    });
    ...
    log({ step: 'Cuvette Tech Jobs workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cuvette Tech Jobs. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cuvette-tech-jobs-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cuvette-tech-jobs';
