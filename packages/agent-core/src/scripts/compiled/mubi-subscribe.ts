/**
 * Subscribe to MUBI India. Curated indie, art-house, and international cinema. Hand-picked films daily.
 *
 * Auto-generated Playwright script for mubi-subscribe.
 * Site: https://mubi.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Mubi Subscribe ──────────────────────────
    log({ step: 'Opening Mubi Subscribe...', status: 'running' });
    await page.goto('https://mubi.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Mubi Subscribe. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Mubi Subscribe',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Mubi Subscribe order',
      details: 'Completing your order on Mubi Subscribe',
      amountInr: 0, // Extracted from page at runtime
      description: 'mubi-subscribe order',
    });
    ...
    log({ step: 'Mubi Subscribe workflow completed', status: 'completed' });
    log({ message: 'Task completed on Mubi Subscribe. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'mubi-subscribe-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'mubi-subscribe';
