/**
 * Shop on Snapdeal — search budget products, compare deals, add to cart, checkout, pay.
 *
 * Auto-generated Playwright script for snapdeal-shopping.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Snapdeal Shopping ──────────────────────────
    log({ step: 'Opening Snapdeal Shopping...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Snapdeal Shopping. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Snapdeal Shopping',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Snapdeal Shopping order',
      details: 'Completing your order on Snapdeal Shopping',
      amountInr: 0, // Extracted from page at runtime
      description: 'snapdeal-shopping order',
    });
    ...
    log({ step: 'Snapdeal Shopping workflow completed', status: 'completed' });
    log({ message: 'Task completed on Snapdeal Shopping. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'snapdeal-shopping-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'snapdeal-shopping';
