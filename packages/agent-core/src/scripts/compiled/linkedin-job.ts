/**
 * Search and apply for jobs on LinkedIn — use Easy Apply, filter by role/location/type, send applications.
 *
 * Auto-generated Playwright script for linkedin-job.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Linkedin Job...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Linkedin Job. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Linkedin Job',
    ...
    log({ step: 'Linkedin Job workflow completed', status: 'completed' });
    log({ message: 'Task completed on Linkedin Job. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'linkedin-job-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'linkedin-job';
