/**
 * Book PVR INOX movie tickets on pvrcinemas.com — select movie, cinema, showtime, seats, food combos, and pay.
 *
 * Auto-generated Playwright script for pvr-tickets.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Pvr Tickets ──────────────────────────
    log({ step: 'Opening Pvr Tickets...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Pvr Tickets. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Pvr Tickets',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Pvr Tickets order',
      details: 'Completing your order on Pvr Tickets',
      amountInr: 0, // Extracted from page at runtime
      description: 'pvr-tickets order',
    });
    ...
    log({ step: 'Pvr Tickets workflow completed', status: 'completed' });
    log({ message: 'Task completed on Pvr Tickets. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'pvr-tickets-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'pvr-tickets';
