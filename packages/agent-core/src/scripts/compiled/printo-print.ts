/**
 * Order document and photo prints online from Printo — upload files, customize print options, select delivery, pay.
 *
 * Auto-generated Playwright script for printo-print.
 * Site: https://www.printo.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Printo Print ──────────────────────────
    log({ step: 'Opening Printo Print...', status: 'running' });
    await page.goto('https://www.printo.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Printo Print. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Printo Print',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Printo Print order',
      details: 'Completing your order on Printo Print',
    ...
    log({ step: 'Printo Print workflow completed', status: 'completed' });
    log({ message: 'Task completed on Printo Print. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'printo-print-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'printo-print';
