/**
 * Buy mutual funds on Zerodha Coin — search funds, start SIP, make lump sum investments, track portfolio.
 *
 * Auto-generated Playwright script for coin-by-zerodha.
 * Site: https://coin.zerodha.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Coin By Zerodha ──────────────────────────
    log({ step: 'Opening Coin By Zerodha...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Coin By Zerodha. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Coin By Zerodha',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Coin By Zerodha order',
      details: 'Completing your order on Coin By Zerodha',
      amountInr: 0, // Extracted from page at runtime
      description: 'coin-by-zerodha order',
    });
    ...
    log({ step: 'Coin By Zerodha workflow completed', status: 'completed' });
    log({ message: 'Task completed on Coin By Zerodha. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'coin-by-zerodha-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'coin-by-zerodha';
