/**
 * Buy exam test series on Testbook — SSC, Bank PO, Railways, UPSC mock tests and practice papers.
 *
 * Auto-generated Playwright script for testbook-exam.
 * Site: https://testbook.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Testbook Exam ──────────────────────────
    log({ step: 'Opening Testbook Exam...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Testbook Exam. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Testbook Exam',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Testbook Exam order',
      details: 'Completing your order on Testbook Exam',
      amountInr: 0, // Extracted from page at runtime
      description: 'testbook-exam order',
    });
    ...
    log({ step: 'Testbook Exam workflow completed', status: 'completed' });
    log({ message: 'Task completed on Testbook Exam. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'testbook-exam-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'testbook-exam';
