/**
 * Research company reviews, salaries, and interview experiences on Glassdoor India — help users evaluate employers before joining.
 *
 * Auto-generated Playwright script for glassdoor-review.
 * Site: https://www.glassdoor.co.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Glassdoor Review ──────────────────────────
    log({ step: 'Opening Glassdoor Review...', status: 'running' });
    await page.goto('https://www.glassdoor.co.in');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Glassdoor Review. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Glassdoor Review',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Glassdoor Review order',
      details: 'Completing your order on Glassdoor Review',
      amountInr: 0, // Extracted from page at runtime
      description: 'glassdoor-review order',
    });
    ...
    log({ step: 'Glassdoor Review workflow completed', status: 'completed' });
    log({ message: 'Task completed on Glassdoor Review. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'glassdoor-review-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'glassdoor-review';
