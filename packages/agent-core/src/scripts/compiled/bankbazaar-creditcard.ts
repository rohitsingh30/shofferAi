/**
 * Compare and apply for credit cards on BankBazaar — browse offers, compare features, apply online.
 *
 * Auto-generated Playwright script for bankbazaar-creditcard.
 * Site: https://www.bankbazaar.com/credit-card.html
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bankbazaar Creditcard ──────────────────────────
    log({ step: 'Opening Bankbazaar Creditcard...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bankbazaar Creditcard. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bankbazaar Creditcard',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bankbazaar Creditcard order',
      details: 'Completing your order on Bankbazaar Creditcard',
      amountInr: 0, // Extracted from page at runtime
      description: 'bankbazaar-creditcard order',
    });
    ...
    log({ step: 'Bankbazaar Creditcard workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bankbazaar Creditcard. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bankbazaar-creditcard-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bankbazaar-creditcard';
