/**
 * Find parking, pay traffic challans, recharge FASTag, and manage vehicle services on Park+ — nearby parking, book spot, challan check.
 *
 * Auto-generated Playwright script for park-plus.
 * Site: https://www.parkplus.io
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Park Plus ──────────────────────────
    log({ step: 'Opening Park Plus...', status: 'running' });
    await page.goto('https://www.parkplus.io');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Park Plus. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Park Plus order',
      details: 'Completing your order on Park Plus',
      amountInr: 0, // Extracted from page at runtime
      description: 'park-plus order',
    });
    ...
    log({ step: 'Park Plus workflow completed', status: 'completed' });
    log({ message: 'Task completed on Park Plus. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'park-plus-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'park-plus';
