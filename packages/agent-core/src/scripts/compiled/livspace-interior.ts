/**
 * Book interior design consultation on Livspace — select room type, set budget, browse designs, schedule consultation with designer.
 *
 * Auto-generated Playwright script for livspace-interior.
 * Site: https://www.livspace.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Livspace Interior ──────────────────────────
    log({ step: 'Opening Livspace Interior...', status: 'running' });
    await page.goto('https://www.livspace.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Livspace Interior. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Livspace Interior',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Livspace Interior order',
      details: 'Completing your order on Livspace Interior',
      amountInr: 0, // Extracted from page at runtime
      description: 'livspace-interior order',
    });
    ...
    log({ step: 'Livspace Interior workflow completed', status: 'completed' });
    log({ message: 'Task completed on Livspace Interior. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'livspace-interior-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'livspace-interior';
