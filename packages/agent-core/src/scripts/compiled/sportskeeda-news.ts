/**
 * Get sports updates, live scores, schedules, and news on Sportskeeda — cricket, football, IPL, FIFA, NBA results.
 *
 * Auto-generated Playwright script for sportskeeda-news.
 * Site: https://www.sportskeeda.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Sportskeeda News ──────────────────────────
    log({ step: 'Opening Sportskeeda News...', status: 'running' });
    await page.goto('https://www.sportskeeda.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Sportskeeda News. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Sportskeeda News',
    ...
    log({ step: 'Sportskeeda News workflow completed', status: 'completed' });
    log({ message: 'Task completed on Sportskeeda News. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'sportskeeda-news-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'sportskeeda-news';
