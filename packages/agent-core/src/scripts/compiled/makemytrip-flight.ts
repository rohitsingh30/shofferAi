/**
 * Book flights on MakeMyTrip — search by route/date, compare airlines/times/prices, select, fill passenger details, pay.
 *
 * Auto-generated Playwright script for makemytrip-flight.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Makemytrip Flight ──────────────────────────
    log({ step: 'Opening Makemytrip Flight...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Makemytrip Flight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Makemytrip Flight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Makemytrip Flight order',
      details: 'Completing your order on Makemytrip Flight',
      amountInr: 0, // Extracted from page at runtime
      description: 'makemytrip-flight order',
    });
    ...
    log({ step: 'Makemytrip Flight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Makemytrip Flight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'makemytrip-flight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'makemytrip-flight';
