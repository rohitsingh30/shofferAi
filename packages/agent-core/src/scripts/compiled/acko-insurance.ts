/**
 * Buy car or bike insurance on Acko — instant quote, zero paperwork, digital policy, best price guarantee.
 *
 * Auto-generated Playwright script for acko-insurance.
 * Site: https://www.acko.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Acko Insurance ──────────────────────────
    log({ step: 'Opening Acko Insurance...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Acko Insurance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Acko Insurance',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Acko Insurance order',
      details: 'Completing your order on Acko Insurance',
      amountInr: 0, // Extracted from page at runtime
      description: 'acko-insurance order',
    });
    ...
    log({ step: 'Acko Insurance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Acko Insurance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'acko-insurance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'acko-insurance';
