/**
 * Shop on HomeTown — browse home decor, furniture, kitchenware, furnishings, checkout, pay.
 *
 * Auto-generated Playwright script for hometown-decor.
 * Site: https://www.hometown.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Hometown Decor ──────────────────────────
    log({ step: 'Opening Hometown Decor...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Hometown Decor. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Hometown Decor',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Hometown Decor order',
      details: 'Completing your order on Hometown Decor',
      amountInr: 0, // Extracted from page at runtime
      description: 'hometown-decor order',
    });
    ...
    log({ step: 'Hometown Decor workflow completed', status: 'completed' });
    log({ message: 'Task completed on Hometown Decor. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'hometown-decor-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'hometown-decor';
