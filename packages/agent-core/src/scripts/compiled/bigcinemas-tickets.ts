/**
 * Book movie tickets at Cinepolis cinemas via their website — select movie, location, showtime, seats, and pay.
 *
 * Auto-generated Playwright script for bigcinemas-tickets.
 * Site: https://www.cinepolisindia.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bigcinemas Tickets ──────────────────────────
    log({ step: 'Opening Bigcinemas Tickets...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bigcinemas Tickets. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bigcinemas Tickets',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bigcinemas Tickets order',
      details: 'Completing your order on Bigcinemas Tickets',
      amountInr: 0, // Extracted from page at runtime
      description: 'bigcinemas-tickets order',
    });
    ...
    log({ step: 'Bigcinemas Tickets workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bigcinemas Tickets. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bigcinemas-tickets-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bigcinemas-tickets';
