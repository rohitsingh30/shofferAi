/**
 * Buy Bosch home appliances on Bosch India — washing machines, dishwashers, refrigerators, dryers, ovens, cooktops.
 *
 * Auto-generated Playwright script for bosch-store.
 * Site: https://www.boschindia.com/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bosch Store ──────────────────────────
    log({ step: 'Opening Bosch Store...', status: 'running' });
    await page.goto('https://www.boschindia.com/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bosch Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bosch Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bosch Store order',
      details: 'Completing your order on Bosch Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'bosch-store order',
    });
    ...
    log({ step: 'Bosch Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bosch Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bosch-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bosch-store';
