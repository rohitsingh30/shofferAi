/**
 * Buy a used car on Ola Cars — browse certified pre-owned cars, inspect, finance, and buy with doorstep delivery.
 *
 * Auto-generated Playwright script for olacars-buy.
 * Site: https://www.olacars.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Olacars Buy ──────────────────────────
    log({ step: 'Opening Olacars Buy...', status: 'running' });
    await page.goto('https://www.olacars.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Olacars Buy. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Olacars Buy',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Olacars Buy order',
      details: 'Completing your order on Olacars Buy',
      amountInr: 0, // Extracted from page at runtime
      description: 'olacars-buy order',
    });
    ...
    log({ step: 'Olacars Buy workflow completed', status: 'completed' });
    log({ message: 'Task completed on Olacars Buy. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'olacars-buy-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'olacars-buy';
