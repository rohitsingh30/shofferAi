/**
 * Print photos and photobooks on Zoomin — upload photos, customize prints, select size/finish, order delivery.
 *
 * Auto-generated Playwright script for zoomin-photo.
 * Site: https://www.zoomin.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Zoomin Photo ──────────────────────────
    log({ step: 'Opening Zoomin Photo...', status: 'running' });
    await page.goto('https://www.zoomin.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Zoomin Photo. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Zoomin Photo',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Zoomin Photo order',
      details: 'Completing your order on Zoomin Photo',
      amountInr: 0, // Extracted from page at runtime
      description: 'zoomin-photo order',
    });
    ...
    log({ step: 'Zoomin Photo workflow completed', status: 'completed' });
    log({ message: 'Task completed on Zoomin Photo. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'zoomin-photo-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'zoomin-photo';
