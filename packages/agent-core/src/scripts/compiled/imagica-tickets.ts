/**
 * Book Imagica theme park tickets — select park (Theme Park, Water Park, Snow Park), date, package, and pay online.
 *
 * Auto-generated Playwright script for imagica-tickets.
 * Site: https://www.imagicaa.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Imagica Tickets ──────────────────────────
    log({ step: 'Opening Imagica Tickets...', status: 'running' });
    await page.goto('https://www.imagicaa.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Imagica Tickets. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Imagica Tickets',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Imagica Tickets order',
      details: 'Completing your order on Imagica Tickets',
      amountInr: 0, // Extracted from page at runtime
      description: 'imagica-tickets order',
    });
    ...
    log({ step: 'Imagica Tickets workflow completed', status: 'completed' });
    log({ message: 'Task completed on Imagica Tickets. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'imagica-tickets-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'imagica-tickets';
