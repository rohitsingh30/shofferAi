/**
 * Order groceries from DMart Ready — browse products, add to cart, schedule pickup or delivery, checkout, pay.
 *
 * Auto-generated Playwright script for dmart-grocery.
 * Site: https://www.dmart.in
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Dmart Grocery ──────────────────────────
    log({ step: 'Opening Dmart Grocery...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Dmart Grocery. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Dmart Grocery',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Dmart Grocery order',
      details: 'Completing your order on Dmart Grocery',
      amountInr: 0, // Extracted from page at runtime
      description: 'dmart-grocery order',
    });
    ...
    log({ step: 'Dmart Grocery workflow completed', status: 'completed' });
    log({ message: 'Task completed on Dmart Grocery. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'dmart-grocery-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'dmart-grocery';
