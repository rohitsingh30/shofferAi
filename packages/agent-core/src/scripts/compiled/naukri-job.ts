/**
 * Search and apply for jobs on Naukri.com — filter by role, location, salary, experience, apply to matching positions.
 *
 * Auto-generated Playwright script for naukri-job.
 * Site: https://www.naukri.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Naukri Job ──────────────────────────
    log({ step: 'Opening Naukri Job...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Naukri Job. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Naukri Job',
    ...
    log({ step: 'Naukri Job workflow completed', status: 'completed' });
    log({ message: 'Task completed on Naukri Job. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'naukri-job-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'naukri-job';
