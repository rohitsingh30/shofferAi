/**
 * Find cheapest flights on Google Flights — search, compare across airlines, track prices, and redirect to airline/OTA to book.
 *
 * Auto-generated Playwright script for google-flights.
...
(async () => {
  ...
  try {
    ...
    log({ step: 'Opening Google Flights...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Google Flights. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Google Flights',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Google Flights order',
      details: 'Completing your order on Google Flights',
      amountInr: 0, // Extracted from page at runtime
      description: 'google-flights order',
    });
    ...
    log({ step: 'Google Flights workflow completed', status: 'completed' });
    log({ message: 'Task completed on Google Flights. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'google-flights-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'google-flights';
