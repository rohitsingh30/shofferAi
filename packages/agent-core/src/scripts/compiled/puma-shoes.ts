/**
 * Shop on Puma India — browse, select size, buy shoes, apparel, and accessories, checkout, pay.
 *
 * Auto-generated Playwright script for puma-shoes.
 * Site: https://in.puma.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Puma Shoes ──────────────────────────
    log({ step: 'Opening Puma Shoes...', status: 'running' });
    await page.goto('https://in.puma.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Puma Shoes. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Puma Shoes',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Puma Shoes order',
      details: 'Completing your order on Puma Shoes',
      amountInr: 0, // Extracted from page at runtime
      description: 'puma-shoes order',
    });
    ...
    log({ step: 'Puma Shoes workflow completed', status: 'completed' });
    log({ message: 'Task completed on Puma Shoes. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'puma-shoes-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'puma-shoes';
