/**
 * Send packages via Swiggy Genie — pick up and drop anything, hyperlocal delivery within the city.
 *
 * Auto-generated Playwright script for swiggy-genie.
 * Site: https://www.swiggy.com/genie
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Swiggy Genie ──────────────────────────
    log({ step: 'Opening Swiggy Genie...', status: 'running' });
    await page.goto('https://www.swiggy.com/genie');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Swiggy Genie. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Swiggy Genie',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Swiggy Genie order',
      details: 'Completing your order on Swiggy Genie',
      amountInr: 0, // Extracted from page at runtime
      description: 'swiggy-genie order',
    });
    ...
    log({ step: 'Swiggy Genie workflow completed', status: 'completed' });
    log({ message: 'Task completed on Swiggy Genie. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'swiggy-genie-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'swiggy-genie';
