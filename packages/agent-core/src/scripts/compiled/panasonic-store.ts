/**
 * Buy Panasonic electronics and appliances on Panasonic India — ACs, TVs, washing machines, trimmers, cameras, kitchen appliances.
 *
 * Auto-generated Playwright script for panasonic-store.
 * Site: https://www.panasonic.com/in/
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Panasonic Store ──────────────────────────
    log({ step: 'Opening Panasonic Store...', status: 'running' });
    await page.goto('https://www.panasonic.com/in/');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Panasonic Store. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Panasonic Store',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Panasonic Store order',
      details: 'Completing your order on Panasonic Store',
      amountInr: 0, // Extracted from page at runtime
      description: 'panasonic-store order',
    });
    ...
    log({ step: 'Panasonic Store workflow completed', status: 'completed' });
    log({ message: 'Task completed on Panasonic Store. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'panasonic-store-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'panasonic-store';
