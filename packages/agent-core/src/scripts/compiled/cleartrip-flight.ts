/**
 * Book flights on Cleartrip — search by route/date, filter by airline/time/stops, compare fares, book and pay.
 *
 * Auto-generated Playwright script for cleartrip-flight.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Cleartrip Flight ──────────────────────────
    log({ step: 'Opening Cleartrip Flight...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Cleartrip Flight. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Cleartrip Flight',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Cleartrip Flight order',
      details: 'Completing your order on Cleartrip Flight',
      amountInr: 0, // Extracted from page at runtime
      description: 'cleartrip-flight order',
    });
    ...
    log({ step: 'Cleartrip Flight workflow completed', status: 'completed' });
    log({ message: 'Task completed on Cleartrip Flight. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'cleartrip-flight-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'cleartrip-flight';
