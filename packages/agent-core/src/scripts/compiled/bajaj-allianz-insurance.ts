/**
 * Buy Bajaj Allianz insurance — motor, health, travel, home, and cyber insurance plans.
 *
 * Auto-generated Playwright script for bajaj-allianz-insurance.
 * Site: https://www.bajajallianz.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Bajaj Allianz Insurance ──────────────────────────
    log({ step: 'Opening Bajaj Allianz Insurance...', status: 'running' });
    await page.goto('https://www.bajajallianz.com');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Bajaj Allianz Insurance. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Bajaj Allianz Insurance',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Bajaj Allianz Insurance order',
      details: 'Completing your order on Bajaj Allianz Insurance',
      amountInr: 0, // Extracted from page at runtime
      description: 'bajaj-allianz-insurance order',
    });
    ...
    log({ step: 'Bajaj Allianz Insurance workflow completed', status: 'completed' });
    log({ message: 'Task completed on Bajaj Allianz Insurance. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'bajaj-allianz-insurance-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'bajaj-allianz-insurance';
