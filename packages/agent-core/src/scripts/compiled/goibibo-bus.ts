/**
 * Book bus tickets on Goibibo — search routes, filter by bus type, select seats, compare operators, book and pay.
 *
 * Auto-generated Playwright script for goibibo-bus.
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Goibibo Bus ──────────────────────────
    log({ step: 'Opening Goibibo Bus...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Goibibo Bus. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Goibibo Bus',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Goibibo Bus order',
      details: 'Completing your order on Goibibo Bus',
      amountInr: 0, // Extracted from page at runtime
      description: 'goibibo-bus order',
    });
    ...
    log({ step: 'Goibibo Bus workflow completed', status: 'completed' });
    log({ message: 'Task completed on Goibibo Bus. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'goibibo-bus-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'goibibo-bus';
