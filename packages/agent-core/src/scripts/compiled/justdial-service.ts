/**
 * Find local services on JustDial — search businesses, compare ratings, get quotes, call or book.
 *
 * Auto-generated Playwright script for justdial-service.
 * Site: https://www.justdial.com
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Justdial Service ──────────────────────────
    log({ step: 'Opening Justdial Service...', status: 'running' });
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Justdial Service. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Justdial Service',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Justdial Service order',
      details: 'Completing your order on Justdial Service',
      amountInr: 0, // Extracted from page at runtime
      description: 'justdial-service order',
    });
    ...
    log({ step: 'Justdial Service workflow completed', status: 'completed' });
    log({ message: 'Task completed on Justdial Service. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'justdial-service-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'justdial-service';
