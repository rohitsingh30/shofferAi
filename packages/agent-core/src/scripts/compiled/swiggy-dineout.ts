/**
 * Book a restaurant table via Swiggy Dineout — search restaurants, select date/time/guests, reserve, get confirmation.
 *
 * Auto-generated Playwright script for swiggy-dineout.
 * Site: https://www.swiggy.com/dineout
...
(async () => {
  ...
  try {
    ...
    // ── Navigate to Swiggy Dineout ──────────────────────────
    log({ step: 'Opening Swiggy Dineout...', status: 'running' });
    await page.goto('https://www.swiggy.com/dineout');
    ...
    const userChoice = await requestFromHost({
      type: 'input_required',
      question: 'I found results on Swiggy Dineout. Which option would you like? (enter a number or describe your preference)',
      inputType: 'freetext',
    });
    ...
    const confirmResp = await requestFromHost({
      type: 'confirm_action',
      action: 'Proceed with Swiggy Dineout',
    ...
    const payResp = await requestFromHost({
      ...
      action: 'Complete Swiggy Dineout order',
      details: 'Completing your order on Swiggy Dineout',
      amountInr: 0, // Extracted from page at runtime
      description: 'swiggy-dineout order',
    });
    ...
    log({ step: 'Swiggy Dineout workflow completed', status: 'completed' });
    log({ message: 'Task completed on Swiggy Dineout. Page: ' + finalTitle });
    ...
    try {
      ...
      await page.screenshot({
        path: path.join(screenshotDir, 'swiggy-dineout-error-' + Date.now() + '.png'),
        fullPage: true,
      });
    } catch {}
  } finally {
...
export const SKILL_ID = 'swiggy-dineout';
