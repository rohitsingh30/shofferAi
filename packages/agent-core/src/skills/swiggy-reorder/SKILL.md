---
name: swiggy-reorder
description: Reorder last food order from Swiggy — view order history, repeat a previous order, checkout, pay.
triggers:
  - reorder swiggy
  - swiggy reorder
  - repeat swiggy order
...
# Swiggy Reorder — Repeat Previous Order
...
- Confirm location set and homepage visible.

### 3. Navigate to Order History
- Click on profile/account icon, then navigate to "My Orders" or "Order History".
...
### 4. Initiate Reorder
- Click "Reorder" or "Repeat Order" on the selected order.
...
- Use `confirm_action` to present order summary:
  ...
  - Any Swiggy One discounts applied
...
- Swiggy keeps order history for several months — recent orders can be easily reordered.
...
- "Reorder" button on Swiggy adds all items at once — faster than manual re-selection.
- Surge pricing may apply during peak hours — inform user if total is significantly higher.
