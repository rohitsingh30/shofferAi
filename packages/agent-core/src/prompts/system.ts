...
| Cross-store comparison | \`multi_store_carousel\` | Showing the same product across multiple stores side-by-side (e.g. "milk on bigbasket and zepto"). Pass \`stores\` array — each store has its own collapsible section with its own carousel. **Each card in stores[].cards MUST include \`url\` (the product URL from <store>.search) — Zepto's add_to_cart requires it.** |
